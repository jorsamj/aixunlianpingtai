...
def build_coco_dataset(root: Path, image_count: int, payload: bytes) -> None:
...
def build_voc_dataset(root: Path, image_count: int, payload: bytes) -> None:
    ...
    for index in range(image_count):
        filename = f"image_{index:05d}.jpg"
        (images_dir / filename).write_bytes(payload)
        (annotations_dir / f"image_{index:05d}.xml").write_text(
            f"""<annotation>
...
def main():
    args = parse_args()
    root = Path(tempfile.mkdtemp(prefix=f"zip-processing-p2c-{args.format}-profile-")).resolve()
    ...
    project = app.create_project(
        app.ProjectCreate(name=f"zip-processing-p2c-{args.format}-profile", labels=[])
    )
    ...
    payload = tiny_jpeg()
    if args.format == "coco":
        build_coco_dataset(dataset, args.images, payload)
        importer = app._v18_import_coco
    else:
        build_voc_dataset(dataset, args.images, payload)
        importer = app._v18_import_voc
    ...
    report = app.v19_build_report_base(
        {"id": f"profile-{args.format}", "file_name": f"profile-{args.format}.zip"}
    )
    imported = False
    import_seconds = 0.0
    commit_seconds = 0.0
    try:
        ...
        try:
            imported = importer(project_id, dataset, "default", report)
    ...
    result = {
        ...
        "images_per_second": round(args.images / wall_seconds, 3) if wall_seconds else 0.0,
    ...
    print("P2C_PROFILE=" + json.dumps(result, ensure_ascii=False, sort_keys=True))
    print(f"P2C_TOP_CUMULATIVE format={args.format}")
