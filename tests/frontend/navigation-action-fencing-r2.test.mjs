...
test('clean confirmation uses authoritative deleted ids and has no broad reload continuation', () => {
  const owner = liveOwner('window.confirmClean427=async function(id)', 4500);
  assertFenceBefore(owner, 'closeModal()', 'confirmClean427');
  assert.match(owner, /deleted_ids/, 'clean confirmation must patch local images from backend-confirmed deleted_ids');
  assert.doesNotMatch(owner, /await loadRelated\(\)/, 'clean confirmation must not broad-refresh project state after mutation');
});
