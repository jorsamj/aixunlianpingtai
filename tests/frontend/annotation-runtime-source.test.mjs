...
test('canonical manual inspector exposes per-box provenance without a second annotation store', () => {
  ...
  assert.match(source, /const truth=window\.annotationTruthViewV66\?\.\(state\.activeImage\)\|\|\{\}/);
  assert.match(source, /truth\.className==='ai-confirmed'.*'AI已确认'/s);
  assert.match(source, /truth\.className==='imported'.*'导入标注'/s);
  assert.match(source, /truth\.className==='mixed'.*'混合来源'/s);
});
