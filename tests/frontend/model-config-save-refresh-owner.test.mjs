...
const saveEndMarker = '\n  window.testModelConfigM4=async function(id){';
...
test('M4 captures the vision modal as the final model-config owner', () => {
  ...
  const m4Open = app.indexOf('window.openModelConfigM4=function', m4Start);
  const capture = app.indexOf('window.__m4OpenModelConfig=window.openModelConfigM4;', m4Open);
...
test('M4 final activation restores the captured modal without any shadowed function owner', () => {
  const capture = app.indexOf('window.__m4OpenModelConfig=window.openModelConfigM4;');
  const activation = app.indexOf('M4 final activation: later compatibility layers must not replace these contracts.');
  const laterFunctionOwner = app.indexOf('window.openModelConfigM4=function', capture + 1);
  ...
  assert.match(activationRegion, /window\.openModelConfigModalV35=window\.__m4OpenModelConfig/);
});
