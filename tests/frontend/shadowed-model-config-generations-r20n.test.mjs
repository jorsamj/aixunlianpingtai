...
test('R20n retires shadowed model-config generations while preserving final M4 ownership', () => {
  ...
  const openOwners = app.match(/window\.openModelConfigM4=function\(id=''\)/g) || [];
  assert.equal(openOwners.length, 1, 'only the live private M4 model-config owner may remain');
  ...
  assert.match(app, /window\.__m4OpenModelConfig=window\.openModelConfigM4;/, 'M4 private capture must remain');
  assert.match(app, /window\.openModelConfigModalV35=window\.__m4OpenModelConfig;/, 'M4 public activation must remain');
