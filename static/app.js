...
window.quickUltraDetect=async()=>{const r=await safe(api('/api/ultralytics_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}));const list=(r?.candidates||r?.items||[]).filter(x=>x.python_path);if(!list.length)return toast('未检测到 Ultralytics，请确认安装目录');const env=list[0];await safe(api('/api/ultralytics_env/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(env)}));await loadAll();render();toast('已启用本机 Ultralytics')};
...
window.installUsability417=function(){
  ...
  render=function(){const result=baseRender417?.(),badge=document.getElementById('versionBadge'),footer=document.querySelector('.nav-footer b');if(badge)badge.textContent='v42.22.0';if(footer)footer.textContent='v42.22.0';return result};
  [120,600,1600].forEach(delay=>setTimeout(()=>{const badge=document.getElementById('versionBadge'),footer=document.querySelector('.nav-footer b');if(badge)badge.textContent='v42.22.0';if(footer)footer.textContent='v42.22.0'},delay));
