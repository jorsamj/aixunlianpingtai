...
(()=>{
  ...
  function verRow429(a,v){const can=!!String(v.stored_path||'').trim(),isCurrent=String(a.current_version_id||'')===String(v.id||'');return `<div class="alg428-version-row ${isCurrent?'is-current':''}">
    ...
    <div class="alg428-version-actions"><button class="btn mini" onclick="event.stopPropagation();openVersionReport429('${a.id}','${v.id}')">训练报告</button>${v.training_lineage?`<button class="btn mini" onclick="event.stopPropagation();openVersionLineage429('${a.id}','${v.id}')">训练溯源</button>`:''}<button class="btn mini primary" onclick="event.stopPropagation();openVersionConvert428('${a.id}','${v.id}')">转换</button>${can?`<a class="btn mini" onclick="event.stopPropagation()" href="/api/v12/projects/${pid()}/algorithms/${a.id}/versions/${v.id}/download">下载模型</a>`:''}${isCurrent?'':`<button class="btn mini" onclick="event.stopPropagation();openVersionRollback('${a.id}','${v.id}')">回退到此版本</button>`}</div>
  </div>`}
  window.renderAlgorithms423=async function(){
    const inds=[...new Set((state.algorithms||[]).map(a=>a.industry).filter(Boolean))].sort();
