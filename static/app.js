...
(()=>{
  ...
  function queueOrder428(a,b){const priority=priorityValue428(a)-priorityValue428(b);if(priority)return priority;const at=Number(a?.priority_tiebreaker),bt=Number(b?.priority_tiebreaker),hasTie=Number.isFinite(at)&&at>0&&Number.isFinite(bt)&&bt>0;if(hasTie&&at!==bt)return at-bt;const ad=Date.parse(a?.queued_at||a?.created_at||''),bd=Date.parse(b?.queued_at||b?.created_at||'');if(Number.isFinite(ad)&&Number.isFinite(bd)&&ad!==bd)return ad-bd;return String(a?.queued_at||a?.created_at||'').localeCompare(String(b?.queued_at||b?.created_at||''))||String(a?.id||'').localeCompare(String(b?.id||''))}
