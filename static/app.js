...
(()=>{
  ...
  function renderSplit(){
    ...
    if(!root.querySelector('#trV3ResourceStrategy')){
      const field=document.createElement('label');field.className='field train-v3-resource-strategy';
...
(()=>{
  ...
  async function loadReviewPage(offset){const review=state.ai60Review;if(!review)return false;const requestEpoch=Number(review.pageRequestEpoch||0)+1;review.pageRequestEpoch=requestEpoch;const nextOffset=Math.max(0,offset),response=await api(`${taskApi(review.id)}/candidates?limit=${review.limit}&cursor=${nextOffset}`);if(state.ai60Review!==review||Number(review.pageRequestEpoch||0)!==requestEpoch)return false;review.offset=nextOffset;review.total=response.total||0;review.items=(response.items||[]).map(item=>{const id=String(item.image_id),edited=review.edits.get(id);return edited?{...item,boxes:edited.map(box=>({...box}))}:item});if(review.items.some(item=>!item.url)){try{await window.MaterialPaginationRuntime61?.ensureFullPool?.()}catch(_){}}if(state.ai60Review!==review||Number(review.pageRequestEpoch||0)!==requestEpoch)return false;if(Array.isArray(response.label_summary)){review.labelSummary=response.label_summary;for(const row of review.labelSummary){const source=String(row.label||'');if(source&&!review.labelMapping.has(source))review.labelMapping.set(source,source)}}for(const item of review.items){review.seen.set(String(item.image_id),item);if(item.status!=='failed'&&!review.decisions.has(String(item.image_id)))review.decisions.set(String(item.image_id),item.accepted===false?false:true)}renderReviewPage();return true}
  window.reviewAiLabel427=async function(id){try{state.ai60Review={id:String(id),offset:0,limit:24,total:0,items:[],decisions:new Map(),edits:new Map(),seen:new Map(),labelSummary:[],labelMapping:new Map(),pageRequestEpoch:0};await loadReviewPage(0)}catch(error){toast(error.message||error)}};
