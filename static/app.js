...
const api=async(url,opt={})=>{const r=await fetch(url,opt);if(!r.ok){let t=await r.text();try{t=JSON.parse(t).detail||t}catch{}throw new Error(t||'请求失败')}const ct=r.headers.get('content-type')||'';return ct.includes('json')?r.json():r.text()};
...
function renderAlgorithms(){const cards=state.algorithms.map(a=>`<div class="item"><div><div class="item-title">${esc(a.name)}</div><div class="item-sub">${esc(a.remark||'无备注')} · ${(a.versions||[]).length} 个版本</div></div><div class="row"><button class="btn small" onclick="editAlgorithm('${a.id}')">编辑</button><button class="btn small soft" onclick="viewAlgorithm('${a.id}')">版本</button><button class="btn small danger" onclick="delAlgorithm('${a.id}')">删除</button></div></div>`).join('');$('#view').innerHTML=`<section class="panel"><div class="panel-head"><div class="panel-title">算法列表</div><button class="btn primary small" onclick="newAlgorithm()">创建算法</button></div><div class="panel-body"><div class="card-list">${cards||'<div class="empty">先创建算法，例如：安全帽检测、明火烟雾检测</div>'}</div></div></section><section class="panel"><div class="panel-head"><div class="panel-title">待发布模型</div></div><div class="panel-body"><table class="table"><thead><tr><th>模型</th><th>大小</th><th>操作</th></tr></thead><tbody>${state.pending.map(m=>`<tr><td>${esc(m.name)}</td><td>${m.size_mb}MB</td><td><button class="btn small primary" onclick="assignVersion('${esc(m.name)}')">归属算法</button></td></tr>`).join('')||'<tr><td colspan="3">暂无待发布模型</td></tr>'}</tbody></table></div></section>`}
window.newAlgorithm=()=>modal('创建算法',`<div class="form"><div class="field"><label>算法名称</label><input id="aname" class="input" placeholder="例如：安全帽检测"></div><div class="field"><label>备注</label><textarea id="aremark" placeholder="可选"></textarea></div><button class="btn primary" onclick="saveAlgorithm()">创建</button></div>`);
window.saveAlgorithm=async()=>{const name=$('#aname').value.trim();if(!name)return toast('请输入算法名称');await safe(api(`/api/v12/projects/${pid()}/algorithms`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,remark:$('#aremark').value})}));closeModal();await reload();toast('已创建算法')};
window.editAlgorithm=id=>{const a=state.algorithms.find(x=>x.id===id);modal('编辑算法',`<div class="form"><div class="field"><label>算法名称</label><input id="aname" class="input" value="${esc(a.name)}"></div><div class="field"><label>备注</label><textarea id="aremark">${esc(a.remark||'')}</textarea></div><button class="btn primary" onclick="saveEditAlgorithm('${id}')">保存</button></div>`)};
window.saveEditAlgorithm=async id=>{await safe(api(`/api/v12/projects/${pid()}/algorithms/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$('#aname').value,remark:$('#aremark').value})}));closeModal();await reload();toast('已保存')};
window.delAlgorithm=async id=>{if(!confirm('确认删除该算法及所有版本？'))return;await safe(api(`/api/v12/projects/${pid()}/algorithms/${id}`,{method:'DELETE'}));await reload();toast('已删除')};
window.viewAlgorithm=id=>{const a=state.algorithms.find(x=>x.id===id);modal(`${a.name} · 版本管理`,`${(a.versions||[]).map(v=>`<div class="item"><div><div class="item-title">${esc(v.version_name||'未命名版本')} · ${esc(v.model_name)}</div><div class="item-sub">${esc(v.created_at)} · ${v.size_mb}MB · ${esc(v.remark||'')}</div></div><div class="row"><button class="btn small soft" onclick="showReport('${a.id}','${v.id}')">报告</button><a class="btn small" href="/api/v12/projects/${pid()}/algorithms/${a.id}/versions/${v.id}/download">下载</a><button class="btn small danger" onclick="delVersion('${a.id}','${v.id}')">删除</button></div></div>`).join('')||'<div class="empty">暂无版本，请先训练并测试归属</div>'}`)};
window.showReport=(aid,vid)=>{const a=state.algorithms.find(x=>x.id===aid);const v=a.versions.find(x=>x.id===vid);const rep=v.report||{};modal('训练报告',`<div class="report">${Object.entries(rep).map(([k,val])=>`<div><b>${esc(k)}</b><span>${esc(val)}</span></div>`).join('')||'<div>暂无报告</div>'}</div>`)};
window.delVersion=async(aid,vid)=>{if(!confirm('确认删除该版本？'))return;await safe(api(`/api/v12/projects/${pid()}/algorithms/${aid}/versions/${vid}`,{method:'DELETE'}));closeModal();await reload();toast('已删除版本')};
...
function renderResources(){
  ...
  $('#view').innerHTML=`<div class="resource-layout"><section class="panel"><div class="panel-head"><div class="panel-title">接入训练资源</div><button class="btn small" onclick="loadAll().then(render)">刷新</button></div><div class="panel-body"><div class="resource-actions"><div class="quick-card"><div class="quick-title">本机 Ultralytics</div><div class="field"><label>安装目录</label><input id="uroot" class="input" value="D:\\lab\\yolosuanfa\\Ultralytics"></div><div class="row"><button class="btn primary small" onclick="detectUltra()">检测并启用</button><button class="btn soft small" onclick="quickUltraDetect()">一键检测常用目录</button></div></div><div class="quick-card"><div class="quick-title">本机模型目录</div><div class="field"><label>模型目录</label><input id="scanRoot" class="input" value="D:\\lab\\yolosuanfa\\models"></div><button class="btn small" onclick="scanModels()">扫描模型</button></div><div class="quick-card"><div class="quick-title">训练服务器</div><div class="field"><label>服务地址</label><input id="quickServerUrl" class="input" placeholder="http://192.168.1.10:8020"></div><button class="btn soft small" onclick="quickAddServer()">接入服务器</button></div></div></div></section><section class="panel"><div class="panel-head"><div class="panel-title">已接入资源</div></div><div class="panel-body"><div class="resource-grid">${resourceCards||'<div class="empty">暂无资源。先检测本机 Ultralytics。</div>'}</div></div></section></div>`;
}
window.quickUltraDetect=async()=>{const roots=['D:\\lab\\yolosuanfa\\Ultralytics','D:\\lab\\yolosuanfa','D:\\Ultralytics','C:\\Ultralytics'];const r=await safe(api('/api/ultralytics_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots})}));const list=(r?.candidates||r?.items||[]).filter(x=>x.python_path);if(!list.length)return toast('未检测到 Ultralytics，请确认安装目录');const env=list[0];await safe(api('/api/ultralytics_env/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(env)}));await loadAll();render();toast('已启用本机 Ultralytics')};
window.detectUltra=async()=>{const root=$('#uroot').value.trim();const r=await safe(api('/api/ultralytics_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots:[root]})}));const env=(r?.candidates||r?.items||[])[0]||r?.active;if(!env||!env.python_path)return toast('没有检测到 Ultralytics 环境');await safe(api('/api/ultralytics_env/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(env)}));await loadAll();render();toast('已启用 Ultralytics 环境')};
window.scanModels=async()=>{const roots=[$('#scanRoot').value.trim()].filter(Boolean);const r=await safe(api('/api/local_models/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots})}));await loadAll();render();toast(`扫描完成：${r?.total||0} 个文件`)};
...
function renderDatasets(){
  ...
  const filtered=state.images.filter(img=>state.imageFilter==='all'||(state.imageFilter==='marked'?(img.box_count||0)>0:state.imageFilter==='unmarked'?(img.box_count||0)===0:(img.split||'train')===state.imageFilter));
  const dsCards=ds.map(d=>`<div class="dataset-card ${state.datasetId===d.id?'active':''}" onclick="selectDataset('${d.id}')"><div><div class="item-title">${esc(d.name)}</div><div class="item-sub">${d.images} 图 · ${d.annotated_images} 已标注 · ${d.boxes} 框</div></div><div class="row"><button class="btn mini" onclick="event.stopPropagation();editDataset('${d.id}')">编辑</button><button class="btn mini danger" onclick="event.stopPropagation();delDataset('${d.id}')">删</button></div></div>`).join('');
  const labelBar=state.labels.map(l=>`<span class="label-chip"><i style="background:${l.color}"></i>${esc(l.display_name)}<b>${esc(l.hotkey||'')}</b></span>`).join('')||'<span class="muted">暂无标签，点击“标签库”创建。</span>';
  const thumbs=filtered.map((img,idx)=>`<div class="thumb"><div class="thumb-img-wrap"><img src="${img.url}" onclick="openAnnotation('${img.id}')"><span class="corner-badge">${img.box_count||0}框</span></div><div class="thumb-bd"><div class="thumb-title" title="${esc(img.filename)}">${esc(img.filename)}</div><div class="row between"><select class="select split-select" onchange="setImageSplit('${img.id}',this.value)"><option value="train" ${(img.split||'train')==='train'?'selected':''}>训练</option><option value="val" ${img.split==='val'?'selected':''}>评测</option><option value="test" ${img.split==='test'?'selected':''}>试验</option></select><button class="btn mini primary" onclick="openAnnotation('${img.id}')">标注</button></div></div></div>`).join('');
  $('#view').innerHTML=`<div class="dataset-shell"><section class="panel dataset-side"><div class="panel-head"><div class="panel-title">数据集</div><button class="btn primary small" onclick="newDataset()">新建</button></div><div class="panel-body"><div class="dataset-list">${dsCards||'<div class="empty">暂无数据集</div>'}</div></div></section><section class="panel dataset-main"><div class="panel-head"><div><div class="panel-title">${esc(cur.name||'数据集')}</div><div class="subline">图片上传后可直接点击标注</div></div><div class="panel-actions"><input id="uploadImages" type="file" multiple accept="image/*" class="file"><button class="btn primary small" onclick="uploadImages()">上传</button><button class="btn soft small" onclick="importData()">导入标注</button><button class="btn small" onclick="manageLabels()">标签库</button></div></div><div class="panel-body"><div class="tool-row"><div class="seg"><button class="${state.imageFilter==='all'?'on':''}" onclick="state.imageFilter='all';render()">全部</button><button class="${state.imageFilter==='unmarked'?'on':''}" onclick="state.imageFilter='unmarked';render()">未标注</button><button class="${state.imageFilter==='marked'?'on':''}" onclick="state.imageFilter='marked';render()">已标注</button><button class="${state.imageFilter==='train'?'on':''}" onclick="state.imageFilter='train';render()">训练</button><button class="${state.imageFilter==='val'?'on':''}" onclick="state.imageFilter='val';render()">评测</button><button class="${state.imageFilter==='test'?'on':''}" onclick="state.imageFilter='test';render()">试验</button></div><div class="row"><button class="btn small" onclick="autoSplit()">自动划分</button><button class="btn small" onclick="checkDatasetQuality()">训练前质检</button><button class="btn green small" onclick="buildYolo()">生成训练文件</button><a class="btn soft small" href="/api/v12/projects/${pid()}/datasets/${state.datasetId}/export/yolo">导出YOLO</a><a class="btn soft small" href="/api/v12/projects/${pid()}/datasets/${state.datasetId}/export/coco">导出COCO</a></div></div><div class="label-strip">${labelBar}</div><div class="thumbs ${filtered.length?'':'empty-grid'}">${thumbs||'<div class="empty">还没有图片。上传普通图片后，点击图片即可框选标注。</div>'}</div></div></section></div>`;
}

window.selectDataset=async id=>{state.datasetId=id;await loadRelated();render()};
window.newDataset=()=>modal('新建数据集',`<div class="form"><div class="field"><label>数据集名称</label><input id="dname" class="input" placeholder="例如：安全帽第一批"></div><div class="field"><label>说明</label><textarea id="ddesc"></textarea></div><button class="btn primary" onclick="saveDataset()">创建</button></div>`);
window.saveDataset=async()=>{const name=$('#dname').value.trim();if(!name)return toast('请输入数据集名称');const d=await safe(api(`/api/projects/${pid()}/datasets`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,description:$('#ddesc').value,kind:'mixed'})}));if(d)state.datasetId=d.id;closeModal();await reload();toast('已创建数据集')};
window.editDataset=id=>{const d=state.datasets.find(x=>x.id===id);modal('编辑数据集',`<div class="form"><div class="field"><label>数据集名称</label><input id="dname" class="input" value="${esc(d.name)}"></div><div class="field"><label>说明</label><textarea id="ddesc">${esc(d.description||'')}</textarea></div><button class="btn primary" onclick="saveEditDataset('${id}')">保存</button></div>`)};
window.saveEditDataset=async id=>{await safe(api(`/api/projects/${pid()}/datasets/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$('#dname').value,description:$('#ddesc').value,kind:'mixed'})}));closeModal();await reload();toast('已保存')};
window.delDataset=async id=>{if(!confirm('确认删除数据集？图片记录不会自动删除。'))return;await safe(api(`/api/projects/${pid()}/datasets/${id}`,{method:'DELETE'}));state.datasetId='default';await reload();toast('已删除')};
window.uploadImages=async()=>{const files=$('#uploadImages').files;if(!files.length)return toast('请选择图片');const fd=new FormData();[...files].forEach(f=>fd.append('files',f));fd.append('dataset_id',state.datasetId);await safe(api(`/api/projects/${pid()}/images`,{method:'POST',body:fd}));await loadRelated();render();toast('图片已上传')};
...
window.doImportData=()=>{const f=$('#importFile').files[0];if(!f)return toast('请选择压缩包');const fd=new FormData();fd.append('file',f);const wrap=$('#importProgressWrap'),bar=$('#importProgressBar'),txt=$('#importProgressText'),pct=$('#importProgressPercent'),res=$('#importResult');wrap.classList.remove('hidden');bar.style.width='0%';pct.textContent='0%';txt.textContent='正在上传';res.innerHTML='';const xhr=new XMLHttpRequest();xhr.open('POST',`/api/v18/projects/${pid()}/datasets/${state.datasetId}/import`,true);xhr.upload.onprogress=e=>{if(e.lengthComputable){const p=Math.max(1,Math.round(e.loaded/e.total*100));bar.style.width=p+'%';pct.textContent=p+'%';if(p>=100)txt.textContent='上传完成，正在解析数据集';}};xhr.onerror=()=>{txt.textContent='导入失败';res.innerHTML='<div class="alert warn">网络或服务异常，请查看启动窗口日志。</div>'};xhr.onload=async()=>{let r=null;try{r=JSON.parse(xhr.responseText||'{}')}catch(e){}if(xhr.status>=200&&xhr.status<300&&r){bar.style.width='100%';pct.textContent='100%';txt.textContent='导入完成';const warns=(r.warnings||[]).map(x=>`<div class="alert warn">${esc(x)}</div>`).join('');res.innerHTML=`<div class="import-result"><div class="stat"><div class="k">格式</div><div class="v">${esc(r.detected_format||'-')}</div></div><div class="stat"><div class="k">图片</div><div class="v">${r.imported_images||0}</div></div><div class="stat"><div class="k">已标注</div><div class="v">${r.annotated_images||0}</div></div><div class="stat"><div class="k">标注框</div><div class="v">${r.boxes||0}</div></div></div>${warns}`;await reload();toast(`导入完成：${r.imported_images||0}图，${r.boxes||0}框`);}else{const msg=(r&&r.detail)||xhr.responseText||'导入失败';txt.textContent='导入失败';res.innerHTML=`<div class="alert warn">${esc(msg)}</div>`;}};xhr.send(fd);};
window.autoSplit=async()=>{await safe(api(`/api/v12/projects/${pid()}/datasets/${state.datasetId}/auto_split`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({train:.7,val:.2,test:.1,include_unannotated:true})}));await reload();toast('已划分训练/评测/试验集')};
window.buildYolo=async()=>{const r=await safe(api(`/api/v12/projects/${pid()}/dataset/build`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({dataset_id:state.datasetId,include_empty:false,train_ratio:.8})}));if(r){await loadRelated();const c=r.counts||{};toast(`训练文件已生成：训练${c.train||0}图/${c.train_boxes||0}框，评测${c.val||0}图/${c.val_boxes||0}框`)}};
window.checkDatasetQuality=async()=>{const r=await safe(api(`/api/v17/projects/${pid()}/datasets/${state.datasetId}/quality`));if(!r)return;const q=r.quality||{};modal('训练前质检',`<div class="quality-grid"><div class="stat"><div class="k">图片</div><div class="v">${q.image_count||0}</div></div><div class="stat"><div class="k">已标注</div><div class="v">${q.annotated_images||0}</div></div><div class="stat"><div class="k">标注框</div><div class="v">${q.box_count||0}</div></div><div class="stat"><div class="k">状态</div><div class="v">${q.can_train?'可训练':'不可训练'}</div></div></div><div class="divider"></div><table class="table"><thead><tr><th>用途</th><th>图片</th><th>框</th></tr></thead><tbody>${['train','val','test'].map(k=>`<tr><td>${k==='train'?'训练集':k==='val'?'评测集':'试验集'}</td><td>${q.splits?.[k]?.images||0}</td><td>${q.splits?.[k]?.boxes||0}</td></tr>`).join('')}</tbody></table><div class="divider"></div>${(q.warnings||[]).map(x=>`<div class="alert warn">${esc(x)}</div>`).join('')||'<div class="alert ok">质检通过，可以生成训练文件。</div>'}`,true)};
window.setImageSplit=async(id,split)=>{await safe(api(`/api/v12/projects/${pid()}/images/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({split})}));await loadRelated()};
...
window.startTrain=async()=>{const t=curTarget();if(!t)return toast('请先接入训练资源');if(!$('#trainDataset').value)return toast('请选择训练数据集');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:$('#alg').value,model:$('#model').value,epochs:+$('#epochs').value,imgsz:+$('#imgsz').value,batch:+$('#batch').value,device:$('#device').value,dataset_id:$('#trainDataset').value,include_empty:false,train_ratio:.8};const r=await safe(api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}));if(r){await reload();toast('训练已启动')}};
...
function renderImportJobsPanel(){
  const jobs=state.importJobs||[];
  return `<div class="import-jobs">${jobs.map(j=>renderImportJobCard(j)).join('')||'<div class="empty">暂无导入任务</div>'}</div><div class="row end"><button class="btn soft" onclick="closeModal()">关闭</button><button class="btn" onclick="loadImportJobs().then(()=>{document.getElementById('modalBody').innerHTML=renderImportJobsPanel()})">刷新</button></div>`;
}
...
window.deleteImportJob=async(id)=>{if(!confirm('确认删除这个导入任务记录？'))return;await safe(api(`/api/v19/projects/${pid()}/import/jobs/${id}`,{method:'DELETE'}));await loadImportJobs();if(!$('#modal').classList.contains('hidden'))$('#modalBody').innerHTML=renderImportJobsPanel();};
...
function renderDatasets(){
  if(!state.selectedImages)state.selectedImages=new Set();
  const ds=state.datasets||[];
  const cur=currentDataset()||{};
  const filtered=state.images.filter(img=>state.imageFilter==='all'||(state.imageFilter==='marked'?(img.box_count||0)>0:state.imageFilter==='unmarked'?(img.box_count||0)===0:(img.split||'train')===state.imageFilter));
  const visibleIds=new Set(filtered.map(x=>x.id));
  [...state.selectedImages].forEach(id=>{if(!state.images.find(x=>x.id===id))state.selectedImages.delete(id)});
  const dsCards=ds.map(d=>`<div class="dataset-card ${state.datasetId===d.id?'active':''}" onclick="selectDataset('${d.id}')"><div><div class="item-title">${esc(d.name)}</div><div class="item-sub">${d.images} 图 · ${d.annotated_images} 已标注 · ${d.boxes} 框</div></div><div class="row"><button class="btn mini" onclick="event.stopPropagation();editDataset('${d.id}')">编辑</button><button class="btn mini danger" onclick="event.stopPropagation();delDataset('${d.id}')">删</button></div></div>`).join('');
  const labelBar=state.labels.map(l=>`<span class="label-chip"><i style="background:${l.color}"></i>${esc(l.display_name)}<b>${esc(l.hotkey||'')}</b></span>`).join('')||'<span class="muted">暂无标签，点击“标签库”创建。</span>';
  const thumbs=filtered.map(img=>`<div class="thumb"><label class="thumb-check"><input type="checkbox" class="imgChk" value="${img.id}" ${state.selectedImages.has(img.id)?'checked':''} onchange="toggleOneImage('${img.id}',this.checked)">选择</label><div class="thumb-img-wrap"><img src="${img.url}" onclick="openAnnotation('${img.id}')"><span class="corner-badge">${img.box_count||0}框</span></div><div class="thumb-bd"><div class="thumb-title" title="${esc(img.filename)}">${esc(img.filename)}</div><div class="row between"><select class="select split-select" onchange="setImageSplit('${img.id}',this.value)"><option value="train" ${(img.split||'train')==='train'?'selected':''}>训练</option><option value="val" ${img.split==='val'?'selected':''}>评测</option><option value="test" ${img.split==='test'?'selected':''}>试验</option></select><button class="btn mini primary" onclick="openAnnotation('${img.id}')">标注</button></div></div></div>`).join('');
  $('#view').innerHTML=`<div class="dataset-shell"><section class="panel dataset-side"><div class="panel-head"><div class="panel-title">数据集</div><button class="btn primary small" onclick="newDataset()">新建</button></div><div class="panel-body"><div class="dataset-list">${dsCards||'<div class="empty">暂无数据集</div>'}</div></div></section><section class="panel dataset-main"><div class="panel-head"><div><div class="panel-title">${esc(cur.name||'数据集')}</div><div class="subline">图片上传后可直接点击标注；支持批量移动到训练集、评测集、试验集。</div></div><div class="panel-actions"><input id="uploadImages" type="file" multiple accept="image/*" class="file"><button class="btn primary small" onclick="uploadImages()">上传</button><button class="btn soft small" onclick="importData()">导入标注</button><button class="btn small" onclick="manageLabels()">标签库</button></div></div><div class="panel-body"><div class="tool-row"><div class="seg"><button class="${state.imageFilter==='all'?'on':''}" onclick="state.imageFilter='all';render()">全部</button><button class="${state.imageFilter==='unmarked'?'on':''}" onclick="state.imageFilter='unmarked';render()">未标注</button><button class="${state.imageFilter==='marked'?'on':''}" onclick="state.imageFilter='marked';render()">已标注</button><button class="${state.imageFilter==='train'?'on':''}" onclick="state.imageFilter='train';render()">训练</button><button class="${state.imageFilter==='val'?'on':''}" onclick="state.imageFilter='val';render()">评测</button><button class="${state.imageFilter==='test'?'on':''}" onclick="state.imageFilter='test';render()">试验</button></div><div class="row"><button class="btn small" onclick="toggleVisibleImages(true)">全选当前</button><button class="btn small" onclick="clearSelectedImages()">清空选择</button><button class="btn small primary" onclick="bulkMoveMenu()">批量移动 <span id="selCount">${selectedCount()}</span></button><button class="btn small" onclick="autoSplit()">自动划分</button><button class="btn small" onclick="checkDatasetQuality()">训练前质检</button><button class="btn green small" onclick="buildYolo()">生成训练文件</button><a class="btn soft small" href="/api/v12/projects/${pid()}/datasets/${state.datasetId}/export/yolo">导出YOLO</a><a class="btn soft small" href="/api/v12/projects/${pid()}/datasets/${state.datasetId}/export/coco">导出COCO</a></div></div><div class="label-strip">${labelBar}</div><div class="thumbs ${filtered.length?'':'empty-grid'}">${thumbs||'<div class="empty">还没有图片。上传普通图片后，点击图片即可框选标注。</div>'}</div></div></section></div>`;
}
...
function renderResources(){
  ...
  $('#view').innerHTML=`<div class="resource-layout"><section class="panel"><div class="panel-head"><div class="panel-title">接入训练资源</div><button class="btn small" onclick="loadAll().then(render)">刷新</button></div><div class="panel-body"><div class="resource-actions"><div class="quick-card"><div class="quick-title">本机 Ultralytics</div><div class="field"><label>安装目录</label><input id="uroot" class="input" value="D:\\lab\\yolosuanfa\\Ultralytics"></div><div class="row"><button class="btn primary small" onclick="detectUltra()">检测并启用</button><button class="btn soft small" onclick="quickUltraDetect()">一键检测常用目录</button></div></div><div class="quick-card"><div class="quick-title">本机飞桨</div><div class="field"><label>Python路径</label><input id="ppy" class="input" placeholder="D:\\paddle_env\\Scripts\\python.exe"></div><div class="field"><label>PaddleDetection目录</label><input id="pdet" class="input" value="D:\\PaddleDetection"></div><div class="field"><label>PaddleX目录</label><input id="pxdir" class="input" value="D:\\PaddleX"></div><div class="row"><button class="btn primary small" onclick="detectPaddle()">检测并启用</button><button class="btn soft small" onclick="quickPaddleDetect()">一键检测</button><button class="btn small" onclick="testPaddle()">只检测</button></div><div id="paddleTestResult" class="item-sub"></div></div><div class="quick-card"><div class="quick-title">本机模型目录</div><div class="field"><label>模型目录</label><input id="scanRoot" class="input" value="D:\\lab\\yolosuanfa\\models"></div><button class="btn small" onclick="scanModels()">扫描模型</button></div><div class="quick-card"><div class="quick-title">训练服务器</div><div class="field"><label>服务地址</label><input id="quickServerUrl" class="input" placeholder="http://192.168.1.10:8020"></div><button class="btn soft small" onclick="quickAddServer()">接入服务器</button></div></div></div></section><section class="panel"><div class="panel-head"><div class="panel-title">已接入资源</div></div><div class="panel-body"><div class="resource-grid">${resourceCards||'<div class="empty">暂无资源。先检测本机 Ultralytics 或飞桨。</div>'}</div></div></section></div>`;
}
...
window.quickPaddleDetect=async()=>{const r=await safe(api('/api/paddle_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots:['D:\\PaddleDetection','D:\\PaddleX','D:\\lab\\yolosuanfa','D:\\lab']})}));const env=(r?.candidates||[])[0];if(!env)return toast('未检测到飞桨环境，请手动填写 Python 路径');await safe(api('/api/paddle_env/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(env)}));await loadAll();render();toast('已启用飞桨环境')};
...
window.startTrain=async()=>{const t=curTarget();if(!t)return toast('请先接入训练资源');if(!$('#trainDataset').value)return toast('请选择训练数据集');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:$('#alg').value,model:$('#model').value,epochs:+$('#epochs').value,imgsz:+$('#imgsz').value,batch:+$('#batch').value,device:$('#device').value,dataset_id:$('#trainDataset').value,include_empty:false,train_ratio:.8,patience:+$('#patience').value,workers:+$('#workers').value,optimizer:$('#optimizer').value,lr0:+$('#lr0').value,lrf:+$('#lrf').value,weight_decay:+$('#weight_decay').value,close_mosaic:+$('#close_mosaic').value,mosaic:+$('#mosaic').value,cache:$('#cache').value,single_cls:$('#single_cls').checked,pretrained:$('#pretrained').checked,rect:$('#rect').checked,amp:$('#amp')?.checked!==false,cos_lr:!!$('#cos_lr')?.checked,freeze:+($('#freeze')?.value||0)};const r=await safe(api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}));if(r){await reload();toast('训练已启动')}};
...
function renderResources(){
  ...
  $('#view').innerHTML=`<div class="resource-layout"><section class="panel"><div class="panel-head"><div class="panel-title">接入训练资源</div><button class="btn small" onclick="loadAll().then(render)">刷新</button></div><div class="panel-body"><div class="resource-actions"><div class="quick-card"><div class="quick-title">本机 Ultralytics</div><div class="field"><label>安装目录</label><input id="uroot" class="input" value="D:\\lab\\yolosuanfa\\Ultralytics"></div><div class="row"><button class="btn primary small" onclick="detectUltra()">检测并启用</button><button class="btn soft small" onclick="quickUltraDetect()">一键检测常用目录</button></div></div><div class="quick-card"><div class="quick-title">本机飞桨</div><div class="field"><label>Python路径</label><input id="ppy" class="input" value="${esc((state.targets||[]).find(t=>t.id==='local_paddle')?.python_path||'D:\\lab\\yolosuanfa\\PaddleEnv\\.venv\\Scripts\\python.exe')}"></div><div class="field"><label>PaddleDetection目录</label><input id="pdet" class="input" value="${esc((state.targets||[]).find(t=>t.id==='local_paddle')?.paddledet_dir||'D:\\lab\\yolosuanfa\\PaddleDetection')}"></div><div class="field"><label>PaddleX目录</label><input id="pxdir" class="input" value="${esc((state.targets||[]).find(t=>t.id==='local_paddle')?.paddlex_dir||'')}"></div><div class="row"><button class="btn primary small" onclick="detectPaddle()">检测并启用</button><button class="btn soft small" onclick="quickPaddleDetect()">一键检测</button><button class="btn small" onclick="testPaddle()">只检测</button></div><div id="paddleTestResult" class="item-sub"></div></div><div class="quick-card"><div class="quick-title">本机模型目录</div><div class="field"><label>模型目录</label><input id="scanRoot" class="input" value="D:\\lab\\yolosuanfa\\models"></div><button class="btn small" onclick="scanModels()">扫描模型</button></div><div class="quick-card"><div class="quick-title">训练服务器</div><div class="field"><label>服务地址</label><input id="quickServerUrl" class="input" placeholder="http://192.168.1.10:8020"></div><button class="btn soft small" onclick="quickAddServer()">接入服务器</button></div></div></div></section><section class="panel"><div class="panel-head"><div class="panel-title">已接入资源</div></div><div class="panel-body"><div class="resource-grid">${resourceCards||'<div class="empty">暂无资源。先检测本机 Ultralytics 或飞桨。</div>'}</div></div></section></div>`;
}
...
window.quickPaddleDetect=async()=>{const r=await safe(api('/api/paddle_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots:['D:\\lab\\yolosuanfa','D:\\lab','D:\\PaddleDetection','D:\\PaddleX']})}));const env=(r?.candidates||[])[0];if(!env)return toast('未检测到飞桨环境，请手动填写 Python 路径');await safe(api('/api/paddle_env/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(env)}));await loadAll();render();toast(`已启用飞桨环境，识别到 ${env.algorithm_scan?.total||0} 个算法配置`)};
...
window.startTrain=async()=>{const t=curTarget();if(!t)return toast('请先接入训练资源');if(!$('#trainDataset').value)return toast('请选择训练数据集');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:$('#alg').value,model:$('#model').value,epochs:+$('#epochs').value,imgsz:+$('#imgsz').value,batch:+$('#batch').value,device:$('#device').value,dataset_id:$('#trainDataset').value,include_empty:false,train_ratio:.8,patience:+$('#patience').value,workers:+$('#workers').value,optimizer:$('#optimizer').value,lr0:+$('#lr0').value,lrf:+$('#lrf').value,weight_decay:+$('#weight_decay').value,close_mosaic:+$('#close_mosaic').value,mosaic:+$('#mosaic').value,cache:$('#cache').value,single_cls:$('#single_cls').checked,pretrained:$('#pretrained').checked,rect:$('#rect').checked,amp:$('#amp')?.checked!==false,cos_lr:!!$('#cos_lr')?.checked,freeze:+($('#freeze')?.value||0)};const r=await safe(api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}));if(r){state.activeLogJob=r.job?.id||null;await loadAll();render();toast('训练已启动，状态会自动刷新')}};
...
(function(){
  ...
  window.startTrain=async()=>{
    const t=curTarget();if(!t)return toast('请先接入训练资源');
    if(!$('#trainDataset').value)return toast('请选择训练数据集');
    const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:$('#alg').value,model:$('#model').value,epochs:+$('#epochs').value,imgsz:+$('#imgsz').value,batch:+$('#batch').value,device:$('#device').value,dataset_id:$('#trainDataset').value,include_empty:false,train_ratio:.8,patience:+$('#patience').value,workers:+$('#workers').value,optimizer:$('#optimizer').value,lr0:+$('#lr0').value,lrf:+$('#lrf').value,weight_decay:+$('#weight_decay').value,close_mosaic:+$('#close_mosaic').value,mosaic:+$('#mosaic').value,cache:$('#cache').value,single_cls:$('#single_cls').checked,pretrained:$('#pretrained').checked,rect:$('#rect').checked,amp:$('#amp')?.checked!==false,cos_lr:!!$('#cos_lr')?.checked,freeze:+($('#freeze')?.value||0),paddle_eval:!!$('#paddle_eval')?.checked};
    const r=await safe(api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}));
    if(r){state.activeLogJob=r.job?.id||null;await refreshJobsOnly();if(state.activeLogJob) await showLog(state.activeLogJob,true);toast('训练已启动，任务状态会在右侧刷新')}
  };
...
(function(){
  ...
  window.renderAlgorithms=function(){
...
(function(){
  ...
  Promise.resolve().then(async()=>{state.page=lastState.page&&RENDER_MAP()[lastState.page]?lastState.page:'工作台';await loadAll();render();state.uiReady=true;});
})();
...
(function(){
  ...
  window.detectUltra=async function(){
    const btn=window.event?.currentTarget; setBtnBusy(btn,true,'检测中'); state.resourceBusy=true;
    try{
      const root=$('#uroot')?.value.trim()||'';
      const r=await api('/api/ultralytics_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots:[root]})});
      const env=(r?.candidates||r?.items||[])[0]||r?.active;
      if(!env||!env.python_path) throw new Error('没有检测到 Ultralytics 环境');
      await api('/api/ultralytics_env/select',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(env)});
      const cur='训练资源'; await loadAll(); state.page=cur; render(); toast('Ultralytics 已启用');
  ...
  window.quickPaddleDetect=async function(){
    ...
    try{
      const r=await api('/api/paddle_env/detect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({roots:['D:\\lab\\yolosuanfa','D:\\PaddleDetection','D:\\lab','D:\\']})});
  ...
  setTimeout(()=>{ if(state?.project){ state.versionInfo={...(state.versionInfo||{}),version:'36.0.0'}; render(); }},80);
})();
...
(function(){
  ...
  renderTop = function(){ oldRenderTopV36(); if($('#versionBadge')) $('#versionBadge').textContent='v36.0.0'; };
  ...
  window.importData=function(){
    modal('导入素材 / 标注',`<div class="form">
      ...
      <div id="sourceImportPane" class="hidden">
        <div class="field"><label>本机路径或服务器URL</label><input id="sourcePathV36" class="input" placeholder="D:\\datasets\\fire 或 \\\\192.168.1.20\\share\\dataset 或 http://server/dataset.zip"></div>
  ...
  setTimeout(()=>{ if(state?.project){ state.versionInfo={...(state.versionInfo||{}),version:'36.0.0'}; render(); }},100);
})();
...
(function(){
  ...
  const oldViewAlgoV39=window.viewAlgorithm;
  window.viewAlgorithm=id=>{const a=state.algorithms.find(x=>x.id===id);if(!a)return oldViewAlgoV39(id);modal(`${a.name} · 版本管理`,`${(a.versions||[]).map(v=>`<div class="item"><div><div class="item-title">${esc(v.version_name||'未命名版本')} · ${esc(v.model_name)}</div><div class="item-sub">${esc(v.created_at)} · ${v.size_mb}MB · ${esc(v.remark||'')}</div></div><div class="row"><button class="btn small primary" onclick="startDeployVersion('${a.id}','${v.id}')">部署转换</button><button class="btn small soft" onclick="showReport('${a.id}','${v.id}')">报告</button><a class="btn small" href="/api/v12/projects/${pid()}/algorithms/${a.id}/versions/${v.id}/download">下载</a><button class="btn small danger" onclick="delVersion('${a.id}','${v.id}')">删除</button></div></div>`).join('')||'<div class="empty">暂无版本</div>'}`,true)};
...
(function(){
  ...
  setTimeout(()=>{const v=document.getElementById('versionBadge');if(v)v.textContent='v42.2.0'},150);
})();
...
(function(){
  ...
  function cleanup(root){
    ...
    root.querySelectorAll('input[placeholder],textarea[placeholder]').forEach(el=>{const v=el.getAttribute('placeholder')||'';if(/^(例如|如：|如 |选填|多个|rtsp|http|D:\\|\\\\)/i.test(v))el.removeAttribute('placeholder')});
    const desc=document.getElementById('pageDesc');if(desc)desc.textContent='';
  }
...
(function(){
  ...
  function renderAlgorithmCards422(){
    const box=document.getElementById('alg422List');if(!box)return;const rows=algorithmRows422();
    box.innerHTML=rows.map(({a,b,m,industry,trained,last})=>`<article class="alg422-row">
      <div class="alg422-mark">${esc((a.name||'算').slice(0,1))}</div><div class="alg422-main"><div class="alg422-name-line"><b>${esc(a.name)}</b>${trained?'<span class="pill ok">已训练</span>':'<span class="pill warn">未训练</span>'}<span class="alg422-industry">${esc(industry)}</span></div><div class="alg422-id">${esc(a.id)}</div><div class="alg422-tags">${(b.labels||[]).slice(0,6).map(x=>`<span>${esc(x)}</span>`).join('')||'<span>未配置标签</span>'}</div></div>
      <div class="alg422-metrics"><div><span>版本</span><b>${(a.versions||[]).length}</b></div><div><span>Precision</span><b>${pct423(m.metrics?.precision)}</b></div><div><span>Recall</span><b>${pct423(m.metrics?.recall)}</b></div><div><span>mAP50</span><b>${pct423(m.metrics?.map50)}</b></div></div>
      <div class="alg422-actions"><button class="btn small" onclick="viewAlgorithm('${a.id}')">详情</button><button class="btn small" onclick="editAlgorithm('${a.id}')">编辑</button><button class="btn small primary" onclick="setPage('训练任务')">训练</button><button class="btn small danger" onclick="delAlgorithm('${a.id}')">删除</button></div>
    </article>`).join('')||'<div class="empty">暂无算法</div>';
  }
  window.filterAlgorithms422=renderAlgorithmCards422;
  window.renderAlgorithms422=async function(){await loadBlueprints423();const inds=[...new Set((state.v42.blueprints||[]).map(x=>x.industry).filter(Boolean))].sort();document.getElementById('view').innerHTML=`<section class="alg422-shell"><div class="alg422-toolbar"><div class="filter422"><input id="alg422Q" class="input" placeholder="搜索算法" oninput="filterAlgorithms422()"><select id="alg422Industry" class="select" onchange="filterAlgorithms422()"><option value="all">全部分类</option>${inds.map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="alg422Status" class="select" onchange="filterAlgorithms422()"><option value="all">全部状态</option><option value="trained">已训练</option><option value="untrained">未训练</option></select></div><button class="btn primary" onclick="openNewAlgorithm422()">＋ 新建算法</button></div><div id="alg422List" class="alg422-list"></div></section>`;renderAlgorithmCards422()};
  window.openNewAlgorithm422=async function(){if(!(state.v42.templates||[]).length){const r=await safe(api('/api/v42/industry-templates'));state.v42.templates=r?.items||[];}const industries=[...new Set((state.v42.templates||[]).map(x=>x.industry).filter(Boolean))];modal('新建算法',`<div class="form two"><div class="field"><label>算法名称</label><input id="n422Name" class="input"></div><div class="field"><label>行业 / 场景</label><input id="n422Industry" class="input" list="n422IndustryList"><datalist id="n422IndustryList">${industries.map(x=>`<option value="${esc(x)}">`).join('')}</datalist></div><div class="field full"><label>检测标签</label><input id="n422Labels" class="input"></div><div class="field full"><label>重点场景</label><textarea id="n422Focus"></textarea></div><div class="field full"><label>反例 / 易误报场景</label><textarea id="n422Negative"></textarea></div><div class="field full"><label>备注</label><input id="n422Remark" class="input"></div></div><div class="row end"><button class="btn" onclick="closeModal()">取消</button><button class="btn primary" onclick="saveNewAlgorithm422()">创建</button></div>`,true)};
  window.saveNewAlgorithm422=async function(){const labels=(document.getElementById('n422Labels')?.value||'').split(/[,，\n]/).map(x=>x.trim()).filter(Boolean);const name=document.getElementById('n422Name')?.value.trim();if(!name)return toast('请输入算法名称');try{await api(`/api/v42/projects/${pid()}/algorithm-blueprints`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,industry:document.getElementById('n422Industry')?.value||'',template_id:'custom',labels,focus_scenes:document.getElementById('n422Focus')?.value||'',negative_scenes:document.getElementById('n422Negative')?.value||'',remark:document.getElementById('n422Remark')?.value||''})});closeModal();state.v42.loaded=false;await loadAll();renderAlgorithms422();toast('算法已创建')}catch(e){toast(e.message||e)}};
  ...
  window.setPage=function(p){clearInterval(state.source422Timer);clearInterval(state.auto422Timer);if(p==='新建算法'||p==='自动迭代')p='算法列表';set422Base(p)};try{setPage=window.setPage}catch(e){}
...
(function(){
  ...
  window.startTrain423=async function(){
    const t=curTarget(),asset=selectedAsset423();if(!asset)return toast('请选择算法');if(!t)return toast('当前算法没有可用训练资源');if(!document.getElementById('trainDataset')?.value)return toast('请选择训练数据集');
    const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:document.getElementById('alg')?.value||'',algorithm_asset_id:asset.id,model:document.getElementById('model')?.value||'',epochs:+(document.getElementById('epochs')?.value||100),imgsz:+(document.getElementById('imgsz')?.value||640),batch:+(document.getElementById('batch')?.value||8),device:document.getElementById('device')?.value||'cpu',dataset_id:document.getElementById('trainDataset')?.value||'default',include_empty:false,train_ratio:.8,patience:+(document.getElementById('patience')?.value||100),workers:+(document.getElementById('workers')?.value||0),optimizer:document.getElementById('optimizer')?.value||'auto',lr0:+(document.getElementById('lr0')?.value||0.01),lrf:+(document.getElementById('lrf')?.value||0.01),weight_decay:+(document.getElementById('weight_decay')?.value||0.0005),close_mosaic:+(document.getElementById('close_mosaic')?.value||10),mosaic:+(document.getElementById('mosaic')?.value||1),cache:document.getElementById('cache')?.value||'False',single_cls:!!document.getElementById('single_cls')?.checked,pretrained:document.getElementById('pretrained')?.checked!==false,rect:!!document.getElementById('rect')?.checked,amp:document.getElementById('amp')?.checked!==false,cos_lr:!!document.getElementById('cos_lr')?.checked,freeze:+(document.getElementById('freeze')?.value||0),paddle_eval:!!document.getElementById('paddle_eval')?.checked};
    try{const r=await api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal();await loadRelated();renderTraining423();toast(`训练任务已创建 · ${r.job?.id||''}`)}catch(e){toast(e.message||e)}
  };
...
(function(){
  ...
  // ---------- video frame tasks ----------
  async function loadVideo424(){state.video424=(await safe(api(`/api/v33/projects/${pid()}/video-tasks`)))?.items||[]}
  function taskStatus424(t){const c=t.status==='done'?'ok':t.status==='failed'?'err':'warn';return`<span class="pill ${c}">${esc(t.status_text||t.status)}</span>`}
  window.renderVideo424=async function(){await loadVideo424();document.getElementById('view').innerHTML=`<section class="taskpage424"><div class="taskpage424-head"><div></div><button class="btn primary" onclick="createVideoTask424()">＋ 创建切帧任务</button></div><section class="panel"><div class="table-wrap"><table class="table task424-table"><thead><tr><th>视频</th><th>切帧方式</th><th>素材归属</th><th>状态</th><th>进度</th><th>预计剩余</th><th>创建/完成</th><th>操作</th></tr></thead><tbody>${state.video424.map(t=>`<tr><td><b>${esc(t.video_name||'-')}</b><div class="muted-line">${esc(t.id)}</div></td><td>${t.extract_fps>0?`每秒 ${t.extract_fps} 帧`:`每 ${t.interval_seconds||1} 秒 1 帧`}<div class="muted-line">最多 ${t.max_frames||'不限'} 张</div></td><td>${splitName424(t.split||'unassigned')}</td><td>${taskStatus424(t)}</td><td><div class="progress424"><i style="width:${t.progress||0}%"></i></div><span>${t.progress||0}% · ${t.extracted_frames||0}张</span></td><td>${fmtTime424(t.eta_seconds)}</td><td>${esc(String(t.created_at||'').slice(0,19))}<div class="muted-line">${esc(String(t.finished_at||'').slice(0,19))}</div></td><td><div class="row"><button class="btn mini" onclick="videoDetail424('${t.id}')">详情</button>${['queued','running'].includes(t.status)?`<button class="btn mini danger" onclick="stopVideo424('${t.id}')">停止</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="8">暂无切帧任务</td></tr>'}</tbody></table></div></section></section>`;if(state.video424.some(t=>['queued','running'].includes(t.status)))setTimeout(()=>{if(state.page==='视频切帧')renderVideo424()},1600)};
  window.createVideoTask424=function(){modal('创建切帧任务',`<div class="form two"><div class="field full"><label>上传视频文件</label><input id="vf424File" type="file" class="file" accept="video/*" onchange="videoEstimate424()"></div><div class="field"><label>切帧方式</label><select id="vf424Mode" class="select" onchange="videoMode424();videoEstimate424()"><option value="interval">按时间间隔</option><option value="fps">按每秒帧数</option></select></div><div class="field"><label>素材归属</label><select id="vf424Split" class="select"><option value="unassigned">未处理</option><option value="train">训练集</option><option value="val">试验集</option><option value="test">评测集</option></select></div><div class="field" id="vf424IntervalBox"><label>切帧间隔（秒）</label><input id="vf424Interval" class="input" type="number" min="0.05" step="0.05" value="1" oninput="videoEstimate424()"></div><div class="field hidden" id="vf424FpsBox"><label>每秒抽取帧数</label><input id="vf424Fps" class="input" type="number" min="0.1" step="0.1" value="1" oninput="videoEstimate424()"></div><div class="field"><label>最大切帧数量</label><input id="vf424Max" class="input" type="number" min="0" value="500" oninput="videoEstimate424()"></div><div class="field full"><div id="vf424Estimate" class="estimate424">选择视频后计算预计帧数</div></div></div><div class="row end"><button class="btn" onclick="closeModal()">取消</button><button class="btn primary" onclick="submitVideo424()">创建任务</button></div>`,true)};
  window.videoMode424=function(){const f=document.getElementById('vf424Mode')?.value==='fps';document.getElementById('vf424IntervalBox')?.classList.toggle('hidden',f);document.getElementById('vf424FpsBox')?.classList.toggle('hidden',!f)};
  window.videoEstimate424=function(){const f=document.getElementById('vf424File')?.files?.[0],out=document.getElementById('vf424Estimate');if(!f||!out)return;const v=document.createElement('video');v.preload='metadata';v.onloadedmetadata=()=>{const dur=v.duration||0,mode=document.getElementById('vf424Mode')?.value,max=+(document.getElementById('vf424Max')?.value||0);let n=mode==='fps'?dur*(+(document.getElementById('vf424Fps')?.value||1)):dur/Math.max(.05,+(document.getElementById('vf424Interval')?.value||1));if(max>0)n=Math.min(n,max);out.textContent=`视频约 ${dur.toFixed(1)} 秒 · 预计生成 ${Math.ceil(n)} 张 · 实际耗时将在任务运行后动态估算`;URL.revokeObjectURL(v.src)};v.src=URL.createObjectURL(f)};
  window.submitVideo424=async function(){const f=document.getElementById('vf424File')?.files?.[0];if(!f)return toast('请选择视频');const mode=document.getElementById('vf424Mode').value,fd=new FormData();fd.append('video',f);fd.append('dataset_id','default');fd.append('split',document.getElementById('vf424Split').value);fd.append('interval_seconds',mode==='interval'?document.getElementById('vf424Interval').value:'0');fd.append('extract_fps',mode==='fps'?document.getElementById('vf424Fps').value:'0');fd.append('max_frames',document.getElementById('vf424Max').value||'0');try{await api(`/api/v33/projects/${pid()}/video-tasks`,{method:'POST',body:fd});closeModal();renderVideo424();toast('切帧任务已创建')}catch(e){toast(e.message||e)}};
  window.stopVideo424=async id=>{await safe(api(`/api/v33/projects/${pid()}/video-tasks/${id}/stop`,{method:'POST'}));renderVideo424()};
  window.videoDetail424=id=>{const t=state.video424.find(x=>x.id===id);if(t)modal('切帧任务详情',`<div class="detail422-grid"><div><span>视频</span><b>${esc(t.video_name)}</b></div><div><span>状态</span><b>${esc(t.status_text||t.status)}</b></div><div><span>素材归属</span><b>${splitName424(t.split||'unassigned')}</b></div><div><span>已生成</span><b>${t.extracted_frames||0} 张</b></div><div><span>进度</span><b>${t.progress||0}%</b></div><div><span>预计剩余</span><b>${fmtTime424(t.eta_seconds)}</b></div></div>`,true)};
  ...
  window.submitTrain424=async function(){const aid=document.getElementById('tr424Alg')?.value,t=(state.targets||[]).find(x=>x.id===document.getElementById('tr424Target')?.value),c=state.train424Config||baseTrainConfig424(),trainIds=selectedTrainIds424('train'),valIds=selectedTrainIds424('val');if(!aid||!t)return toast('请选择算法和可用训练资源');if(!trainIds.length||!valIds.length)return toast('训练集和试验集都必须选择已标注数据');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:(t.algorithms||[])[0]?.key||'',algorithm_asset_id:aid,model:c.model||'',epochs:c.epochs,imgsz:c.imgsz,batch:c.batch,device:c.device||'cpu',dataset_id:'__all__',include_empty:false,patience:c.patience,workers:c.workers||0,optimizer:c.optimizer||'auto',lr0:c.lr0||.01,lrf:c.lrf||.01,weight_decay:c.weight_decay||.0005,close_mosaic:c.close_mosaic??10,mosaic:c.mosaic??1,cache:c.cache||'False',pretrained:c.pretrained!==false,amp:c.amp!==false,rect:!!c.rect,cos_lr:!!c.cos_lr,freeze:c.freeze||0,train_image_ids:trainIds,val_image_ids:valIds,val_max_samples:c.val_max_samples||0,eval_interval:c.eval_interval||0,eval_metric:c.eval_metric||'map50',continue_threshold:c.continue_threshold||0,stop_threshold:c.stop_threshold||0,auto_supplement:!!c.auto_supplement,supplement_count:c.supplement_count||0};try{const r=await api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal();await loadRelated();renderTraining424();toast(`训练任务已创建 · ${r.job?.id||''}`)}catch(e){toast(e.message||e)}};
...
(()=>{
  ...
  window.submitTrain425=async function(){const t=target425(),alg=trainAlgCfg425(),aid=document.getElementById('tr425AssetAlg')?.value,c=state.train425Config||cfg425(),trainIds=selected425('train'),valIds=selected425('val');if(!t||!alg)return toast('请选择可用训练资源和训练算法');if(!aid)return toast('请选择关联算法');if(!trainIds.length||!valIds.length)return toast('训练集和试验集都必须选择已标注数据');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:alg.key||'',algorithm_asset_id:aid,model:c.model||alg.base_model||'',epochs:c.epochs,imgsz:c.imgsz,batch:c.batch,device:c.device||'cpu',dataset_id:'__all__',include_empty:false,patience:c.patience,workers:c.workers,optimizer:c.optimizer,lr0:c.lr0,lrf:c.lrf,momentum:c.momentum,weight_decay:c.weight_decay,warmup_epochs:c.warmup_epochs,close_mosaic:c.close_mosaic,mosaic:c.mosaic,mixup:c.mixup,hsv_h:c.hsv_h,hsv_s:c.hsv_s,hsv_v:c.hsv_v,degrees:c.degrees,translate:c.translate,scale:c.scale,shear:c.shear,perspective:c.perspective,flipud:c.flipud,fliplr:c.fliplr,cache:c.cache,pretrained:c.pretrained,amp:c.amp,rect:c.rect,cos_lr:c.cos_lr,freeze:c.freeze,multi_scale:c.multi_scale,save_period:c.save_period,seed:c.seed,deterministic:c.deterministic,train_image_ids:trainIds,val_image_ids:valIds,val_max_samples:c.val_max_samples,eval_interval:c.eval_interval,eval_metric:c.eval_metric,continue_threshold:c.continue_threshold,stop_threshold:c.stop_threshold,auto_supplement:c.auto_supplement,supplement_count:c.supplement_count};try{const r=await api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal();await loadRelated();renderTraining425();toast(`训练任务已创建 · ${r.job?.id||''}`)}catch(e){toast(e.message||e)}};
...
(()=>{
  ...
  window.previewStep426=function(d){const list=state.data426PreviewList||[];if(!list.length)return;state.data426PreviewIndex=Math.max(0,Math.min(list.length-1,(state.data426PreviewIndex||0)+d));const layers=[...document.querySelectorAll('.v424-modal-layer')],top=layers[layers.length-1],body=top?.querySelector('.modal-body')||document.getElementById('modalBody');if(body)body.innerHTML=previewBody426()};
...
(()=>{
  ...
  window.submitTrain425=async function(){const t=(state.targets||[]).find(x=>x.id===document.getElementById('tr425Target')?.value),alg=(t?.algorithms||[]).find(x=>x.key===document.getElementById('tr425TrainAlg')?.value),aid=document.getElementById('tr425AssetAlg')?.value,c=state.train425Config||window.cfg425(),trainIds=[...(state.train425Selected?.train||new Set())],valIds=[...(state.train425Selected?.val||new Set())];if(!t||!alg)return toast('请选择可用训练资源和训练算法');if(!aid)return toast('请选择关联算法');if(!trainIds.length||!valIds.length)return toast('训练集和试验集都必须选择已标注数据');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:alg.key||'',algorithm_asset_id:aid,model:c.model||alg.base_model||'',epochs:c.epochs,imgsz:c.imgsz,batch:c.batch,device:c.device||'cpu',dataset_id:'__all__',include_empty:false,patience:c.patience,workers:c.workers,optimizer:c.optimizer,lr0:c.lr0,lrf:c.lrf,momentum:c.momentum,weight_decay:c.weight_decay,warmup_epochs:c.warmup_epochs,close_mosaic:c.close_mosaic,mosaic:c.mosaic,mixup:c.mixup,hsv_h:c.hsv_h,hsv_s:c.hsv_s,hsv_v:c.hsv_v,degrees:c.degrees,translate:c.translate,scale:c.scale,shear:c.shear,perspective:c.perspective,flipud:c.flipud,fliplr:c.fliplr,cache:c.cache,pretrained:c.pretrained,amp:c.amp,rect:c.rect,cos_lr:c.cos_lr,freeze:c.freeze,multi_scale:c.multi_scale,save_period:c.save_period,seed:c.seed,deterministic:c.deterministic,train_image_ids:trainIds,val_image_ids:valIds,val_max_samples:c.val_max_samples,eval_interval:c.eval_interval,eval_metric:c.eval_metric,continue_threshold:c.continue_threshold,stop_threshold:c.stop_threshold,auto_supplement:c.auto_supplement,supplement_count:c.supplement_count,ai_intervention_enabled:!!c.ai_intervention_enabled,ai_intervention_epochs:c.ai_intervention_epochs||[],ai_model_config_id:c.ai_model_config_id||'',ai_eval_samples:c.ai_eval_samples||20,ai_action_mode:c.ai_action_mode||'auto',ai_extra_epochs:c.ai_extra_epochs||20,ai_max_rounds:c.ai_max_rounds||1};try{const r=await api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal();await loadRelated();renderTraining425();toast(`训练任务已创建 · ${r.job?.id||''}`)}catch(e){toast(e.message||e)}};
...
(()=>{
  ...
  state.train428Config=state.train428Config||null;
  ...
  function usableTargets428(a){const paddle=algoMeta428(a).type==='paddle_detection';return(state.targets||[]).filter(t=>t.status==='ready'&&(paddle?t.framework==='paddle':t.framework!=='paddle'))}
  function selectedTarget428(){return(state.targets||[]).find(x=>x.id===document.getElementById('tr428Target')?.value)}
  function targetAlgorithm428(){const t=selectedTarget428(),key=document.getElementById('tr428TrainAlg')?.value;return(t?.algorithms||[]).find(x=>x.key===key)}
  function initTrainingSelection428(){state.train425Selected={train:new Set((state.images||[]).filter(x=>(x.split||'unassigned')==='train'&&x.annotated).map(x=>x.id)),val:new Set((state.images||[]).filter(x=>(x.split||'unassigned')==='val'&&x.annotated).map(x=>x.id))}}
  function refreshTrain428(){const c=state.train428Config||baseCfg428(),t=selectedTarget428(),tr=selectedIds428('train').length,va=selectedIds428('val').length;const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('tr428Model',String(c.model||'-').split(/[\\/]/).pop());set('tr428Epoch',c.epochs);set('tr428Size',c.imgsz);set('tr428Batch',c.batch);set('tr428TrainCount',`${tr} 张`);set('tr428ValCount',`${va} 张`);set('tr428TrainLabels',selectedLabels428('train').join('、')||'-');set('tr428ValLabels',selectedLabels428('val').join('、')||'-');set('tr428Gate',`每 ${c.eval_interval||10} 轮 · ${c.val_max_samples>0?c.val_max_samples+'张':'全部'} · ${gateMetricName428(c.eval_metric)}`);set('tr428ResourceName',t?.name||'-');const auto=(c.auto_convert_targets||[]).map(x=>TARGET_NAMES428[x]).join('、');set('tr428Convert',auto||'不自动转换');
    const est=document.getElementById('tr428Estimate'),hist=(state.jobs||[]).filter(j=>DONE428.has(j.status)&&j.elapsed_seconds&&j.epochs);if(est){if(hist.length&&tr){const per=hist.map(j=>j.elapsed_seconds/Math.max(1,j.epochs)/Math.max(1,j.dataset_counts?.train||tr));const sec=per.reduce((s,x)=>s+x,0)/per.length*tr*(c.epochs||100);est.innerHTML=`预计训练时间 <b>${fmtTime424(sec)}</b> · 运行后会按实际速度持续修正`;}else est.textContent='任务启动后将根据实际 Epoch 速度计算预计完成时间'}}
  window.startAlgorithmTraining423=function(aid){openTrain428(aid)};
  window.openTrain428=function(aid){const a=(state.algorithms||[]).find(x=>x.id===aid);if(!a)return toast('算法不存在');if(!algTrainable428(a))return toast('当前算法类型尚未接入统一训练执行器');const targets=usableTargets428(a);if(!targets.length)return toast('当前没有可用训练资源，请先配置并检测训练资源');state.train428AlgorithmId=aid;state.train428Config=baseCfg428();initTrainingSelection428();modal(`训练 · ${a.name}`,`<div class="train428-create">
    <section class="train428-identity"><div><span>本次迭代算法</span><h2>${esc(a.name)}</h2><p>${esc(algTypeName428(a.algorithm_type))}${a.industry?` · ${esc(a.industry)}`:''}</p></div><div class="train428-version-rule"><span>版本规则</span><b>训练完成时间自动生成</b><em>YYYYMMDDHHMMSS</em></div></section>
    <div class="train428-grid"><section class="train428-panel"><header><b>1. 训练资源与算法</b></header><div class="form two"><div class="field"><label>训练资源</label><select id="tr428Target" class="select" onchange="trainTargetChanged428()">${targets.map(t=>`<option value="${t.id}">${esc(t.name)} · ${t.type==='server'?'训练服务器':'本机'}</option>`).join('')}</select></div><div class="field"><label>训练算法</label><select id="tr428TrainAlg" class="select" onchange="trainAlgChanged428()"></select></div><div class="field"><label>关联算法</label><input class="input" value="${esc(a.name)}" readonly></div><div class="field"><label>任务优先级</label><select id="tr428Priority" class="select"><option value="50">普通</option><option value="80">优先</option><option value="100">最高</option></select></div></div><div class="train428-resource-line"><span>执行机器</span><b id="tr428ResourceName">-</b></div></section>
    <section class="train428-panel"><header><b>2. 训练数据</b></header><div class="train428-data"><article><div><b>训练集</b><strong id="tr428TrainCount">0 张</strong></div><p>标签：<span id="tr428TrainLabels">-</span></p><div class="row"><button class="btn mini" onclick="openTrainDataPicker425('train')">选择数据</button><button class="btn mini" onclick="trainDataQuality425('train')">查看质量</button></div></article><article><div><b>试验集</b><strong id="tr428ValCount">0 张</strong></div><p>标签：<span id="tr428ValLabels">-</span></p><div class="row"><button class="btn mini" onclick="openTrainDataPicker425('val')">选择数据</button><button class="btn mini" onclick="trainDataQuality425('val')">查看质量</button></div></article></div></section>
    <section class="train428-panel train428-wide"><header class="row between"><b>3. 训练配置</b><button class="btn" onclick="openTrainSettings428()">配置设置</button></header><div class="train428-config-summary"><div><span>基础模型</span><b id="tr428Model">-</b></div><div><span>总轮数</span><b id="tr428Epoch">100</b></div><div><span>图片尺寸</span><b id="tr428Size">640</b></div><div><span>Batch</span><b id="tr428Batch">8</b></div><div><span>阶段检查</span><b id="tr428Gate">-</b></div><div><span>达标后自动转换</span><b id="tr428Convert">不自动转换</b></div></div></section></div>
    <div id="tr428Estimate" class="estimate424"></div><div class="row end train428-footer"><button class="btn" onclick="closeModal()">取消</button><button class="btn primary" onclick="submitTrain428()">开始训练</button></div>
  </div>`,true);setTimeout(trainTargetChanged428,30)};
  window.trainCounts425=function(){refreshTrain428()};
  window.trainTargetChanged428=function(){const t=selectedTarget428(),sel=document.getElementById('tr428TrainAlg');if(!sel)return;sel.innerHTML=(t?.algorithms||[]).map(x=>`<option value="${esc(x.key)}">${esc(x.name||x.short_name||x.key)}</option>`).join('')||'<option value="">当前资源没有可训练算法</option>';const c=state.train428Config||baseCfg428();c.device=t?.type==='server'?'0':(t?.recommendation?.device||'cpu');state.train428Config=c;trainAlgChanged428()};
  window.trainAlgChanged428=function(){const x=targetAlgorithm428(),t=selectedTarget428(),c=state.train428Config||baseCfg428();if(x){c.model=x.base_model||c.model||'';c.epochs=x.default_epochs||c.epochs;c.imgsz=x.default_imgsz||c.imgsz;c.batch=x.default_batch||c.batch}if(!c.model){const m=(t?.base_models||[])[0];c.model=m?.value||m?.label||''}state.train428Config=c;refreshTrain428()};

  window.openTrainSettings428=function(){const c=state.train428Config||baseCfg428(),t=selectedTarget428(),models=t?.base_models||[];modal('训练配置设置',`<div class="train428-settings">
  ...
  window.saveTrainSettings428=function(){const c=state.train428Config||baseCfg428(),num=(id,d)=>{const e=document.getElementById(id);return e?Number(e.value||d):d},low=num('ts428Low',0)/100,goal=num('ts428Goal',90)/100;if(goal>0&&low>0&&low>=goal)return toast('“低于停止”必须小于“达到达标”的正确率');Object.assign(c,{model:document.getElementById('ts428Model')?.value||c.model,epochs:num('ts428Epoch',100),imgsz:num('ts428Size',640),batch:num('ts428Batch',8),eval_interval:Math.max(1,num('ts428EvalInt',10)),val_max_samples:Math.max(0,num('ts428ValN',0)),eval_metric:document.getElementById('ts428Metric')?.value||'map50',continue_threshold:low,stop_threshold:goal,optimizer:document.getElementById('ts428Opt')?.value||'auto',patience:num('ts428Patience',100),workers:num('ts428Workers',0),lr0:num('ts428Lr0',.01),momentum:num('ts428Momentum',.937),weight_decay:num('ts428WD',.0005),mosaic:num('ts428Mosaic',1),mixup:num('ts428Mixup',0),seed:num('ts428Seed',0),save_period:num('ts428Save',-1),freeze:num('ts428Freeze',0),cache:document.getElementById('ts428Cache')?.value||'False',pretrained:!!document.getElementById('ts428Pretrained')?.checked,amp:!!document.getElementById('ts428Amp')?.checked,deterministic:!!document.getElementById('ts428Det')?.checked,cos_lr:!!document.getElementById('ts428Cos')?.checked,auto_convert_targets:[...document.querySelectorAll('.ts428AutoConvert:checked')].map(x=>x.value)});state.train428Config=c;closeModal();refreshTrain428();toast('训练配置已应用')};
  window.submitTrain428=async function(){const a=(state.algorithms||[]).find(x=>x.id===state.train428AlgorithmId),t=selectedTarget428(),ta=targetAlgorithm428(),c=state.train428Config||baseCfg428(),trainIds=selectedIds428('train'),valIds=selectedIds428('val');if(!a||!t||!ta)return toast('训练资源或训练算法不可用');if(!trainIds.length)return toast('请至少选择一张已标注训练集图片');if(!valIds.length)return toast('请至少选择一张已标注试验集图片');const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:ta.key||'',algorithm_asset_id:a.id,model:c.model||ta.base_model||'',epochs:c.epochs,imgsz:c.imgsz,batch:c.batch,device:c.device||'cpu',dataset_id:'__all__',include_empty:false,patience:c.patience,workers:c.workers,optimizer:c.optimizer,lr0:c.lr0,lrf:c.lrf,momentum:c.momentum,weight_decay:c.weight_decay,warmup_epochs:c.warmup_epochs,close_mosaic:c.close_mosaic,mosaic:c.mosaic,mixup:c.mixup,hsv_h:c.hsv_h,hsv_s:c.hsv_s,hsv_v:c.hsv_v,degrees:c.degrees,translate:c.translate,scale:c.scale,shear:c.shear,perspective:c.perspective,flipud:c.flipud,fliplr:c.fliplr,cache:c.cache,pretrained:c.pretrained,amp:c.amp,rect:c.rect,cos_lr:c.cos_lr,freeze:c.freeze,multi_scale:c.multi_scale,save_period:c.save_period,seed:c.seed,deterministic:c.deterministic,train_image_ids:trainIds,val_image_ids:valIds,val_max_samples:c.val_max_samples,eval_interval:c.eval_interval,eval_metric:c.eval_metric,continue_threshold:c.continue_threshold,stop_threshold:c.stop_threshold,auto_supplement:false,supplement_count:0,queue_priority:Number(document.getElementById('tr428Priority')?.value||50),auto_convert_targets:c.auto_convert_targets||[],ai_intervention_enabled:false,ai_intervention_epochs:[],ai_model_config_id:''};try{const r=await api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal();await loadRelated();state.alg428Expanded[a.id]=true;renderAlgorithms423();toast(`${r.job?.status==='running'?'训练已开始':'训练已进入队列'} · ${r.job?.id||''}`)}catch(e){toast(e.message||e)}};

  // ---------------------- training task center: active/history only ----------------------
  function queuePosition428(j){if(j.status!=='queued')return'';const same=(state.jobs||[]).filter(x=>x.status==='queued'&&(x.resource_key||'')===(j.resource_key||'')).sort((a,b)=>(Number(b.queue_priority||0)-Number(a.queue_priority||0))||String(a.queued_at||a.created_at||'').localeCompare(String(b.queued_at||b.created_at||'')));const i=same.findIndex(x=>x.id===j.id);return i>=0?`队列第 ${i+1} 位`:''}
  ...
  function trainRows428(rows){return rows.map(j=>`<tr><td><div class="train428-taskname"><b>${esc(j.asset_algorithm_name||j.algorithm_name||j.id)}</b><span>${esc(j.id)}</span>${j.auto_version_name?`<em>版本 ${esc(j.auto_version_name)}</em>`:''}</div></td><td>${statusPill428(j.status)}${queuePosition428(j)?`<small class="queuepos428">${queuePosition428(j)}</small>`:''}</td><td><div class="train428-resource"><b>${esc(resourceName428(j))}</b><span>${esc(j.framework==='paddle'?'PaddleDetection':'Ultralytics / YOLO')}</span></div></td><td><div class="progress424"><i style="width:${Math.max(0,Math.min(100,Number(j.progress_percent||0)))}%"></i></div><span class="train428-progress-txt">${j.current_epoch||0}/${j.total_epochs||j.epochs||'-'} · ${Number(j.progress_percent||0).toFixed(0)}%</span></td><td>${fmtTime424(j.elapsed_seconds)}</td><td>${fmtTime424(j.eta_seconds)}</td><td>${dt428(j.started_at||j.created_at)}</td><td><div class="row wrap">${trainActions428(j)}</div></td></tr>`).join('')||'<tr><td colspan="8" class="empty-row">暂无记录</td></tr>'}
...
(()=>{
  ...
  state.train429Selected=state.train429Selected||new Set();
  ...
  window.renderAiRefs429=function(){const g=document.getElementById('ai429RefGrid');if(!g)return;g.innerHTML=refs429().slice(0,120).map(x=>`<button class="${state.ai429RefSelected.has(x.id)?'on':''}" onclick="toggleRef429('${x.id}')"><img src="${x.url}" loading="lazy"><b>${esc(x.filename)}</b><span>${esc((x.labels||[]).join('、'))}</span></button>`).join('')||'<div class="empty">没有符合筛选条件的已标注图片</div>'};
  ...
  window.renderTrainPicker429=function(){const q=(document.getElementById('tr429Q')?.value||'').toLowerCase(),labs=[...state.train429PickerLabels],rows=pool429().filter(x=>(!q||String(x.filename).toLowerCase().includes(q))&&(!labs.length||labs.some(l=>(x.labels||[]).includes(l))));const g=document.getElementById('tr429Grid');if(g)g.innerHTML=rows.slice(0,300).map(x=>`<button class="${state.train429Selected.has(x.id)?'on':''}" onclick="toggleTrainImage429('${x.id}')"><img src="${x.url}" loading="lazy"><b>${esc(x.filename)}</b><span>${esc((x.labels||[]).join('、'))}</span></button>`).join('')||'<div class="empty">没有符合条件的已处理标注图片</div>'};
  window.toggleTrainImage429=function(id){state.train429Selected.has(id)?state.train429Selected.delete(id):state.train429Selected.add(id);renderTrainPicker429()};
...
(()=>{
  ...
  window.previewStep411=function(d){const list=state.data426PreviewList||[];let i=Math.max(0,Math.min(list.length-1,(state.data426PreviewIndex||0)+d));state.data426PreviewIndex=i;const layers=[...document.querySelectorAll('.v424-modal-layer')],top=layers[layers.length-1],body=top?.querySelector('.modal-body')||document.getElementById('modalBody');if(body&&list[i])body.innerHTML=previewHtml411(list[i],list,i)};
  ...
  window.doUploadImages426=function(inp){
    const fs=[...(inp.files||[])];if(!fs.length)return;const total=fs.reduce((a,f)=>a+f.size,0),started=performance.now();closeModal();setTimeout(()=>modal('图片上传',uploadModal411('正在上传图片',fs.length,total),true),20);const fd=new FormData();fs.forEach(f=>fd.append('files',f));fd.append('dataset_id','default');
...
(()=>{
  ...
  window.loadCore412=async function(){
    ...
    let ds=await safe(api(`/api/projects/${id}/datasets`)); state.datasets=ds?.items||[];
    if(!state.datasets.length){await safe(api(`/api/projects/${id}/datasets`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'默认数据集',description:'',kind:'mixed'})}));ds=await safe(api(`/api/projects/${id}/datasets`));state.datasets=ds?.items||[]}
    if(!state.datasets.find(d=>d.id===state.datasetId))state.datasetId=state.datasets[0]?.id||'default';
    const [images,labels,algos]=await Promise.all([
      safe(api(`/api/projects/${id}/images`)),
      safe(api(`/api/v12/projects/${id}/labels`)),
      safe(api(`/api/v12/projects/${id}/algorithms`))
    ]);
    state.images=images||[]; state.labels=labels?.items||[]; state.algorithms=algos?.items||[];
  };
  ...
  async function extras412(){if(!state.project)return;const id=state.project.id;const q=p=>p.catch(()=>null);const [info,opts,pending,testModels,infer,rec,local]=await Promise.all([
    q(api(`/api/projects/${id}`)),q(api(`/api/training_options?project_id=${id}`)),q(api(`/api/v12/projects/${id}/publish/pending`)),q(api(`/api/v12/projects/${id}/test_models`)),q(api('/api/v16/inference_envs')),q(api('/api/system/recommendation')),q(api('/api/local_models'))
  ]);if(info){state.project=info.project||state.project;state.jobs=info.jobs||[];state.models=info.models||[]}state.targets=opts?.targets||[];state.pending=pending?.items||[];state.testModels=testModels?.items||[];state.inferenceEnvs=infer?.items||[];state.rec=rec;state.localModels=local?.items||[];try{renderSummary()}catch(e){};pollAnnotationIndex412();
  }
  loadAll=async function(){await window.loadCore412();if(!state.__extras412)state.__extras412=extras412().finally(()=>state.__extras412=null)};
  ...
  window.batchDelete412=async()=>{const ids=[...state.data412Selected];if(!ids.length)return toast('请选择要删除的素材');if(!confirm(`确认删除 ${ids.length} 张素材？对应图片和标注会一起删除。`))return;const r=await api(`/api/v46/projects/${pid()}/images/batch-delete`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image_ids:ids})});const s=new Set(ids);state.images=(state.images||[]).filter(x=>!s.has(x.id));state.data412Selected.clear();state.data412DeleteMode=false;renderDatasets424();toast(`已删除 ${r.deleted||ids.length} 张`)};
  ...
  window.selectImportAll412=()=>{(state.import412?.image_ids||[]).forEach(id=>state.import412Selected.add(String(id)));const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1)||document.getElementById('modalBody');if(body)body.innerHTML=reviewHtml412()};
  window.invertImport412=()=>{(state.import412?.image_ids||[]).forEach(id=>state.import412Selected.has(String(id))?state.import412Selected.delete(String(id)):state.import412Selected.add(String(id)));const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1)||document.getElementById('modalBody');if(body)body.innerHTML=reviewHtml412()};
  window.remapImport412=async(source,inputId)=>{const target=document.getElementById(inputId)?.value.trim();if(!target)return toast('请输入新标签名');const ids=[...state.import412Selected];if(!ids.length)return toast('请选择本次导入素材');const r=await api(`/api/v52/projects/${pid()}/labels/remap`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image_ids:ids,source_label:source,target_label:target})});await window.loadCore412();const rr=await api(`/api/v52/projects/${pid()}/import/jobs/${state.import412.job_id}/review`);state.import412.label_box_counts=rr.label_box_counts||{};const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1)||document.getElementById('modalBody');if(body)body.innerHTML=reviewHtml412();toast(`已将 ${r.changed_boxes||0} 个 ${source} 标注改为 ${target}`)};
  ...
  // Patch ZIP completion: preserve v42.11 progress, but review batch after completion and use fast reload.
  const oldZip412=window.doUploadZip426;
  if(typeof oldZip412==='function'){
    // do not replace transport; observe completed import job via the existing state and surface review button from dock/modal.
    const obs=new MutationObserver(()=>{const t=state.import411;if(!t?.jobId||t.__reviewBound||t.stage!=='导入完成')return;t.__reviewBound=true;const extra=`<div class="row end"><button class="btn primary review412-btn" onclick="showImportReview412('${t.jobId}')">整理本次导入素材</button></div>`;if(!String(t.resultHtml||'').includes('review412-btn'))t.resultHtml=String(t.resultHtml||'')+extra;setTimeout(()=>{const r=document.getElementById('zip411Result');if(r&&!r.querySelector('.review412-btn'))r.insertAdjacentHTML('beforeend',extra);if(r&&!t.__autoReviewOpened){t.__autoReviewOpened=true;setTimeout(()=>showImportReview412(t.jobId),280)}},30)});obs.observe(document.body,{childList:true,subtree:true});
  }
...
(()=>{
 const V413='42.14.0', sleep=ms=>new Promise(r=>setTimeout(r,ms));
 ...
 function apply(s){state.projects=s.projects||[];state.project=s.project||null;state.datasets=s.datasets||[];state.datasetId=state.datasets.find(d=>d.id===state.datasetId)?.id||state.datasets[0]?.id||'default';state.images=s.images||[];state.labels=s.labels||[];state.algorithms=s.algorithms||[];state.jobs=s.jobs||[];state.models=s.models||[];state.targets=s.targets||[];state.inferenceEnvs=s.inference_envs||[];state.rec=s.recommendation||null;state.localModels=s.local_models||[];state.pending=s.pending||[];state.testModels=s.test_models||[];state.versionInfo={version:V413,name:'畅联云算法训练'};try{const x=JSON.parse(localStorage.getItem('mc_train_ui_state_v34')||'{}');x.projectId=state.project?.id||'';x.ts=Date.now();localStorage.setItem('mc_train_ui_state_v34',JSON.stringify(x))}catch(e){}}
 window.loadStartupSnapshot413=async function(force=false){if(force)await api('/api/v53/bootstrap/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({preferred_project_id:savedProject(),force:true})});await waitReady();const s=await api(`/api/v53/bootstrap/snapshot?preferred_project_id=${encodeURIComponent(savedProject())}`);apply(s);return s};
 window.__clInit=async function(){const view=document.getElementById('view');try{await window.loadStartupSnapshot413(false);state.uiReady=true;render()}catch(e){if(view)view.innerHTML=`<div class="boot413"><div class="boot413-card error"><b>平台数据加载失败</b><p>${esc(e.message||e)}</p><div class="row"><button class="btn primary" onclick="window.__clInit()">重新加载</button><button class="btn" onclick="location.reload()">刷新页面</button></div></div></div>`}};
 const oldTop=renderTop;renderTop=function(){oldTop();const v=document.getElementById('versionBadge');if(v)v.textContent='v'+V413;const r=document.getElementById('refreshBtn');if(r)r.onclick=async()=>{r.disabled=true;const t=r.textContent;r.textContent='重新加载中';try{await window.loadStartupSnapshot413(true);render();toast('数据与环境已重新加载')}catch(e){toast(e.message||e)}finally{r.disabled=false;r.textContent=t||'刷新'}}};
})();
...
(()=>{
  ...
  window.saveNewAlgorithm414=async function(){const name=(document.getElementById('alg414Name')?.value||'').trim(),industry=(document.getElementById('alg414Industry')?.value||'').trim(),algorithm_type=document.getElementById('alg414Type')?.value||'yolo_ultralytics',remark=(document.getElementById('alg414Remark')?.value||'').trim(),btn=document.getElementById('alg414Save');if(!name)return toast('请输入算法名称');if(btn){btn.disabled=true;btn.textContent='创建中…'}try{const item=await api(`/api/v12/projects/${pid()}/algorithms`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,industry,algorithm_type,remark})});state.algorithms=[item,...(state.algorithms||[]).filter(x=>x.id!==item.id)];closeModal();renderAlgorithms423();toast('算法已创建')}catch(e){toast(`创建失败：${e.message||e}`)}finally{if(btn){btn.disabled=false;btn.textContent='创建算法'}}};
  ...
  window.saveEditAlgorithm414=async id=>{const payload={name:(document.getElementById('alg414EditName')?.value||'').trim(),industry:(document.getElementById('alg414EditIndustry')?.value||'').trim(),algorithm_type:document.getElementById('alg414EditType')?.value||'',remark:(document.getElementById('alg414EditRemark')?.value||'').trim()};if(!payload.name)return toast('请输入算法名称');try{const item=await api(`/api/v12/projects/${pid()}/algorithms/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const i=(state.algorithms||[]).findIndex(x=>String(x.id)===String(id));if(i>=0)state.algorithms[i]=item;closeModal();renderAlgorithms423();toast('算法已更新')}catch(e){toast(e.message||e)}};
  ...
  window.startAlgorithmTraining429=async function(aid){
    ...
    try{const r=await api(`/api/v54/projects/${pid()}/algorithms/${aid}/iteration-base?framework=ultralytics`);state.iteration414[aid]=r.base||null;setTimeout(()=>showIterationBase414(aid),20)}catch(e){state.iteration414[aid]={error:String(e.message||e)};setTimeout(()=>showIterationBase414(aid),20)}
  };
  window.showIterationBase414=function(aid){const layer=[...document.querySelectorAll('.v424-modal-layer')].at(-1),body=layer?.querySelector('.modal-body');if(!body)return;let box=body.querySelector('.iteration414');if(!box){box=document.createElement('div');box.className='iteration414';body.prepend(box)}const b=state.iteration414[aid];if(!b)box.innerHTML='<span>迭代起点</span><b>正在读取最新版本…</b>';else if(b.error)box.innerHTML=`<span>迭代起点</span><b>读取失败</b><em>${esc(b.error)}</em>`;else if(b.version_name)box.innerHTML=`<span>迭代起点</span><b>从最新可训练版本继续：${esc(b.version_name)}</b><em>${esc(b.model_name||'模型权重')}</em>`;else box.innerHTML='<span>迭代起点</span><b>首次训练：使用所选母模型</b><em>后续版本会自动以上一个可用版本继续训练</em>'};
...
(()=>{
  ...
  window.toggleImport414=(id,on)=>{on?state.import412Selected.add(String(id)):state.import412Selected.delete(String(id));const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1);if(body)body.innerHTML=importReview414Html()};
  window.selectImportAll414=()=>{(state.import412?.image_ids||[]).forEach(id=>state.import412Selected.add(String(id)));const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1);if(body)body.innerHTML=importReview414Html()};
  window.invertImport414=()=>{(state.import412?.image_ids||[]).forEach(id=>state.import412Selected.has(String(id))?state.import412Selected.delete(String(id)):state.import412Selected.add(String(id)));const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1);if(body)body.innerHTML=importReview414Html()};
  window.remapImport414=async function(source,selectId){const target=document.getElementById(selectId)?.value||'';if(!target)return toast('请选择标签库中的标准标签');const ids=[...state.import412Selected];if(!ids.length)return toast('请选择本次导入素材');try{const rr=await api(`/api/v52/projects/${pid()}/labels/remap`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image_ids:ids,source_label:source,target_label:target})});await Promise.all([window.loadCore412(),refreshLabels414(false)]);const review=await api(`/api/v52/projects/${pid()}/import/jobs/${state.import412.job_id}/review`);state.import412.label_box_counts=review.label_box_counts||{};const body=[...document.querySelectorAll('.v424-modal-layer .modal-body')].at(-1);if(body)body.innerHTML=importReview414Html();toast(`已同步 ${rr.changed_boxes||0} 个框：${source} → ${target}`)}catch(e){toast(e.message||e)}};
