...
(()=>{
  ...
  function renderSplit(){
    ...
    if(!root.querySelector('#trV3ResourceStrategy')){
      ...
      field.innerHTML='<header><span class="train-ui-card-icon">⚡</span><div><b>训练资源策略</b><small>推荐模式由调度器在真正分配到服务器和 GPU 后再计算资源</small></div></header><div class="form two"><label class="field"><span>资源控制</span><select id="trV3ResourceStrategy" class="select"><option value="auto">自动调度（推荐）</option><option value="manual">手动配置</option></select></label><label class="field"><span>自动档位</span><select id="trV3ResourceProfile" class="select"><option value="balanced">智能推荐（推荐）</option><option value="performance">性能优先</option><option value="stability">稳定优先</option></select></label></div><small>自动模式会按空闲显存、GPU繁忙度、CPU、内存和数据集大小自动确定 Batch / Workers / Cache；训练开始后只监控，不中途改变 Batch。</small>';
      panel.before(field);
