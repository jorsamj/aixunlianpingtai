...
(function(){
  ...
  window.openRknnHardwareVerify=function(id){const job=(state.deployJobs||[]).find(x=>String(x.id)===String(id));if(!job)return toast('未找到转换任务');const chip=String(job.params?.chip||'').toUpperCase();modal('RKNN 板端验证',`<div class="deploy-hw-verify"><div class="compact-note"><b>${esc(job.source_name||'RKNN模型')}</b> · ${esc(chip)}<br>上传一张真实测试图片，系统会把 .rknn 模型和图片送到匹配的瑞芯微 Agent，在板端 RKNNLite Runtime 完成一次真实推理。</div><div class="field"><label>测试图片</label><input id="rknnVerifyFile" class="input" type="file" accept="image/*"></div><div id="rknnVerifyState" class="alert soft">等待选择测试图片</div><div class="row end"><button class="btn" onclick="closeModal()">取消</button><button id="rknnVerifySubmit" class="btn primary" onclick="submitRknnHardwareVerify('${id}')">开始板端验证</button></div></div>`,true)};
...
(()=>{
  ...
  window.refreshConvertResource428=function(){
    ...
    if(sel){sel.innerHTML=ready.map(x=>`<option value="${esc(x.id)}">${esc(x.name||x.id)} · ${esc(x.mode==='agent'?'Agent':x.mode==='remote'?'远程':'本机')}</option>`).join('')||'<option value="">暂无已检测可用资源</option>';sel.onchange=()=>{const r=all.find(x=>x.id===sel.value);if(target==='rockchip'&&chip){chip.value=(r?.supported_chips||[])[0]||'rk3588'}else if(target==='ascend'&&chip){const socs=r?.detected_soc_versions||r?.remote_health?.soc_versions||[];chip.value=socs[0]||''}}}
    const first=ready[0];
    if(chip){if(target==='rockchip')chip.value=(first?.supported_chips||[])[0]||'rk3588';else if(target==='sophon')chip.value='bm1684x';else{const socs=first?.detected_soc_versions||first?.remote_health?.soc_versions||[];chip.value=socs[0]||''}}
