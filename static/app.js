...
window.openVersionRollback=(aid,targetVersionId)=>{const algorithm=(state.algorithms||[]).find(item=>String(item.id)===String(aid)),versions=algorithm?.versions||[],currentVersionId=String(algorithm?.current_version_id||''),current=versions.find(item=>String(item.id)===currentVersionId),target=versions.find(item=>String(item.id)===String(targetVersionId));if(!algorithm||!current||!target)return toast('版本信息已变化，请刷新算法列表后重试');if(currentVersionId===String(targetVersionId))return toast('该版本已经是当前版本');const external=String(algorithm.source_type||'').toUpperCase()==='EXTERNAL';modal('删除当前版本并回退',`<div class="version-rollback"><div class="version-rollback-algorithm"><span>算法名称</span><b>${esc(algorithm.name||algorithm.code||algorithm.id)}</b></div><div class="version-rollback-flow"><div><span>将删除</span><b>${esc(current.version_name||current.id)}</b></div><i>→</i><div><span>回到版本</span><b>${esc(target.version_name||target.id)}</b></div></div><div class="alert warn"><b>回退会删除当前版本</b><span>${external?'该算法来自新畅联，系统会先调用新畅联版本删除接口；远端删除失败时不会删除本地版本。':'删除成功后，目标历史版本自动成为当前版本。'}</span></div><div class="version-rollback-boundary">系统会先检查运行中的训练、转换和部署测试引用；存在活动引用时禁止删除。</div><div class="row end"><button class="btn soft" onclick="closeModal()">取消</button><button id="rollbackConfirmButton" class="btn danger" onclick="submitVersionRollback('${esc(aid)}','${esc(targetVersionId)}','${esc(currentVersionId)}')">确认删除并回退</button></div></div>`)};
window.submitVersionRollback=async(aid,targetVersionId,currentVersionId)=>{const button=document.getElementById('rollbackConfirmButton');if(button){button.disabled=true;button.textContent='正在删除并回退…'}const result=await safe(api(`/api/v12/projects/${pid()}/algorithms/${aid}/versions/${targetVersionId}/rollback`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({delete_current_version:true,expected_current_version_id:currentVersionId})}));if(!result){if(button){button.disabled=false;button.textContent='确认删除并回退'}return}closeModal();const runtime=window.AlgorithmListRuntime;if(!runtime?.refresh){toast('当前版本已删除，请刷新页面查看');return}await runtime.refresh({render:true});if(result.cleanup_status&&result.cleanup_status!=='cleanup_completed')toast('当前版本已删除并回退，部分物理产物清理失败，请查看操作记录');else toast('当前版本已删除，已回到目标版本')};

window.quickUltraDetect=()=>window.ResourceDiscoveryRuntime?.detectEnvironment({scope:'auto'})||toast('资源检测模块正在加载，请稍后重试');
window.detectUltraLegacy_1=()=>{const root=$('#uroot')?.value.trim()||'';return window.ResourceDiscoveryRuntime?.detectEnvironment(root?{scope:'fast',roots:[root]}:{scope:'auto'})||toast('资源检测模块正在加载，请稍后重试')};
...
async function openAnnotation(id){
  ...
  renderAnnotator();
} window.openAnnotationLegacy_1=openAnnotation;
...
window.assignVersionLegacy=name=>{if(!state.algorithms.length)return toast('请先在算法列表创建算法');modal('归属到算法',`<div class="form"><div class="field"><label>模型</label><input class="input" value="${esc(name)}" disabled></div><div class="field"><label>算法</label><select id="algoSel" class="select">${state.algorithms.map(a=>`<option value="${a.id}">${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>版本名称</label><input id="verName" class="input" placeholder="例如：V1.0"></div><div class="field"><label>备注</label><textarea id="verRemark"></textarea></div><button class="btn primary" onclick="saveAssign('${esc(name)}')">确认归属</button></div>`)};
window.saveAssignLegacy=async name=>{await safe(api(`/api/v12/projects/${pid()}/algorithms/${$('#algoSel').value}/versions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model_name:name,model_source:'project',version_name:$('#verName').value,remark:$('#verRemark').value})}));closeModal();await reload();toast('已归属到算法')};

async function init(){await loadAll();render()} window.__clInitLegacyBase=init;
...
setTimeout(async()=>{if(window.ZipImportRuntime)return;ensureImportDock();await loadImportJobs();if((state.importJobs||[]).some(j=>j.status==='running'))startImportPolling();},600);
...
(function(){
  window.renderDetectionResultLegacy1= function(r,title){
...
(()=>{
  ...
  window.renderStorageSources61=async function({force=false}={}){
    ...
    let shell=view.querySelector('.storage61-shell');
    if(!shell){
      ...
      shell=view.querySelector('.storage61-shell');
    }
    paintStorageRows61();
  ...
  window.openStorageRescan61=async function(sourceId){
    ...
    function syncFormat(){
      const selected=String(format?.value||'images'),yolo=selected==='yolo',detection=['yolo','coco','voc'].includes(selected);
    ...
    function renderDetectionConfirm(task){
      if(!['yolo','coco','voc'].includes(task.import_format)){annotationPolicy.hidden=true;qualityBox.innerHTML='';mappingBox.innerHTML='';return}
    ...
    async function loadPreflight(){
      try{
        ...
        preflight={agent_available:false,reason:error.message||String(error),eligible_nodes:[],agent_supported_formats:[],local_supported_formats:['images','yolo','coco','voc']};
    ...
    async function poll(generation){
      try{
        ...
        const annotationLine=['yolo','coco','voc'].includes(task.import_format)?Object.entries(annotationNames).map(([key,label])=>`${label} ${Number(task.annotation_counts?.[key]||0)}`).join(' · '):'';
    ...
    execution.onchange=()=>{
      const supported=execution.value==='agent'?(preflight?.agent_supported_formats||['images']):(preflight?.local_supported_formats||['images','yolo','coco','voc']);
      if(!supported.includes(format.value))format.value='images';
      syncFormat();
    };
    document.getElementById('sr61Start').onclick=async()=>{
      const button=document.getElementById('sr61Start'),mode=execution?.value==='agent'?'agent':'local',selectedFormat=['yolo','coco','voc'].includes(format?.value)?format.value:'images';
      if(mode==='agent'&&!preflight?.agent_available){target.textContent=preflight?.reason||'当前没有可用远程 Agent';return}
      const supported=mode==='agent'?(preflight?.agent_supported_formats||['images']):(preflight?.local_supported_formats||['images','yolo','coco','voc']);
      ...
      try{
        ...
        if(['yolo','coco','voc'].includes(selectedFormat))body.import_format=selectedFormat;
  ...
  function sourceFields61(type,source={}){
    ...
    if(type==='oss')return `<div class="field"><label>Endpoint *</label><input id="ss61Endpoint" class="input" value="${esc(c.endpoint||'')}" placeholder="oss-cn-hangzhou.aliyuncs.com"></div><div class="field"><label>Bucket *</label><input id="ss61Bucket" class="input" value="${esc(c.bucket||'')}"></div><div class="field full"><label>外网地址 *</label><input id="ss61PublicBaseUrl" class="input" value="${esc(c.public_base_url||'')}" placeholder="https://your-bucket.oss-cn-hangzhou.aliyuncs.com"><div class="subline">用于生成写给畅联云的长期 filePath；该地址应已包含 Bucket host，平台不会再次拼接 Bucket。</div></div><div class="field"><label>素材目录（可选）</label><input id="ss61Prefix" class="input" value="${esc(c.prefix||'')}" placeholder="changlian-ai/materials/"><div class="subline">只用于素材 Provider；算法产物使用独立 Artifact root_prefix。</div></div><div class="field"><label>AccessKey ID</label><input id="ss61Access" class="input" autocomplete="off"></div><div class="field"><label>AccessKey Secret</label><input id="ss61Secret" type="password" class="input" autocomplete="new-password">${secret}</div>`;
  ...
  window.installServerMaterialImport61=function(){
    ...
    window.syncStorageImportFormat61=function(){
      ...
      if(!format)return;
      const remote=mode==='storage_scan'||(mode==='server_zip'&&state.serverZipExecutionMode61==='agent'),auto=format.querySelector('option[value="auto"]'),coco=format.querySelector('option[value="coco"]'),voc=format.querySelector('option[value="voc"]');
    ...
    window.setServerZipExecution61=function(mode){
      const execution=mode==='agent'?'agent':'local';
      state.serverZipExecutionMode61=execution;
      const localWrap=document.getElementById('si61ZipLocalSourceWrap'),remoteWrap=document.getElementById('si61ZipRemoteSourceWrap'),help=document.getElementById('si61ZipExecutionHelp');
      if(localWrap)localWrap.hidden=execution==='agent';
      if(remoteWrap)remoteWrap.hidden=execution!=='agent';
      if(help)help.textContent=execution==='agent'?'中央端只负责把 ZIP 暂存到对象存储；远程 Agent 安全解包、检查标注并生成审查结果，确认后再建立正式素材索引。':'由中央 Storage Worker 在服务器本地解包并扫描，目标必须是本地存储源。';
    ...
    window.setStorageImportMode61=function(mode){
      ...
      if(mode==='server_zip')setServerZipExecution61(state.serverZipExecutionMode61||'local');
      else syncStorageImportFormat61();
    };
    ...
    window.openStorageImport61=async function(){
      ...
      state.serverZipExecutionMode61=localSources.length?'local':(objectSources.length?'agent':'local');
      abortPolling();
      modal('从存储导入素材',`<div id="si61ImportShell" class="storage61-import">
        ...
        <section data-import-panel="server_zip"><div class="form two"><div class="field"><label>执行位置</label><select id="si61ZipExecution" class="select" onchange="setServerZipExecution61(this.value)"><option value="local" ${state.serverZipExecutionMode61==='local'?'selected':''} ${localSources.length?'':'disabled'}>中央 Worker</option><option value="agent" ${state.serverZipExecutionMode61==='agent'?'selected':''} ${objectSources.length?'':'disabled'}>远程 Agent</option></select></div><div class="field" id="si61ZipLocalSourceWrap"><label>目标本地存储源</label><select id="si61ZipSource" class="select" ${localDisabled}>${localOptions||'<option>暂无已启用的本地存储</option>'}</select></div><div class="field" id="si61ZipRemoteSourceWrap" hidden><label>目标对象存储源</label><select id="si61ZipRemoteSource" class="select" ${objectDisabled}>${objectOptions||'<option>暂无已启用的 OSS / S3 / MinIO 存储</option>'}</select></div><div class="field"><label>服务器 ZIP（导入目录下的相对路径）</label><input id="si61ZipPath" class="input" placeholder="例如 fire.zip"></div><div class="field"><label>目标目录</label><input id="si61TargetPrefix" class="input" placeholder="例如 fire/2026"></div></div><div class="storage61-import-help"><span id="si61ZipExecutionHelp">目标目录已存在且非空时会拒绝导入，不会覆盖原文件。</span></div><button class="btn primary" onclick="startStorageImport61('server_zip')" ${localSources.length||objectSources.length?'':'disabled'}>开始 ZIP 导入</button></section>
    ...
    window.startStorageImport61=async function(mode=state.serverMaterialImportMode61||'directory_scan'){
      ...
      try{
        let values;
        if(mode==='server_zip'){
          const executionMode=state.serverZipExecutionMode61||'local';
          values={mode,executionMode,storageSourceId:executionMode==='agent'?document.getElementById('si61ZipRemoteSource')?.value:document.getElementById('si61ZipSource')?.value,zipPath:document.getElementById('si61ZipPath')?.value,targetPrefix:document.getElementById('si61TargetPrefix')?.value};
  ...
  window.openDataUploadStorage61=async function(){
...
window.installUsability417=function(){
  ...
  window.decorateDatasetUsability417=function(){
  ...
  window.renderAnnotatorLegacy_1=function(){
  ...
  window.syncReferenceLabels417=syncReferenceLabels417;
  window.decorateAiReferenceLabels417=function(){window.renderAiRefs429?.();syncReferenceLabels417()};
  ...
  window.refreshProjectedTraining417=refreshProjected417;

  window.decorateTrainingIteration417=function(aid){const base=state.iteration414?.[aid],root=document.querySelector('.train429-create'),select=root?.querySelector('#tr429Alg'),field=select?.closest('.field'),model=root?.querySelector('#tr429Model');if(base?.version_name){if(field){select.innerHTML='';select.style.display='none';let locked=field.querySelector('.iteration417-engine');if(!locked){locked=document.createElement('div');locked.className='input iteration417-engine';field.appendChild(locked)}field.querySelector('label').textContent='训练引擎（迭代任务锁定）';locked.textContent='Ultralytics Detect'}if(model)model.textContent=`${base.version_name} · ${base.model_name||'最新模型成果'}`}refreshProjected417()};
  window.syncTrainingIteration417=function(aid){const a=(state.algorithms||[]).find(x=>String(x.id)===String(aid)),latest=a?.versions?.[0];if(latest)state.iteration414[aid]={version_id:latest.id,version_name:latest.version_name,model_name:latest.model_name,path:latest.stored_path||latest.path||''};window.refreshTrain429?.();if(latest)showIterationBase414(aid);refreshProjected417();return latest||null};
  window.decorateTrainSettingsIteration417=function(){const base=state.iteration414?.[state.trainingDraft?.algorithmId],model=document.getElementById('ts428Model');if(base?.version_name&&model){const text=`${base.version_name} · ${base.model_name||'最新模型成果'}`;if(model.tagName==='SELECT'){model.innerHTML=`<option value="${esc(base.path||base.model_name||'latest')}">${esc(text)}</option>`;model.value=base.path||base.model_name||'latest'}else model.value=text;model.readOnly=true;model.disabled=true;const label=model.closest('.field')?.querySelector('label');if(label)label.textContent='迭代起点（锁定最新版本）'}};
  window.decorateConvertResourceButton417=function(){const root=document.querySelector('.convert428-create'),footer=root?.querySelector('.row.end'),primary=footer?.querySelector('.btn.primary');if(footer&&primary&&!footer.querySelector('.configure417-resource'))primary.insertAdjacentHTML('beforebegin',`<button class="btn configure417-resource" onclick="closeModal();closeModal();setPage('部署资源')">配置部署资源</button>`)};
...
(()=>{
  ...
  window.openModelConfigM4=function(id=''){
...
(function(){
  ...
  window.renderDetectionResultLegacy2= function(r,title){
...
(function(){
  ...
  window.openExportModelLegacy=function(modelName){
  ...
  window.submitExportModelLegacy=async function(modelName){
  ...
  window.renderTestCore30=function renderTestCore30(){
...
(function(){
  ...
  window.openAutoLabelLegacy_1=async function(){
  ...
  window.startPrelabelLegacy_1=async function(){
...
(function(){
  ...
  window.testModelConfigLegacyV35=async function(id){const action=window.NavigationStability?.action?.(state.page);try{const r=await api(`/api/v35/model-configs/${id}/test-annotation`,{method:'POST'});if(action&&!action.isCurrent())return;modal('模型连接与标注解析测试',`<div class="model-m4-test"><div class="report429-kpis"><div><span>连接</span><b>${r.reachable?'成功':'失败'}</b></div><div><span>提供商</span><b>${esc(providerNames[r.provider]||r.provider||'-')}</b></div><div><span>模型</span><b>${esc(r.model||'-')}</b></div><div><span>耗时</span><b>${Number(r.latency_ms||0)} ms</b></div></div><div class="alert ok">成功解析 ${r.parsed_boxes?.length||0} 个候选框；本次测试不会写入任何图片标注。</div><details><summary>脱敏响应预览</summary><pre class="log small-log">${esc(r.raw_preview||'')}</pre></details><div class="row end"><button class="btn" onclick="closeModal()">关闭</button></div></div>`,true)}catch(e){if(action&&!action.isCurrent())return;toast(e.message||e)}};
...
(function(){
  ...
  window.selectDeployTargetCoreV39=k=>{state.deployTarget=k;renderDeployFormOnly()};
  ...
  function clearDeployPollV39(){
    ...
    else if(window.__deployPollV39)clearTimeout(window.__deployPollV39);
    window.__deployPollV39=null;
  }
  function armDeployPollV39(){
    clearDeployPollV39();
    if(state.page!=='部署转换'||!(state.deployJobs||[]).some(j=>['queued','waiting_resource','running'].includes(j.status)))return null;
    const callback=()=>window.refreshDeployJobsV39?.().catch?.(()=>{});
    if(window.PollRegistryRuntime?.startTimeout){
      window.__deployPollV39=window.PollRegistryRuntime.startTimeout('deploy-jobs-v39','部署转换',callback,1800);
    }else{
      window.__deployPollV39=setTimeout(callback,1800);
    }
  ...
  window.stopDeployJob=async id=>{const result=await safe(api(`/api/v39/projects/${pid()}/deploy/jobs/${id}/stop`,{method:'POST'}));if(!result)return;if(await window.refreshActiveVersionConvert428?.())return;await window.refreshDeployJobsV39()};
  window.openRknnHardwareVerify=async function(id){const job=window.versionConversionJob428?.(id)||(state.deployJobs||[]).find(x=>String(x.id)===String(id));if(!job)return toast('未找到转换任务');const chip=String(job.params?.chip||'').toUpperCase();modal('RKNN 板端验证',`<div class="deploy-hw-verify"><div class="compact-note"><b>${esc(job.source_name||'RKNN模型')}</b> · ${esc(chip)}<br>系统会先检查模型完整性和匹配的真实 Rockchip 节点，再允许上传测试图片执行 RKNNLite 推理。</div><div id="rknnVerifyPreflight" class="alert soft">正在检查板端验收条件…</div><div id="rknnVerifyInput" class="field" hidden><label>测试图片</label><input id="rknnVerifyFile" class="input" type="file" accept="image/*"></div><div id="rknnVerifyState" class="alert soft">等待验收条件检查</div><div class="row end"><button class="btn" onclick="closeModal()">取消</button><button id="rknnVerifySubmit" class="btn primary" disabled onclick="submitRknnHardwareVerify('${id}')">开始板端验证</button></div></div>`,true);try{const ready=await api(`/api/v39/projects/${pid()}/deploy/jobs/${id}/hardware-tests/preflight`),nodes=Array.isArray(ready.board_nodes)?ready.board_nodes:[],preflight=document.getElementById('rknnVerifyPreflight'),input=document.getElementById('rknnVerifyInput'),submit=document.getElementById('rknnVerifySubmit'),status=document.getElementById('rknnVerifyState');if(!preflight)return;state.rknnHardwarePreflight=ready;if(ready.ready){const labels=nodes.map(n=>`${n.display_name||n.node_id||'Rockchip节点'}${n.rknn_lite_version?` · RKNNLite ${n.rknn_lite_version}`:''}`).join('；');preflight.className='alert ok';preflight.innerHTML=`<b>验收条件已满足</b> · 已匹配 ${nodes.length} 个 ${esc(String(ready.chip||chip).toUpperCase())} 板端节点${labels?`<br>${esc(labels)}`:''}`;if(input)input.hidden=false;if(submit)submit.disabled=false;if(status)status.textContent='请选择一张真实测试图片'}else{preflight.className='alert warn';preflight.innerHTML=`<b>暂不能开始板端验证</b> · ${esc(ready.reason||'未满足验收条件')}${ready.solution?`<br>${esc(ready.solution)}`:''}`;if(status)status.textContent='等待板端节点满足验收条件'}}catch(e){const preflight=document.getElementById('rknnVerifyPreflight'),status=document.getElementById('rknnVerifyState');if(preflight){preflight.className='alert err';preflight.textContent=e.message||String(e)}if(status)status.textContent='验收条件检查失败'}};
  function patchRknnHardwareVerifyProgress(status,task,taskId){if(!status)return false;let shell=status.querySelector?.('[data-rknn-verify-live]');if(!shell){status.className='alert soft rknn-verify-live-wrap';status.innerHTML='<div class="rknn-verify-live" data-rknn-verify-live><div class="rknn-verify-live-head"><b data-rknn-verify-title></b><span data-rknn-verify-percent>0%</span></div><div class="rknn-verify-live-progress"><i data-rknn-verify-bar data-progress="0.00" style="transform:scaleX(0)"></i></div><small data-rknn-verify-detail></small></div>';shell=status.querySelector?.('[data-rknn-verify-live]')}const view=window.PlatformCore?.deployment?.deploymentTaskView?.(task)||{statusText:task?.status,phase:task?.phase||task?.stage||'',percent:Number(task?.progress_percent??task?.progress??0),runtimeText:''},percent=Math.max(0,Math.min(100,Number(view.percent||0))),title=shell?.querySelector?.('[data-rknn-verify-title]'),label=shell?.querySelector?.('[data-rknn-verify-percent]'),bar=shell?.querySelector?.('[data-rknn-verify-bar]'),detail=shell?.querySelector?.('[data-rknn-verify-detail]');if(title)title.textContent=view.statusText||task?.status||'板端验证';if(label)label.textContent=percent.toFixed(percent%1?1:0)+'%';if(bar){bar.dataset.progress=percent.toFixed(2);bar.style.transform='scaleX('+(percent/100).toFixed(4)+')'}if(detail)detail.textContent=[view.runtimeText,view.phase,taskId?`任务 ${taskId}`:''].filter(Boolean).join(' · ');return true}
  window.submitRknnHardwareVerify=async function(id){const file=document.getElementById('rknnVerifyFile')?.files?.[0];if(!file)return toast('请选择测试图片');const btn=document.getElementById('rknnVerifySubmit'),status=document.getElementById('rknnVerifyState');if(btn)btn.disabled=true;try{const form=new FormData();form.append('file',file);let task=await api(`/api/v39/projects/${pid()}/deploy/jobs/${id}/hardware-tests`,{method:'POST',body:form});const taskId=task.id||task.task_id;if(!taskId)throw new Error('板端验证任务创建失败');task=await window.PlatformCore.taskPoller.waitForTaskTerminal({initialTask:task,registry:window.PollRegistryRuntime,key:`rknn-hardware-verify:${taskId}`,ownerPages:String(state.page||'算法列表'),delay:900,maxAttempts:700,load:()=>api(`/api/v62/projects/${pid()}/tasks/${taskId}`),onUpdate:current=>patchRknnHardwareVerifyProgress(status,current,taskId)});if(String(task.status||'').toUpperCase()!=='SUCCEEDED')throw new Error(task.error||`板端验证未通过：${task.status}`);if(!(await window.refreshActiveVersionConvert428?.())){await loadDeployData(true);renderDeployJobsOnly()}closeModal();toast('RKNN 已完成瑞芯微实机 Runtime 验证')}catch(e){if(e?.name==='AbortError')return;if(status){status.className='alert err';status.textContent=e.message||String(e)}toast(e.message||e)}finally{if(btn)btn.disabled=false}};
  window.openRknnAcceptanceReport=async function(id){try{const report=await api(`/api/v39/projects/${pid()}/deploy/jobs/${id}/hardware-tests/report`),v=report.verification||{},board=report.board||{},model=report.model||{},target=report.target||{},input=v.input||{};modal('RKNN 实机验收报告',`<div class="deploy-hw-report"><div class="alert ok"><b>板端 Runtime 验收通过</b> · ${esc(String(target.chip||'').toUpperCase())}</div><div class="report429-kpis"><div><span>验收节点</span><b>${esc(board.node_id||'-')}</b></div><div><span>RKNNLite</span><b>${esc(board.rknn_lite_version||'-')}</b></div><div><span>推理耗时</span><b>${Number(v.inference_ms||0).toFixed(2)} ms</b></div><div><span>输出数量</span><b>${Number(v.output_count||0)}</b></div></div><div class="field"><label>模型 SHA256</label><code>${esc(model.sha256||'-')}</code></div><div class="field"><label>测试图 SHA256</label><code>${esc(input.sha256||'-')}</code></div><div class="field"><label>验证任务 / generation</label><div>${esc(v.task_id||'-')} / ${esc(v.execution_generation??'-')}</div></div><div class="field"><label>验证时间</label><div>${esc(v.verified_at||'-')}</div></div><div class="field"><label>输出 shape</label><pre class="log small-log">${esc(JSON.stringify(v.output_shapes||[]))}</pre></div><div class="alert soft">${esc(report.statement||'')}</div><div class="row end"><a class="btn" href="/api/v39/projects/${pid()}/deploy/jobs/${id}/hardware-tests/report?download=true" target="_blank" rel="noopener">下载 JSON 报告</a><button class="btn primary" onclick="closeModal()">关闭</button></div></div>`,true)}catch(e){toast(e.message||e)}};
  window.deleteDeployJob=async id=>{if(!confirm('确认删除这条转换任务和产物？'))return;const result=await safe(api(`/api/v39/projects/${pid()}/deploy/jobs/${id}`,{method:'DELETE'}));if(!result)return;if(await window.refreshActiveVersionConvert428?.())return;await loadDeployData(true);renderDeployJobsOnly()};
  window.openDeployLog=async id=>{const job=window.versionConversionJob428?.(id)||(state.deployJobs||[]).find(x=>x.id===id)||{};const txt=await safe(api(`/api/v39/projects/${pid()}/deploy/jobs/${id}/log`))||'';modal(`转换日志 · ${job.source_name||id}`,`<div class="deploy-log-head"><div>${statusPill(job.status)} <span class="item-sub">${esc(job.stage||'')}</span></div><button class="btn mini" onclick="refreshDeployLog('${id}')">刷新</button></div><pre id="deployLogText" class="log deploy-log">${esc(txt)}</pre>`,true);const pre=document.getElementById('deployLogText');if(pre)pre.scrollTop=pre.scrollHeight};
...
(function(){
  ...
  const COMPONENT_SCAN_POLL_KEY='component-scan-v40';
  function componentScanActive(s=state.componentScan){return ['queued','running'].includes(String(s?.status||'').toLowerCase())}
  function componentProgressValue(s=state.componentScan){return Math.max(0,Math.min(100,Number(s?.progress||0)))}
  function componentAtlasHtml(s){return s?.atlas?`<div class="atlas-check"><div><b>华为 Atlas / Ascend</b><span>${s.atlas.atc_ready?'OM 转换可用':'ATC 未就绪'}</span></div><div class="item-sub">${(s.atlas.detected_soc_versions||[]).length?'检测到 '+s.atlas.detected_soc_versions.join('、'):'未读取到本机 NPU；仍可在纯 CANN 转换服务器编译 OM，创建任务时填写目标 soc_version。'}</div></div>`:''}
  function componentBodyHtml(){const s=state.componentScan||{},sum=s.summary||{ready:0,warning:0,missing:0,total:0},progress=componentProgressValue(s),running=componentScanActive(s);return `<div class="component-summary"><div><span>满足</span><b data-component-summary="ready">${sum.ready||0}</b></div><div><span>需确认</span><b data-component-summary="warning">${sum.warning||0}</b></div><div><span>缺失</span><b data-component-summary="missing">${sum.missing||0}</b></div><div><span>总项</span><b data-component-summary="total">${sum.total||0}</b></div></div><div class="component-progress" data-component-progress ${running?'':'hidden'}><div class="progress-bar"><i data-component-progress-bar data-progress="${progress.toFixed(2)}" style="transform:scaleX(${(progress/100).toFixed(4)})"></i></div><div><b data-component-stage>${esc(s.stage||'检测中')}</b><span data-component-percent>${Math.round(progress)}%</span></div></div><div class="capability-grid" data-component-capabilities>${(s.capabilities||[]).map(capCard).join('')}</div><div class="component-grid" data-component-details><section class="component-group"><div class="component-group-title">组件明细</div>${(s.components||[]).map(compRow).join('')||'<div class="empty">尚未检测</div>'}</section></div><div data-component-atlas>${componentAtlasHtml(s)}</div>`}
  function renderCompBody(){const box=document.getElementById('componentBody');if(!box)return false;box.innerHTML=componentBodyHtml();return true}
...
(function(){
  ...
  const DEPLOY_PLUGIN_CACHE_KEY='cl_deploy_plugins_v41_snapshot';
  const deployPluginCacheRowV41=p=>({
    id:p?.id||'',name:p?.name||'',sdk:p?.sdk||'',output:p?.output||'',
    chips:Array.isArray(p?.chips)?p.chips:[],status:p?.status||'missing',
    description:p?.description||'',configured_count:Number(p?.configured_count||0),
  });
  function restoreDeployPluginCacheV41(){
    try{
      const cached=JSON.parse(localStorage.getItem(DEPLOY_PLUGIN_CACHE_KEY)||'null');
      if(!cached||!Array.isArray(cached.items))return false;
      state.deployPlugins=cached.items.map(deployPluginCacheRowV41);
      state.deployHostOs=String(cached.host_os||'');
      state.deployPluginsLoadedAt=Number(cached.ts||0);
      return state.deployPlugins.length>0;
    }catch(_){return false}
  }
  function persistDeployPluginCacheV41(){
    try{localStorage.setItem(DEPLOY_PLUGIN_CACHE_KEY,JSON.stringify({
      ts:Date.now(),host_os:state.deployHostOs||'',items:(state.deployPlugins||[]).map(deployPluginCacheRowV41),
    }))}catch(_){}
  }
  window.invalidateDeployPluginCacheV41=()=>{
    state.deployPluginsLoadedAt=0;
    try{localStorage.removeItem(DEPLOY_PLUGIN_CACHE_KEY)}catch(_){}
  };
  restoreDeployPluginCacheV41();
  const pluginState=s=>`<span class="pill ${s==='ready'?'ok':'err'}">${s==='ready'?'可用':'缺少 SDK/工具链'}</span>`;
  window.loadDeployPluginsV41=async({force=false}={})=>{const age=Date.now()-Number(state.deployPluginsLoadedAt||0);if(!force&&state.deployPluginsLoadedAt>0&&age>=0&&age<DEPLOY_PLUGIN_CACHE_TTL_MS)return state.deployPlugins;const r=await safe(api('/api/v41/deploy/plugins'));if(r){state.deployPlugins=r.items||[];state.deployHostOs=r.host_os||'';state.deployPluginsLoadedAt=Date.now();persistDeployPluginCacheV41()}return state.deployPlugins};
  ...
  window.renderDeployPluginsV41=async({force=false}={})=>{
    const hasSnapshot=state.deployPluginsLoadedAt>0||(state.deployPlugins||[]).length>0;
...
(function(){
  ...
  window.runPolicyLegacy42=async id=>{try{await api(`/api/v42/projects/${pid()}/iteration-policies/${id}/run`,{method:'POST'});toast('自动迭代已启动');setTimeout(()=>renderIterationV42(),900)}catch(e){toast(e.message||e)}};
...
(function(){
  ...
  state.source422ProjectId=String(state.source422ProjectId||'');
  ...
  async function refreshDashboardExtras422({force=false}={}){
    ...
    const sourceAge=Date.now()-Number(state.source422LoadedAt||0);
    const sourceSnapshotFresh=state.source422ProjectId===projectId
      && state.source422LoadedAt>0
      && sourceAge>=0
      && sourceAge<SOURCE422_CACHE_TTL_MS;
    ...
    const task=(async()=>{
      const [sources,quality]=await Promise.all([
        sourceSnapshotFresh ? Promise.resolve(null) : safe(api(`/api/v42/projects/${projectId}/sources`)),
        safe(api(`/api/v42/projects/${projectId}/quality-overview`)),
      ]);
      if(String(pid()||'')!==projectId)return false;
      let changed=sourceSnapshotFresh;
      if(sources){state.v42.sources=sources.items||[];state.source422LoadedAt=Date.now();state.source422ProjectId=projectId;changed=true}
  ...
  async function loadSourcesOnly422(){
    const projectId=String(pid()||'');if(!projectId)return null;
    const r=await safe(api(`/api/v42/projects/${projectId}/sources`));
    if(!r||String(pid()||'')!==projectId)return null;
    state.v42.sources=r.items||[];
    state.source422LoadedAt=Date.now();
    state.source422ProjectId=projectId;
    return state.v42.sources;
  }
  ...
  function paintSourceSnapshot422(){
    const s=state.v42.sources||[],stats=document.querySelectorAll('.source422-stats>div>b');
    const values=[s.length,s.filter(x=>x.collect_mode==='auto').length,s.filter(x=>['queued','running'].includes(x.runtime_status)).length,s.filter(x=>(x.runtime_status||x.status)==='failed').length];
    stats.forEach((node,index)=>{if(index<values.length)node.textContent=String(values[index])});
    renderSourceRows422();
  }
  ...
  async function refreshSources422(){
    const projectId=String(pid()||'');if(!projectId)return state.v42.sources||[];
    const r=await safe(api(`/api/v42/projects/${projectId}/sources`));
    if(r&&String(pid()||'')===projectId){state.v42.sources=r.items||[];state.source422LoadedAt=Date.now();state.source422ProjectId=projectId}
    if(state.page==='素材接入'&&String(pid()||'')===projectId)paintSourceSnapshot422();
    return state.v42.sources||[];
  }
  window.renderSources422=function(){const currentProjectId=String(pid()||''),sameProject=state.source422ProjectId===currentProjectId,hasSnapshot=sameProject&&(Number(state.source422LoadedAt||0)>0||(state.v42.sources||[]).length>0);const s=hasSnapshot?(state.v42.sources||[]):[];document.getElementById('view').innerHTML=`<div class="source422"><div class="source422-top"><div class="source422-stats"><div><span>素材源</span><b>${s.length}</b></div><div><span>自动执行</span><b>${s.filter(x=>x.collect_mode==='auto').length}</b></div><div><span>运行中</span><b>${s.filter(x=>['queued','running'].includes(x.runtime_status)).length}</b></div><div><span>异常</span><b>${s.filter(x=>(x.runtime_status||x.status)==='failed').length}</b></div></div><button class="btn primary" onclick="openSource422()">＋ 新增素材源</button></div><section class="panel"><div class="source422-filterbar"><input id="src422Q" class="input" placeholder="搜索名称或地址" oninput="applySourceFilters422()"><select id="src422Type" class="select" onchange="applySourceFilters422()"><option value="all">全部接入方式</option><option value="folder">目录 / 共享目录</option><option value="rtsp">视频流 / RTSP</option><option value="http_json">业务系统 API</option></select><select id="src422Status" class="select" onchange="applySourceFilters422()"><option value="all">全部状态</option><option value="ready">正常</option><option value="queued">排队中</option><option value="running">运行中</option><option value="failed">失败</option><option value="unchecked">未检测</option><option value="disabled">已停用</option></select><select id="src422Mode" class="select" onchange="applySourceFilters422()"><option value="all">全部执行方式</option><option value="manual">手动</option><option value="auto">自动</option></select><button class="btn" onclick="refreshSources422()">刷新</button></div><div class="table-wrap"><table class="table source422-table"><thead><tr><th>素材源</th><th>接入方式</th><th>状态</th><th>执行方式</th><th>目标数据集</th><th>最近完成</th><th>下次执行</th><th>最近入库</th><th>操作</th></tr></thead><tbody id="source422Rows"></tbody></table></div></section></div>`;const body=document.getElementById('source422Rows');if(hasSnapshot)renderSourceRows422();else if(body)body.innerHTML='<tr><td colspan="9" class="empty-row">首次读取素材源…</td></tr>';window.PollRegistryRuntime?.replaceSourceTimer?.();const age=Date.now()-Number(state.source422LoadedAt||0);if(hasSnapshot&&age>=0&&age<SOURCE422_CACHE_TTL_MS)return;if(state.source422RenderRefresh)return state.source422RenderRefresh;state.source422RenderRefresh=Promise.resolve(loadSourcesOnly422()).then(rows=>{if(state.page!=='素材接入')return rows;if(rows===null&&!hasSnapshot){const target=document.getElementById('source422Rows');if(target)target.innerHTML='<tr><td colspan="9" class="empty-row">素材源读取失败，请点击刷新重试</td></tr>';return rows}paintSourceSnapshot422();return rows}).finally(()=>{state.source422RenderRefresh=null});return state.source422RenderRefresh};
...
(function(){
  ...
  window.renderAlgorithmsLegacy423_1=async function(){
  ...
  window.showVersionDeploymentsLegacy423_1=async function(aid,vid){
...
(function(){
  ...
  state.video424=[]; state.video424LoadedAt=Number(state.video424LoadedAt||0); state.video424RefreshPromise=null; state.prelabel424=[];
  ...
  window.renderQualityCenterLegacy424_1=async function(){
  ...
  window.setDataTabLegacy424_1=function(t){state.data424Tab=t;state.data424Selected.clear();renderDatasets424()};
  ...
  async function loadVideo424(){
    ...
    state.video424=rows.map(t=>videoCore424()?.normalizeVideoTask(t)||t);
    state.video424LoadedAt=Date.now();
    return state.video424;
  }
  ...
  function videoTaskRow424(t){const progress=Math.max(0,Math.min(100,Number(t.progress||0)));return`<tr data-task-id="${esc(t.id)}" data-updated-at="${esc(t.updated_at||'')}"><td><b>${esc(t.video_name||'-')}</b><div class="muted-line">${esc(t.id)}</div></td><td>${esc(t.samplingText||'-')}<div class="muted-line">最多 ${t.max_frames||'不限'} 张</div></td><td>${splitName424(t.split||'unassigned')}</td><td>${taskStatus424(t)}${t.runtimeText?`<div class="muted-line">${esc(t.runtimeText)}</div>`:''}</td><td><div class="progress424"><i data-progress="${progress.toFixed(2)}" style="transform:scaleX(${(progress/100).toFixed(4)})"></i></div><span data-video-progress-text>${progress}% · ${t.extractedFrames||0}张</span></td><td>${esc(t.current_item||'-')}</td><td>${esc(String(t.created_at||'').slice(0,19))}<div class="muted-line">${esc(String(t.finished_at||'').slice(0,19))}</div></td><td><div class="row"><button class="btn mini" onclick="videoDetail424('${t.id}')">详情</button>${videoCore424()?.isActiveVideoTask(t)?`<button class="btn mini danger" onclick="stopVideo424('${t.id}')">停止</button>`:''}</div>${t.error?`<div class="alert err mini-alert">${esc(t.error)}</div>`:''}</td></tr>`}
  function patchVideoRow424(current,next){if(!current||!next||current.cells?.length!==next.cells?.length)return next;current.dataset.updatedAt=next.dataset.updatedAt||'';for(let index=0;index<next.cells.length;index+=1){const currentCell=current.cells[index],nextCell=next.cells[index];if(index===4){const currentBar=currentCell.querySelector('.progress424 i'),nextBar=nextCell.querySelector('.progress424 i'),currentText=currentCell.querySelector('[data-video-progress-text]'),nextText=nextCell.querySelector('[data-video-progress-text]');if(currentBar&&nextBar&&currentText&&nextText){currentBar.dataset.progress=nextBar.dataset.progress||'';currentBar.style.transform=nextBar.style.transform;currentText.textContent=nextText.textContent;continue}}if(currentCell.innerHTML!==nextCell.innerHTML)currentCell.innerHTML=nextCell.innerHTML}return current}
  function patchVideoRows424(){const body=document.getElementById('video424Rows');if(!body)return;body.querySelector('[data-video-loading]')?.remove();const tasks=state.video424||[],seen=new Set();tasks.forEach((t,index)=>{const id=String(t.id||'');if(!id)return;seen.add(id);let current=body.querySelector(`[data-task-id="${CSS.escape(id)}"]`);if(current?.dataset.updatedAt===(t.updated_at||''))return;const tpl=document.createElement('template');tpl.innerHTML=videoTaskRow424(t).trim();const next=tpl.content.firstElementChild;if(!current)current=next;else current=patchVideoRow424(current,next);const reference=body.children[index]||null;if(reference!==current)body.insertBefore(current,reference)});body.querySelectorAll('[data-task-id]').forEach(row=>{if(!seen.has(String(row.dataset.taskId||'')))row.remove()});const empty=body.querySelector('[data-video-empty]');if(tasks.length)empty?.remove();else if(!empty)body.innerHTML='<tr data-video-empty><td colspan="8">暂无切帧任务</td></tr>'}
  window.refreshVideo424Delta=function(){
    if(state.video424RefreshPromise)return state.video424RefreshPromise;
    state.video424RefreshPromise=(async()=>{
      const rows=await loadVideo424();
      if(state.page==='视频切帧'){
        if(rows!==null)patchVideoRows424();
        else if(!state.video424LoadedAt){const body=document.getElementById('video424Rows');if(body)body.innerHTML='<tr data-video-empty><td colspan="8">视频切帧任务读取失败，请稍后刷新</td></tr>'}
      }
...
(()=>{
  ...
  window.startAlgorithmTrainingLegacy423_2=function(id){window.setPage?.('训练任务');setTimeout(()=>{if(state.page==='训练任务')openTrain425(id)},30)};
  ...
  window.openTrainSettingsCore425=function(){const c=state.train425Config||cfg425(),t=target425(),models=t?.base_models||[];modal('训练配置设置',`<div class="train425-settings">
  ...
  function completion425(j,cfg,hist){
    const requested=Number(j.requested_epochs||j.total_epochs||j.epochs||cfg.epochs||0)||0;
    const historyEpoch=Number(hist?.at?.(-1)?.epoch||0)||0;
    const completed=Number(j.completed_epochs||j.current_epoch||historyEpoch||0)||0;
    const reason=String(j.completion_reason||'').trim();
    const reasonText=({
      quality_target_reached:'达到目标指标，提前完成',
      requested_epochs_completed:'达到最大训练轮次',
      early_stopping:'提前结束',
      completed:'训练完成',
      manual_stop:'人工停止',
      stopped:'人工停止',
    })[reason]||String(j.completion_message||j.quality_gate_reason||j.message||'训练完成');
    const targetText=j.training_outcome==='target_reached'?'已达标':(
      reason==='requested_epochs_completed'&&j.quality_gate?.stop_threshold>0?'未达目标，已跑满':'-'
    );
    return{requested,completed,reasonText,targetText};
  }
...
(()=>{
  ...
  loadRelated=async function loadRelatedCanonical426(){
    ...
    if(info){state.project=info.project;state.models=info.models||[]}
  ...
  window.doUploadImagesLegacy4261=async function(inp){const fs=[...(inp.files||[])];if(!fs.length)return;const fd=new FormData();fs.forEach(f=>fd.append('files',f));fd.append('dataset_id','default');try{await api(`/api/projects/${pid()}/images`,{method:'POST',body:fd});closeModal();await loadRelated();renderDatasets424();toast(`已上传 ${fs.length} 张图片`)}catch(e){toast(e.message||e)}};
  window.doUploadZipLegacy426_1=async function(inp){if(!inp?.files?.length)return;if(!window.ZipImportRuntime?.upload)return toast('ZIP 导入模块正在加载，请稍后重试');return window.ZipImportRuntime.upload(inp).catch(()=>null)};
...
(()=>{
  ...
  function gateDecision428(g){const d=String(g?.decision||'');if(d.includes('target')||d.includes('达标'))return '达到目标，提前完成';if(d.includes('continue_below_target'))return '未达到目标，继续训练';return '通过本次检查，继续训练'}
  ...
  window.toggleAlgorithmLegacy428_1=function(id){state.alg428Expanded[id]=!state.alg428Expanded[id];renderAlgCards428()};
  window.filterAlgorithms428=renderAlgCards428;
  window.renderAlgorithmsLegacy423_2=async function(){
...
(()=>{
  ...
  function active429(a){return(state.jobs||[]).find(j=>(j.asset_algorithm_id===a.id||j.algorithm_asset_id===a.id)&&['queued','waiting','pending','starting','running','pausing','paused','resuming','stopping','cancel_requested'].includes(String(j.status||'').toLowerCase()))}
  function targetName429(t){return t?.name||t?.base_url||'训练资源'}
  function status429(s){const value=String(s||'').toLowerCase();return({queued:'排队中',waiting:'等待资源',pending:'等待中',starting:'启动中',running:'训练中',pausing:'暂停中',paused:'已暂停',resuming:'恢复中',stopping:'停止中',cancel_requested:'取消中',done:'已完成',finished:'已完成',completed:'已完成',succeeded:'已完成',success:'已完成',failed:'失败',stopped:'已停止',cancelled:'已取消',canceled:'已取消'})[value]||s||'-'}

  // ---------- algorithm list: every asset can start a training iteration ----------
  function verRow429(a,v){const can=!!String(v.stored_path||'').trim(),isCurrent=String(a.current_version_id||'')===String(v.id||'');return `<div class="alg428-version-row ${isCurrent?'is-current':''}">
    <div class="alg428-version-id"><i></i><div><b>${esc(v.version_name||'-')} ${isCurrent?'<em class="alg428-current-badge">当前版本</em>':''}</b><span>${dt429(v.finished_at||v.created_at)}</span></div></div>
  ...
  window.renderAlgorithmsLegacy423_3=async function(){
    const inds=[...new Set((state.algorithms||[]).map(a=>a.industry).filter(Boolean))].sort();
  ...
  window.viewAlgorithm429=function(id){const a=(state.algorithms||[]).find(x=>x.id===id);if(!a)return;const vs=a.versions||[],jobs=(state.jobs||[]).filter(j=>String(j.asset_algorithm_id||j.algorithm_asset_id||'')===String(a.id)),run=active429(a),current=vs.find(v=>String(v.id||'')===String(a.current_version_id||''))||vs[0],success=jobs.filter(j=>['done','finished','completed','succeeded','success'].includes(String(j.status||'').toLowerCase())).length;modal('算法详情',`<div class="alg428-detail"><section><div class="alg428-detail-name"><div class="alg428-logo">${esc((a.name||'算').slice(0,1))}</div><div><h2>${esc(a.name)}</h2><span>${esc(algType429(a.algorithm_type))}${a.industry?` · ${esc(a.industry)}`:''}</span></div></div><p>${esc(a.remark||'暂无算法说明')}</p><div class="alg429-detail-stats"><div><span>训练任务</span><b>${jobs.length}</b></div><div><span>成功训练</span><b>${success}</b></div><div><span>算法版本</span><b>${vs.length}</b></div><div><span>当前版本</span><b>${esc(current?.version_name||'尚未训练')}</b></div><div><span>当前 mAP50</span><b>${pct429(metric429(current))}</b></div></div>${run?`<div class="alg429-detail-live"><span class="pill warn">${esc(status429(run.status))}</span><b>${esc(run.run_name||run.task_name||run.id)}</b><em>${Math.max(0,Math.min(100,Number(run.progress_percent||0))).toFixed(0)}%</em></div>`:''}<div class="row"><button class="btn primary" onclick="startAlgorithmTraining429('${a.id}')">开始训练</button><button class="btn" onclick="algorithmReport429('${a.id}')">查看综合训练报告</button></div></section><section><div class="alg428-version-head"><b>版本列表</b><span>${vs.length} 个版本</span></div>${vs.length?vs.map(v=>verRow429(a,v)).join(''):'<div class="empty">暂无版本</div>'}</section></div>`,true)};
  ...
  window.openVersionLineage429=function(aid,vid){
    const a=(state.algorithms||[]).find(x=>String(x.id)===String(aid)),v=(a?.versions||[]).find(x=>String(x.id)===String(vid)),l=v?.training_lineage;
    if(!l)return toast('该历史版本没有训练溯源记录');
    const base=l.base||{},ex=l.execution||{},params=l.parameters||{},arts=Array.isArray(l.artifacts)?l.artifacts:[];
    const rows=[
      ['数据版本',l.dataset_revision_id],['训练快照',l.snapshot_id],['训练任务',l.task_id],
      ['训练框架',l.framework],['基础版本',base.version_name||base.version_id],['基础模型',base.model],
      ['基础选择',base.selection_reason],['执行方式',ex.mode],['执行 Worker',ex.worker_id],
      ['执行节点',ex.node_id],['执行代次',ex.execution_generation],['请求设备',ex.requested_device],
      ['分配设备',ex.assigned_device],['实际设备',ex.actual_device],['GPU',ex.gpu_name||ex.gpu_uuid||ex.gpu_id],
    ].filter(([,value])=>value!==undefined&&value!==null&&String(value).trim()!=='');
    const paramRows=Object.entries(params.actual||params.requested||{});
  ...
  window.resumeConfirmedIterationActionCore429=async function(aid,vid){
    ...
    if(!a||!v||!confirmed||confirmed.status!=='confirmed')return toast('当前版本没有已确认的迭代动作');
    const source=confirmed.source||{};
    if(String(source.algorithm_id||'')!==String(aid)||String(source.version_id||'')!==String(vid)){
      return toast('已确认动作溯源与当前版本不一致，请刷新后重试');
    }
    if(confirmed.action==='continue_training'){
      state.trainingIterationAction=confirmed;
      closeModal();
      await startAlgorithmTraining429(aid);
      setTimeout(()=>{
        const root=document.querySelector('.train429-create');
        if(root&&!root.querySelector('.iteration-action429')){
          root.insertAdjacentHTML('afterbegin',`<div class="alert ok iteration-action429"><b>已确认继续训练</b><span>基于 ${esc(v.version_name||vid)} 的持久化评测动作恢复训练草稿；只有点击“开始训练”后才会创建真实训练任务。</span></div>`);
        }
      },30);
      return;
    }
    if(confirmed.action==='supplement_data'){
      const labels=confirmed.data_draft?.weak_labels||[];
      state.iterationDataDraft=confirmed.data_draft||{};
      ...
      setTimeout(()=>{
        ...
        if(root&&!root.querySelector('.iteration-action429')){
          root.insertAdjacentHTML('afterbegin',`<div class="alert ok iteration-action429"><b>评测补数据草稿</b><span>来自 ${esc(v.version_name||vid)} 的已确认动作${labels.length?` · 优先标签：${labels.map(esc).join('、')}`:''}</span></div>`);
        }
      },20);
      return;
    }
    if(confirmed.action==='business_validation'){
      ...
      setTimeout(()=>{
        ...
        if(root&&!root.querySelector('.iteration-action429')){
          root.insertAdjacentHTML('afterbegin',`<div class="alert ok iteration-action429"><b>已确认进入业务验证</b><span>来源版本 ${esc(v.version_name||vid)} · 继续选择真实验证/发布方式。</span></div>`);
        }
      },20);
      return;
    }
    if(confirmed.action==='manual_review'){
      const review=confirmed.review_entry||{},reasons=review.reason_codes||[],actions=review.recommended_actions||[];
      modal('人工处理记录',`<div class="report429"><section class="report429-card"><header><b>已确认人工处理</b><span>该动作来自算法版本持久化 truth</span></header><dl class="report429-dl"><dt>算法版本</dt><dd>${esc(v.version_name||vid)}</dd><dt>Decision ID</dt><dd title="${esc(source.decision_id||'')}">${esc(source.decision_id||'-')}</dd><dt>Evaluation ID</dt><dd title="${esc(source.evaluation_id||'')}">${esc(source.evaluation_id||'-')}</dd><dt>原因</dt><dd>${esc(reasons.join('、')||'-')}</dd><dt>建议</dt><dd>${esc(actions.join('、')||'-')}</dd></dl></section><div class="row end"><button class="btn" onclick="closeModal()">关闭</button></div></div>`,true);
      return;
    }
    toast('已确认动作类型不受当前页面支持');
  };

  window.confirmIterationAction429=async function(aid,vid){
    const a=(state.algorithms||[]).find(x=>String(x.id)===String(aid));
    const v=(a?.versions||[]).find(x=>String(x.id)===String(vid));
    const decision=v?.iteration_decision||{};
    const action=({needs_data:'supplement_data',continue_training:'continue_training',ready_for_business_validation:'business_validation',review_required:'manual_review'})[decision.decision];
    if(!a||!v||!action||!decision.decision_id)return toast('当前版本没有可确认的迭代动作');
    try{
      const result=await api(`/api/v12/projects/${pid()}/algorithms/${aid}/versions/${vid}/iteration-actions/confirm`,{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({decision_id:decision.decision_id,action}),
      });
      v.confirmed_iteration_action=result.action||{};
      await window.resumeConfirmedIterationAction429(aid,vid);
    }catch(error){toast(error.message||error)}
  };

  window.openVersionEvaluation429=function(aid,vid){
    const a=(state.algorithms||[]).find(x=>String(x.id)===String(aid)),v=(a?.versions||[]).find(x=>String(x.id)===String(vid)),e=v?.evaluation;
    if(!e)return toast('该历史版本没有独立评测记录');
    const metrics=e.metrics||{},perClass=Array.isArray(e.per_class)?e.per_class:[],errors=Array.isArray(e.error_samples)?e.error_samples:[],weak=Array.isArray(e.weak_labels)?e.weak_labels:[];
    const status=({succeeded:'已完成',not_requested:'未配置独立试验集',failed:'评测失败'})[String(e.status||'').toLowerCase()]||String(e.status||'-');
    const decision=v?.iteration_decision||{},gate=decision.quality_gate||{},signals=decision.signals||{};
    const decisionLabel=({ready_for_business_validation:'可进入业务验证',needs_data:'需补充数据',continue_training:'建议继续训练',review_required:'需人工确认'})[decision.decision]||'尚未形成决策';
    const actionLabel={proceed_to_business_validation:'进入业务验证',supplement_weak_label_data:'补充弱标签数据',supplement_training_data:'补充训练数据',review_fp_fn_samples:'复核 FP / FN 问题样本',continue_from_current_version:'基于当前版本继续训练',configure_quality_gate:'配置训练质量门槛',configure_independent_test_split:'配置独立试验集',inspect_evaluation_failure:'检查评测失败原因',review_evaluation_configuration:'检查评测配置'};
    const m50=metrics['metrics/mAP50(B)'],m95=metrics['metrics/mAP50-95(B)'],precision=metrics['metrics/precision(B)'],recall=metrics['metrics/recall(B)'];
    const confirmed=v?.confirmed_iteration_action||null;
    const confirmedMatches=!!(confirmed&&confirmed.status==='confirmed'&&String(confirmed.source?.decision_id||'')===String(decision.decision_id||''));
    const confirmedLabel=({supplement_data:'补数据',continue_training:'继续训练',business_validation:'业务验证',manual_review:'人工处理'})[confirmed?.action]||'下一步';
    const resumeLabel=({supplement_data:'继续补数据',continue_training:'继续创建训练',business_validation:'继续业务验证',manual_review:'查看人工处理记录'})[confirmed?.action]||'继续处理';
    const actionControl=confirmedMatches
      ?`<div class="alert ok iteration-action-confirmed429"><b>已确认：${esc(confirmedLabel)}</b><span>确认时间 ${esc(confirmed.confirmed_at||'-')} · 后续页面可随时从该版本恢复，不依赖临时页面状态。</span><div class="row end"><button class="btn primary iteration-action-resume429" onclick="resumeConfirmedIterationAction429('${aid}','${vid}')">${esc(resumeLabel)}</button></div></div>`
      :(decision.requires_confirmation===true?`<div class="row end"><button class="btn primary iteration-action-confirm429" onclick="confirmIterationAction429('${aid}','${vid}')">${esc(({needs_data:'确认准备补数据',continue_training:'确认继续训练',ready_for_business_validation:'确认进入业务验证',review_required:'确认人工处理'})[decision.decision]||'确认下一步')}</button></div>`:'');
    const effect=v?.feedback_adoption_outcome||{},effectRows=Array.isArray(effect.weak_label_effects)?effect.weak_label_effects:[],effectOverall=effect.overall_metrics||{},benchmark=e?.benchmark_scope||{};
    const deltaText=value=>{const n=Number(value);return Number.isFinite(n)?`${n>0?'+':''}${(n*100).toFixed(1)} pp`:'-'};
    const strictEffect=effect.status==='comparable'&&effect.strictly_comparable===true;
    const comparisonLabel=effect.status!=='comparable'?'暂不可比较':(strictEffect?'严格可比':'描述性对比');
    const directionLabel=strictEffect?{improved:'改善',declined:'下降',unchanged:'持平'}:{improved:'指标上升',declined:'指标下降',unchanged:'指标持平'};
    const comparisonReason={benchmark_scope_missing:'历史版本缺少固定评测基准',benchmark_scope_mismatch:'前后版本 Test Scope 不同',evaluation_protocol_missing:'历史版本缺少评测协议身份',evaluation_protocol_mismatch:'前后版本评测协议不同',benchmark_input_binding_missing:'历史评测缺少已校验的实际 Test Bundle 绑定'};
    const effectHtml=effect.outcome_id?`<section class="report429-card feedback-effect429"><header><b>补数据效果</b><span>${strictEffect?'同一固定评测基准上的前后版本对比':'来自持久化独立评测，仅作描述性参考'}</span></header><div class="report429-kpis"><div><span>对比状态</span><b>${esc(comparisonLabel)}</b></div><div><span>采用反馈</span><b>${Number(effect.adopted_candidate_count||0)} 条</b></div><div><span>mAP50 变化</span><b>${deltaText(effectOverall['metrics/mAP50(B)']?.delta)}</b></div><div><span>Recall 变化</span><b>${deltaText(effectOverall['metrics/recall(B)']?.delta)}</b></div></div>${effectRows.length?`<div class="table-wrap"><table class="table"><thead><tr><th>原弱标签</th><th>结论</th><th>mAP50</th><th>Recall</th><th>弱项信号</th></tr></thead><tbody>${effectRows.map(row=>`<tr><td>${esc(row.label||'-')}</td><td>${row.comparable?esc(directionLabel[row.direction]||row.direction||'-'):'不可比较'}</td><td>${deltaText(row.metrics?.map50?.delta)}</td><td>${deltaText(row.metrics?.recall?.delta)}</td><td>${deltaText(row.weak_signal?.delta)}</td></tr>`).join('')}</tbody></table></div>`:''}${(effect.reason_codes||[]).length?`<p class="item-sub">无法比较原因：${esc((effect.reason_codes||[]).join('、'))}</p>`:''}${(effect.comparison_reason_codes||[]).length?`<p class="item-sub">严格可比性：${esc((effect.comparison_reason_codes||[]).map(code=>comparisonReason[code]||code).join('；'))}</p>`:''}<p class="item-sub">仅描述本次采用反馈后的评测变化，不会自动触发下一轮训练。</p><details><summary>效果溯源</summary><dl class="report429-dl"><dt>Outcome ID</dt><dd title="${esc(effect.outcome_id||'')}">${esc(effect.outcome_id||'-')}</dd><dt>来源版本</dt><dd>${esc(effect.source_version_id||'-')}</dd><dt>新版本</dt><dd>${esc(effect.new_version_id||'-')}</dd><dt>来源 Benchmark</dt><dd title="${esc(effect.source_benchmark_scope_id||'')}">${esc(effect.source_benchmark_scope_id||'-')}</dd><dt>新版本 Benchmark</dt><dd title="${esc(effect.new_benchmark_scope_id||'')}">${esc(effect.new_benchmark_scope_id||'-')}</dd><dt>Candidate Set</dt><dd title="${esc(effect.candidate_set_id||'')}">${esc(effect.candidate_set_id||'-')}</dd><dt>Adoption ID</dt><dd title="${esc(effect.adoption_id||'')}">${esc(effect.adoption_id||'-')}</dd></dl></details></section>`:''; 
    modal('独立评测',`<div class="report429 evaluation429"><section class="report429-head"><div><span>冻结 Test Split</span><h2>${esc(a?.name||'')} · ${esc(v?.version_name||'-')}</h2><p>先完成无标注图片推理，再读取隐藏 Ground Truth 评分；结果随算法版本持久化。</p></div><div class="report429-kpis"><div><span>状态</span><b>${esc(status)}</b></div><div><span>试验图片</span><b>${Number(e.image_count||0)} 张</b></div><div><span>mAP50</span><b>${pct429(m50)}</b></div><div><span>Recall</span><b>${pct429(recall)}</b></div></div></section>${decision.decision?`<section class="report429-card evaluation-decision429"><header><b>迭代决策</b><span>来自版本持久化的后端决策，不由页面推算</span></header><div class="report429-kpis"><div><span>当前结论</span><b>${esc(decisionLabel)}</b></div><div><span>${esc(gate.metric||"评测指标")}</span><b>${pct429(gate.metric_value)}</b></div><div><span>继续训练下限</span><b>${gate.continue_threshold?pct429(gate.continue_threshold):"-"}</b></div><div><span>达标阈值</span><b>${gate.stop_threshold?pct429(gate.stop_threshold):"-"}</b></div></div>${(decision.recommended_actions||[]).length?`<div class="data426-tags">${decision.recommended_actions.map(code=>`<span>${esc(actionLabel[code]||code)}</span>`).join("")}</div>`:""}<p class="item-sub">FP ${Number(signals.false_positive||0)} · FN ${Number(signals.false_negative||0)} · 问题样本 ${Number(signals.error_sample_count||0)}；系统仅给出建议，不会自动发起下一次训练。</p>${actionControl}</section>`:""}<section class="report429-card"><header><b>总体指标</b><span>Precision / Recall / mAP 来自独立试验集</span></header><dl class="report429-dl"><dt>Precision</dt><dd>${pct429(precision)}</dd><dt>Recall</dt><dd>${pct429(recall)}</dd><dt>mAP50</dt><dd>${pct429(m50)}</dd><dt>mAP50-95</dt><dd>${pct429(m95)}</dd></dl></section>${effectHtml}${weak.length?`<section class="report429-card"><header><b>弱标签</b><span>低于当前评测阈值的标签</span></header><p>${weak.map(x=>`<span class="tag">${esc(x)}</span>`).join(' ')}</p></section>`:''}<section class="report429-card"><header><b>逐标签评测</b></header><div class="table-wrap"><table class="table"><thead><tr><th>标签</th><th>Precision</th><th>Recall</th><th>mAP50</th><th>TP</th><th>FP</th><th>FN</th></tr></thead><tbody>${perClass.map(row=>`<tr><td>${esc(row.label||String(row.class_id??'-'))}</td><td>${pct429(row.precision)}</td><td>${pct429(row.recall)}</td><td>${pct429(row.map50)}</td><td>${Number(row.true_positive||0)}</td><td>${Number(row.false_positive||0)}</td><td>${Number(row.false_negative||0)}</td></tr>`).join('')||'<tr><td colspan="7">暂无逐标签评测数据</td></tr>'}</tbody></table></div></section><section class="report429-card"><header><b>FP / FN 问题样本</b><span>来自独立试验集，不是训练/验证样本</span></header><div class="table-wrap"><table class="table"><thead><tr><th>图片</th><th>误检 FP</th><th>漏检 FN</th><th>涉及标签</th></tr></thead><tbody>${errors.map(row=>`<tr><td>${esc(row.image||'-')}</td><td>${Number(row.fp_count||0)}</td><td>${Number(row.fn_count||0)}</td><td>${esc([...(row.fp_labels||[]),...(row.fn_labels||[])].join('、')||'-')}</td></tr>`).join('')||'<tr><td colspan="4">暂无 FP / FN 问题样本</td></tr>'}</tbody></table></div></section><section class="report429-card"><details><summary>评测溯源</summary><dl class="report429-dl"><dt>Evaluation ID</dt><dd title="${esc(e.evaluation_id||'')}">${esc(e.evaluation_id||'-')}</dd><dt>评测基准</dt><dd title="${esc(benchmark.scope_id||'')}">${benchmark.scope_id?`${esc(benchmark.scope_id)} · ${Number(benchmark.test_image_count||0)} 张`:'历史版本未冻结'}</dd><dt>评测输入绑定</dt><dd>${benchmark.binding_level==='bundle_verified'?'已校验 Test Bundle':'仅 Snapshot truth'}</dd><dt>评测协议 ID</dt><dd title="${esc(e.evaluation_protocol_id||'')}">${esc(e.evaluation_protocol_id||'-')}</dd><dt>数据版本</dt><dd title="${esc(e.dataset_revision_id||'')}">${esc(e.dataset_revision_id||'-')}</dd><dt>训练快照</dt><dd title="${esc(e.snapshot_id||'')}">${esc(e.snapshot_id||'-')}</dd><dt>模型 SHA256</dt><dd title="${esc(e.model_sha256||'')}">${esc(e.model_sha256||'-')}</dd></dl></details></section></div>`,true);
  };
  ...
  window.renderData429CardsLegacy_1=function(){const all=matchData429(),pages=Math.max(1,Math.ceil(all.length/state.data429PageSize));state.data429Page=Math.min(state.data429Page,pages);const st=(state.data429Page-1)*state.data429PageSize,rows=all.slice(st,st+state.data429PageSize);const g=document.getElementById('data429Grid');if(g)g.innerHTML=rows.map(dataCard429).join('')||'<div class="empty data426-empty">当前筛选条件下没有图片</div>';const c=document.getElementById('data429Count');if(c)c.textContent=`${all.length} 张`;const sc=document.getElementById('data429Selected');if(sc)sc.textContent=`已选 ${state.data424Selected.size} 张`;const p=document.getElementById('data429Pager');if(p)p.innerHTML=`<button class="btn mini" ${state.data429Page<=1?'disabled':''} onclick="dataPage429(-1)">上一页</button><span>${state.data429Page} / ${pages}</span><button class="btn mini" ${state.data429Page>=pages?'disabled':''} onclick="dataPage429(1)">下一页</button>`};
  ...
  const CLEAN_IMAGE_REVIEW_BATCH_429=60;
  ...
  function renderCleanImageReviewItems429(){
    ...
    const reviewState=state.cleanImageReview429||{},filtered=filteredCleanImageReviewItems429(),visible=Math.min(filtered.length,Math.max(CLEAN_IMAGE_REVIEW_BATCH_429,Number(reviewState.visible||CLEAN_IMAGE_REVIEW_BATCH_429)));
    holder.innerHTML=filtered.slice(0,visible).map(cleanImageReviewCardHtml429).join('')||'<div class="empty">当前筛选条件下没有需要复核的图片</div>';
    const count=document.getElementById('cleanImageReviewCount429');if(count)count.textContent=`显示 ${visible} / ${filtered.length} · 全部 ${(reviewState.items||[]).length}`;
    const more=document.getElementById('cleanImageReviewMore429');if(more){const remain=Math.max(0,filtered.length-visible);more.hidden=!remain;more.textContent=remain?`加载更多 ${Math.min(CLEAN_IMAGE_REVIEW_BATCH_429,remain)} 张`:'已显示全部'}
  ...
  window.setCleanImageReviewFilter429=function(filter){
    ...
    reviewState.filter=['all','suggested','failed'].includes(filter)?filter:'all';reviewState.visible=CLEAN_IMAGE_REVIEW_BATCH_429;renderCleanImageReviewItems429();
  };
  window.setCleanImageIssueFilter429=function(issueKey){
    const reviewState=state.cleanImageReview429;if(!reviewState)return;
    reviewState.issue=String(issueKey||'all');reviewState.visible=CLEAN_IMAGE_REVIEW_BATCH_429;renderCleanImageReviewItems429();
  };
  window.loadMoreCleanImageReview429=function(){
    const reviewState=state.cleanImageReview429;if(!reviewState)return;
    reviewState.visible=Math.max(CLEAN_IMAGE_REVIEW_BATCH_429,Number(reviewState.visible||0))+CLEAN_IMAGE_REVIEW_BATCH_429;renderCleanImageReviewItems429();
  };
  ...
  function renderCleanAuditItems429(){
    const holder=document.getElementById('cleanAuditItems429');if(!holder)return;
    const auditState=state.cleanAnnotationAudit429||{};
    holder.innerHTML=(auditState.items||[]).map(cleanAuditCardHtml429).join('')||'<div class="empty">本次正式标注未发现需要人工复核的问题</div>';
    const more=document.getElementById('cleanAuditMore429');
    if(more){more.hidden=!auditState.nextCursor;more.disabled=!!auditState.loading;more.textContent=auditState.loading?'加载中…':'加载更多标注问题'}
  }
  ...
  window.loadMoreCleanAudit429=async function(id){
    const auditState=state.cleanAnnotationAudit429;
    if(!auditState||String(auditState.taskId)!==String(id)||!auditState.nextCursor||auditState.loading)return;
    auditState.loading=true;renderCleanAuditItems429();
    try{
      const page=await api(`/api/v47/projects/${pid()}/clean-tasks/${id}/annotation-audit?cursor=${encodeURIComponent(auditState.nextCursor)}&limit=100`);
      const known=new Set((auditState.items||[]).map(item=>String(item.image_id)));
      for(const item of page.items||[]){if(!known.has(String(item.image_id))){auditState.items.push(item);known.add(String(item.image_id))}}
      auditState.nextCursor=page.next_cursor||null;
    }catch(error){toast(error.message||error)}
    finally{auditState.loading=false;renderCleanAuditItems429()}
  };
  window.cleanDetailCore429=async function(id){
    try{
      ...
      state.cleanImageReview429={taskId:String(id),items:reviewItems,filter:'all',issue:'all',visible:CLEAN_IMAGE_REVIEW_BATCH_429,issueOptions:imageIssueOptions};
      state.cleanAnnotationAudit429={taskId:String(id),items:[...(audit.items||[])],nextCursor:audit.next_cursor||null,loading:false};
...
(()=>{
  ...
  function mapImportProcessingProgress411(progress){const p=Math.max(0,Math.min(100,Number(progress)||0));return Math.round((3800+p*61)/10)/10}
  ...
  window.doUploadZipLegacy426_4=function(inp){
...
(()=>{
  ...
  window.loadCore412=async function({authoritative=false}={}){
    if(!state.project)await ensureWorkspace(); if(!state.project)return;
    const id=state.project.id;
    const snapshot=await api(`/api/v53/bootstrap/snapshot${authoritative?'?refresh=true':''}`);
    ...
    state.modelConfigs=snapshot.model_configs||[];state.__coreSnapshotGeneratedAt=Date.parse(snapshot.generated_at||'')||Date.now();
  ...
  window.refreshTestPageDataV3=async function(){
    const page=state.page;
    await extras412('测试发布');
  ...
  function datasetScope412(){const storageApi=window.PlatformCore?.storage,selected=state.materialSourceFilter61||'all',feedbackIds=state.iterationFeedbackCandidateIds63 instanceof Set?state.iterationFeedbackCandidateIds63:new Set();let rows=state.images||[];if(selected!=='all')rows=rows.filter(row=>storageApi?.sourceMatches?.(row,selected));if(state.iterationFeedbackOnly63&&feedbackIds.size)rows=rows.filter(row=>feedbackIds.has(String(row.id)));return rows}
  ...
  function materialCardNode412(row){
    const shell=document.createElement('div');shell.innerHTML=card412(row);const node=shell.firstElementChild;
    if(node)node.dataset.renderSignature=dataCardSignature412(row);
    return node;
  }
  function decorateMaterialSource412(card,row){
    if(!card||!row)return;
    const meta=card.querySelector('.data426-meta');if(!meta)return;
    const storageApi=window.PlatformCore?.storage,source=(state.storageSources61||[]).find(item=>item.id===(row.storage_source_id||'default_local'));
    const text=storageApi?.storageSourceLabel?.(source)||row.storage_type||'本地';
    let badge=meta.querySelector('.storage61-badge');
    if(!badge){badge=document.createElement('span');badge.className='storage61-badge';meta.appendChild(badge)}
    badge.textContent=text;
  }
  window.renderData412Cards=function(){
    const all=dataRows412(),pages=Math.max(1,Math.ceil(all.length/state.data412PageSize));state.data412Page=Math.min(Math.max(1,state.data412Page),pages);
    const st=(state.data412Page-1)*state.data412PageSize,rows=all.slice(st,st+state.data412PageSize),g=document.getElementById('data412Grid');
    if(g){
      const existing=new Map([...g.querySelectorAll('.data412-card[data-material-id]')].map(node=>[String(node.dataset.materialId),node]));
      const keep=new Set();
      if(!rows.length){
        if(!g.querySelector('.data426-empty'))g.innerHTML='<div class="empty data426-empty">当前没有素材</div>';
      }else{
        g.querySelector('.data426-empty')?.remove();
        rows.forEach((row,index)=>{
          const key=String(row.id),signature=dataCardSignature412(row);keep.add(key);
          let node=existing.get(key);
          if(!node){node=materialCardNode412(row)}
          else if(node.dataset.renderSignature!==signature){const next=materialCardNode412(row);node.replaceWith(next);node=next}
          if(!node)return;
          decorateMaterialSource412(node,row);
          const at=g.children[index];if(at!==node)g.insertBefore(node,at||null);
        });
        for(const [key,node] of existing)if(!keep.has(key))node.remove();
      }
    }
  ...
  window.remapImport412=(source,inputId)=>window.remapImport414(source,inputId);
  ...
  window.completeZipImportReview412=function(jobId){
    ...
    const syncReview=()=>{const r=document.getElementById('zip411Result');if(r&&!r.querySelector('.review412-btn'))r.insertAdjacentHTML('beforeend',extra)};
    syncReview();setTimeout(syncReview,30);
    if(!t.__autoReviewOpened){t.__autoReviewOpened=true;setTimeout(()=>window.showImportReview412?.(t.jobId),280)}
  };
  ...
  window.decorateTrainPickerBulk412=function(){const q=document.getElementById('tr429Q');if(q&&!document.getElementById('tr412SelectBar'))q.insertAdjacentHTML('afterend','<div id="tr412SelectBar" class="picker412-actions"><button class="btn mini" onclick="trainSelectAll412(\'all\')">全选当前</button><button class="btn mini" onclick="trainSelectAll412(\'invert\')">反选当前</button></div>')};
  window.openTrainPicker429=function openTrainPickerCanonical412(){const result=window.openTrainPickerCore429?.();setTimeout(()=>window.decorateTrainPickerBulk412?.(),30);return result};
  window.aiRefSelectCore412=function(mode){const q=(document.getElementById('ai429RefQ')?.value||'').toLowerCase(),labs=[...(state.ai429RefLabels||new Set())],rows=(state.images||[]).filter(x=>x.annotated&&(!q||String(x.filename||'').toLowerCase().includes(q))&&(!labs.length||labs.some(l=>(x.labels||[]).includes(l))));rows.forEach(x=>mode==='invert'?(state.ai429RefSelected.has(x.id)?state.ai429RefSelected.delete(x.id):state.ai429RefSelected.add(x.id)):state.ai429RefSelected.add(x.id));renderAiRefs429()};
  window.decorateAiReferenceBulk412=function(){const q=document.getElementById('ai429RefQ');if(q&&!document.getElementById('ai412SelectBar'))q.insertAdjacentHTML('afterend','<div id="ai412SelectBar" class="picker412-actions"><button class="btn mini" onclick="aiRefSelect412(\'all\')">全选当前</button><button class="btn mini" onclick="aiRefSelect412(\'invert\')">反选当前</button></div>')};

  // Cleaning review explicit all/invert.
  window.decorateCleanDetailSelection412=function(){const head=[...document.querySelectorAll('.clean429-card header')].find(x=>x.textContent.includes('逐图确认'));if(head&&!head.querySelector('.clean412-sel'))head.insertAdjacentHTML('beforeend','<div class="clean412-sel"><button class="btn mini" onclick="cleanSelect412(\'all\')">全选</button><button class="btn mini" onclick="cleanSelect412(\'invert\')">反选</button></div>')};
  window.cleanDetail429=async function cleanDetailCanonical429(id){const result=await window.cleanDetailCore429?.(id);setTimeout(()=>window.decorateCleanDetailSelection412?.(),20);return result};
...
(()=>{
  ...
  window.openAnnotationLegacy_2=async function(id){
  ...
  window.loadTrainingIterationBase414=async function(aid){
    ...
    showIterationBase414(aid);
    return state.iteration414[aid];
  };
  window.showIterationBaseCore414=function(aid){const dynamic=[...document.querySelectorAll('.v424-modal-layer')].at(-1),layer=dynamic||document.querySelector('#modal:not(.hidden)'),body=layer?.querySelector('.modal-body');if(!body)return;let box=body.querySelector('.iteration414');if(!box){box=document.createElement('div');box.className='iteration414';body.prepend(box)}const p=iterationPresentation414(state.iteration414[aid]);box.innerHTML=`<span>迭代起点</span><b>${esc(p.title)}</b>${p.detail?`<em>${esc(p.detail)}</em>`:''}`};
...
(()=>{
  ...
  window.selectDeployTarget=function selectDeployTargetCanonicalM4(kind){window.selectDeployTargetCoreV39?.(kind);setTimeout(()=>window.decorateDeployTargetM4?.(kind),0)};
})();
...
(()=>{
  ...
  function resourceStatusHtml416(rows,target){
    ...
    return rows.map(r=>{const ready=r.status==='ready'&&(r.targets||[]).includes(target),detail=r.message||(!ready?'请执行资源检测并确认目标能力':'可以创建真实转换任务'),chips=target==='rockchip'?(r.supported_chips||[]).map(x=>String(x||'').toUpperCase()).filter(Boolean):[],capability=chips.length?`支持芯片：${chips.join(' / ')}`:'';return `<div class="convert428-resource-status-row ${ready?'ready':'not-ready'}"><div><b>${esc(r.name||r.id||'未命名资源')}</b><span>${esc(r.mode==='agent'?'服务节点 Agent':r.mode==='remote'?'远程服务器':'本机')} · ${esc(statusLabel416(r.status))}</span></div><p>${esc(detail)}${capability?` · ${esc(capability)}`:''}</p></div>`}).join('');
  }
...
window.aiRefSelect412=function aiRefSelectCanonical412(mode){
  window.aiRefSelectCore412?.(mode);
  window.syncReferenceLabels417?.();
};
window.createAiLabel429=async function createAiLabelCanonical429(opts={}){
  try{
    ...
    toast(error?.message||error||'读取素材失败');
    return false;
  }
  const result=window.createAiLabelCore429?.(opts);
  window.decorateAiReferenceLabels417?.();
  setTimeout(()=>window.decorateAiReferenceBulk412?.(),30);
  return result;
};
window.createAiLabel427=window.createAiLabel429;

window.openNewConvert428=async function openNewConvertCanonical428(...args){
  const result=await window.openNewConvertCore416?.(...args);
  setTimeout(()=>window.decorateConvertResourceButton417?.(),20);
  return result;
};

window.refreshTrain429=function refreshTrainingCreateCanonical429(){
  window.refreshTrainCore429?.();
  window.decorateTrainingExperiment415?.();
  window.refreshProjectedTraining417?.();
  window.refreshTrainingCreateUi?.();
};
window.toggleTrainImage429=function toggleTrainingImageCanonical429(id){
  window.toggleTrainImageCore429?.(id);
  window.refreshProjectedTraining417?.();
};
window.showIterationBase414=function showTrainingIterationCanonical414(aid){
  window.showIterationBaseCore414?.(aid);
  window.decorateTrainingIteration417?.(aid);
};
...
(()=>{
  ...
  const uniqueUi=values=>[...new Set((values||[]).map(value=>String(value||'').trim()).filter(Boolean))];
  function splitPresentation(s=splitState()){
    const validation=Math.max(0,Math.min(100,Number(s.validation)||0));
    const experiment=s.mode==='random_test_from_training_pool'?Math.max(0,Math.min(100-validation,Number(s.experiment)||0)):0;
    return {training:Math.max(0,100-validation-experiment),validation,experiment};
  }
  function currentModelLabel(){
    const draft=state.trainingDraft||{},model=String(draft.config?.model||'').trim(),target=(state.targets||[]).find(row=>String(row.id)===String(document.getElementById('tr429Target')?.value||''));
    return (target?.base_models||[]).find(row=>String(row.value||row.label)===model)?.label||model.split(/[\\/]/).pop()||'-';
  }
  function currentDeviceLabel(){
    const id=String(state.trainingDraft?.resource?.device||'auto');
  ...
  function refreshTrainingCreateUi(){
    ...
    const card=root.closest('.modal-card');card?.classList.add('training-create-modal-card');card?.closest('.modal')?.classList.add('training-create-modal-layer');
  ...
  window.refreshTrainingCreateUi=refreshTrainingCreateUi;
  ...
  window.openTrainingCreateCanonical429=async function(aid){
    ...
    const applyDevices=(devices,{persist=true,paint=true}={})=>{state.trainingDevicesV3={...devices,loading:false};if(persist){state.trainingDevicesV3LoadedAt=Date.now();persistTrainingDeviceCacheV3(devices)}const options=devices?.options||[],autoOption=options.find(row=>String(row?.id||'')==='auto'&&row?.available!==false),requestedDevice=autoOption?'auto':String(devices?.recommended||'auto'),available=autoOption||options.find(row=>String(row?.id||'')===requestedDevice&&row?.available!==false)||options.find(row=>row?.available!==false),recommendedDevice=String(available?.id||requestedDevice);const deviceSelect=document.getElementById('trV3Device');if(deviceSelect)deviceSelect.value=recommendedDevice;window.TrainingDraftRuntime?.update?.({resource:{device:recommendedDevice}});if(paint)renderSplit()};
...
(()=>{
  ...
  function refreshAnnotationTasks60(){
    ...
    state.annotationTasks60RefreshPromise=api(`${taskApi()}?limit=50`).then(response=>{
      state.annotationTasks60=response.items||[];
      state.annotationTasks60LoadedAt=Date.now();
      return state.annotationTasks60;
  ...
  function renderAiLabelMapping60(){
    ...
    box.innerHTML=`<div class="storage61-import-mapping ai60-label-mapping">
      ...
      ${rows.map(row=>{
        const source=String(row.label||''),target=review.labelMapping.get(source)||source,display=window.PlatformCore.materials.labelDisplay(source,state.labels);
  ...
  window.closeAiReview60=()=>{
    ...
    if(review){review.closed=true;review.pageRequestEpoch=Number(review.pageRequestEpoch||0)+1}
    state.ai60Edit=null;
    closeModal();
  };
...
(()=>{
  ...
  window.benchPredictOne=async function(selectId,file,conf,batchMeta=null){
    ...
    task=await window.PlatformCore.taskPoller.waitForTaskTerminal({
      ...
      key:`deployment-test:${taskId}`,
      ownerPages:['质量中心'],
      delay:900,
      maxAttempts:700,
      load:()=>api(`/api/v62/projects/${pid()}/tasks/${taskId}`),
      onUpdate:current=>patchDeploymentTestProgress(current,taskId,model),
    });
...
/* v64: quality-center model detection workbench */
(()=>{
  const IMAGE_RE64=/\.(?:jpe?g|png|bmp|webp|gif|tiff?)$/i;
  const REVIEW_NAMES64={correct:'正确',missed:'漏检',false_positive:'误检',box_inaccurate:'框不准',wrong_class:'类别错误'};
  const escAttr64=value=>esc(String(value??''));

  function detectionModels64(){
  ...
  function defaultModelIndex64(role){
    const rows=state.testModels||[];
    if(!rows.length)return -1;
    if(role==='A'){
      const original=rows.findIndex(model=>String(model?.model_source||'')==='builtin');
  ...
  function modelOptions64(query='',selectedIndex=-1){
    ...
    source.forEach((model,index)=>{
      ...
      groups.get(group).push({model,index,text});
    });
    if(!groups.size)return '<option value="">没有匹配的模型</option>';
    return [...groups].map(([group,rows])=>`<optgroup label="${escAttr64(group)}">${rows.map(({index,text,model})=>`<option value="${index}" ${index===Number(selectedIndex)?'selected':''}>${esc(text)}${model?.is_current_version?' · 当前版本':''}</option>`).join('')}</optgroup>`).join('');
  }
  ...
  function batch64(){return state.benchBatch64||null}
  function resultName64(result,fallback){return result?.m?.label||result?.m?.model_name||result?.m?.path||fallback}

  window.filterBenchModels64=function(side){
    const select=document.getElementById(`benchModel${side}`),search=document.getElementById(`benchModelSearch${side}`);if(!select)return;
    const selected=Number(select.value);select.innerHTML=modelOptions64(search?.value||'',Number.isFinite(selected)?selected:defaultModelIndex64(side));
    if([...select.options].some(option=>Number(option.value)===selected))select.value=String(selected);
  };

  window.addBenchFiles64=function(input){
    const incoming=[...(input?.files||[])],valid=incoming.filter(validImage64),invalid=incoming.length-valid.length,current=state.benchFiles64||[],seen=new Set(current.map(fileKey64));
    for(const file of valid){const key=fileKey64(file);if(!seen.has(key)){current.push(file);seen.add(key)}}
    state.benchFiles64=current;
    if(input)input.value='';
    renderBenchFileQueue64();
    if(invalid)toast(`已忽略 ${invalid} 个非图片文件`);
  };
  window.clearBenchFiles64=function(){
    for(const row of state.benchBatch64?.results||[]){if(row.previewUrl)URL.revokeObjectURL(row.previewUrl)}
    state.benchFiles64=[];state.benchBatch64=null;renderBenchFileQueue64();renderBenchBatch64();
  };
  window.removeBenchFile64=function(key){
    state.benchFiles64=(state.benchFiles64||[]).filter(file=>fileKey64(file)!==String(key));
    renderBenchFileQueue64();
  };
  window.previewBenchFile64=function(index=0){
    const file=(state.benchFiles64||[])[Number(index)];if(!file)return;
    const box=document.getElementById('benchPreview64');if(!box)return;
    const url=URL.createObjectURL(file),previous=box.dataset.objectUrl;if(previous)URL.revokeObjectURL(previous);
    box.dataset.objectUrl=url;box.innerHTML=`<img src="${url}" alt=""><span>${esc(file.webkitRelativePath||file.name)}</span>`;
  };
  window.renderBenchFileQueue64=function(){
    const files=state.benchFiles64||[],count=document.getElementById('benchFileCount64'),list=document.getElementById('benchFileList64');
    if(count)count.textContent=files.length?`已选择 ${files.length} 张图片`:'尚未选择图片';
    if(list)list.innerHTML=files.slice(0,80).map((file,index)=>`<button type="button" class="bench64-file-chip" onclick="previewBenchFile64(${index})"><span>${esc(file.webkitRelativePath||file.name)}</span><em>${(Number(file.size||0)/1024/1024).toFixed(2)} MB</em><i onclick="event.stopPropagation();removeBenchFile64('${escAttr64(fileKey64(file))}')">×</i></button>`).join('')+(files.length>80?`<div class="item-sub">另有 ${files.length-80} 张图片已加入本次检测。</div>`:'');
    if(files.length)window.previewBenchFile64(0);else{const box=document.getElementById('benchPreview64');if(box)box.innerHTML='<span>选择图片后在这里预览</span>'}
  };

  function summary64(){
    const run=batch64(),box=document.getElementById('benchBatchSummary64');if(!box)return;
    if(!run){box.innerHTML='<div class="bench64-empty"><b>暂无本次检测结果</b><span>选择图片后开始检测，批量结果会保留在当前页面。</span></div>';return}
    const percent=run.total?Math.round(run.completed/run.total*100):0;
    box.innerHTML=`<div class="bench64-summary-grid"><div><span>本次图片</span><b>${run.total}</b></div><div><span>已完成</span><b>${run.completed}</b></div><div><span>成功</span><b>${run.success}</b></div><div><span>失败</span><b>${run.failed}</b></div><div><span>进度</span><b>${percent}%</b></div></div><div class="bench64-runbar"><i style="transform:scaleX(${(percent/100).toFixed(4)})"></i></div><div class="row between"><span class="item-sub">${run.running?'正在按图片顺序执行真实模型推理；不会用前端模拟结果。':run.cancelled?'本次检测已停止。':'本次检测已结束。'}</span>${run.running?'<button class="btn danger small" onclick="stopBenchBatch64()">停止检测</button>':''}</div>`;
  }
  function rowHtml64(row,index){
    const aCount=Number(row.a?.r?.detections?.length||0),bCount=Number(row.b?.r?.detections?.length||0),review=row.review?REVIEW_NAMES64[row.review]||row.review:'未核验';
    const status=row.status==='done'?'完成':row.status==='failed'?'失败':row.status==='running'?'检测中':'等待';
    return `<div class="bench64-result-row ${row.status}" data-bench-result-id="${escAttr64(row.id)}"><div class="bench64-result-main"><b>${esc(row.file.webkitRelativePath||row.file.name)}</b><span>${status}${row.error?' · '+esc(row.error):''}</span></div><div><span>A 模型</span><b>${row.a?aCount+' 个目标':'—'}</b></div><div><span>B 模型</span><b>${row.b?bCount+' 个目标':'—'}</b></div><div><span>人工核验</span><b>${esc(review)}</b></div><button class="btn mini" ${row.status==='done'?'':'disabled'} onclick="openBenchResult64(${index})">查看详情</button></div>`;
  }
  window.renderBenchBatch64=function(){
    summary64();
    const run=batch64(),list=document.getElementById('benchBatchList64');
    if(!list)return;
    if(!run){list.innerHTML='';return}
    list.innerHTML=run.results.map(rowHtml64).join('');
  };
  function patchBenchRow64(index){
    summary64();
    const run=batch64(),list=document.getElementById('benchBatchList64'),row=run?.results?.[index];if(!list||!row)return;
    let current=list.querySelector(`[data-bench-result-id="${CSS.escape(String(row.id))}"]`);
    const shell=document.createElement('div');shell.innerHTML=rowHtml64(row,index);const next=shell.firstElementChild;
    if(current&&next)current.replaceWith(next);else if(next)list.appendChild(next);
  }
  window.stopBenchBatch64=function(){if(state.benchBatch64?.running)state.benchBatch64.cancelled=true};

  window.runBenchBatch64=async function(){
    if(state.benchBatch64?.running)return toast('本次检测仍在进行中');
    const files=[...(state.benchFiles64||[])];if(!files.length)return toast('请先选择一张或多张图片');
    const mode=document.getElementById('benchMode64')?.value||'compare',conf=Math.max(0.001,Math.min(1,Number(document.getElementById('benchConf')?.value||.25)));
