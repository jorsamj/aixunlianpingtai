...
(()=>{
  ...
  window.openStorageRescan61=async function(sourceId){
    ...
    const terminal=status=>['SUCCEEDED','PARTIAL_SUCCESS','POST_PROCESSING_FAILED','FAILED','CANCELLED','BLOCKED_BY_ENVIRONMENT','BLOCKED_BY_HARDWARE'].includes(status);
  ...
  window.installServerMaterialImport61=function(){
    ...
    function renderImportTask(task){
      ...
      const labels=(state.labels||[]).filter(label=>label.status!=='disabled'),mappingHtml=view.canConfirm&&classes.length?`<div class="storage61-import-mapping"><b>外部类别 → 平台标签</b><p>建议映射仅供参考；每个外部类别都需要明确选择。</p>${serverApi().externalClassMappingHtml(classes,labels,esc)}</div>`:'';
      ...
      const updateConfirm=()=>{const button=document.getElementById('si61Confirm');if(button)button.disabled=Object.keys(quality?.issues||{}).length>0&&!document.getElementById('si61AcceptQuality')?.checked};
      status.querySelectorAll('[data-class-action]').forEach(select=>select.addEventListener('change',()=>{const row=select.closest('[data-import-class]'),action=select.value;row.querySelector('[data-target-label]').hidden=action!=='map';row.querySelector('[data-new-code]').hidden=action!=='create';row.querySelector('[data-new-display]').hidden=action!=='create'}));
      status.querySelectorAll('[data-sample-class]').forEach(button=>button.addEventListener('click',async()=>{try{const data=await importRequest(`${taskUrl(taskId)}/classes/${encodeURIComponent(button.dataset.sampleClass)}/samples?limit=6`);modal(`类别样例 · ${button.closest('[data-import-class]')?.dataset.importName||button.dataset.sampleClass}`,`<div class="storage61-import-samples">${serverApi().sampleGalleryHtml(data.items||[],esc)}</div><div class="row end"><button class="btn" onclick="closeModal()">关闭</button></div>`,true)}catch(error){toast(error.message||error)}}));
    ...
    window.confirmStorageImport61=async function(taskId){
      ...
      try{
        const rows=[...document.querySelectorAll('[data-import-class]')].map(row=>({classId:row.dataset.importClass,name:row.dataset.importName,action:row.querySelector('[data-class-action]')?.value,targetLabelId:row.querySelector('[data-target-label]')?.value,code:row.querySelector('[data-new-code]')?.value,displayName:row.querySelector('[data-new-display]')?.value}));
...
(function(){
  ...
  function status423(s){const names={queued:'排队中',running:'运行中',done:'已完成',finished:'已完成',completed:'已完成',post_processing_failed:'训练完成 · 后处理失败',failed:'失败',stopped:'已停止',ready:'正常',unchecked:'未检测',disabled:'已停用'};const cls=['done','finished','completed','ready'].includes(s)?'ok':['failed','post_processing_failed'].includes(s)?'err':'warn';return `<span class="pill ${cls}">${esc(names[s]||s||'-')}</span>`}
...
(function(){
  ...
  function dt423(v){return v?String(v).replace('T',' ').slice(0,19):'-'}
  function pct423(v){if(v==null||Number.isNaN(Number(v)))return '-';const n=Number(v);return (n>1?n:n*100).toFixed(1)+'%'}
  function status423(s){const names={queued:'排队中',running:'训练中',done:'已完成',finished:'已完成',completed:'已完成',post_processing_failed:'训练完成 · 后处理失败',failed:'失败',stopped:'已停止',ready:'正常',unchecked:'未检测'};const cls=['done','finished','completed','ready'].includes(s)?'ok':['failed','post_processing_failed'].includes(s)?'err':'warn';return `<span class=\"pill ${cls}\">${esc(names[s]||s||'-')}</span>`}
...
(function(){
  ...
  window.retryPostProcessing424=async id=>{try{await api(`/api/v63/projects/${pid()}/training-tasks/${id}/retry-post-processing`,{method:'POST'});await refreshTrain423();toast('后处理重试已排队，将复用现有 best.pt')}catch(e){toast(e.message||e)}};
...
(()=>{
  ...
  const labelState=()=>state.trainLabelsV4||(state.trainLabelsV4={key:'',loading:false,loaded:false,labels:[],selected:new Set(),error:''});
  ...
  function renderSplit(){
    ...
    const ls=labelState(),labelHtml=ls.loading?'<div class="empty">正在统计本次素材标签…</div>':ls.error?`<div class="empty">${esc(ls.error)}</div>`:(ls.labels.map(row=>`<label class="check train-v4-label"><input type="checkbox" ${ls.selected.has(String(row.label_id))?'checked':''} onchange="toggleTrainingLabelV4('${esc(String(row.label_id))}',this.checked)"><span><b>${esc(row.display_name||row.code)} · ${esc(row.code)}</b><em>有框图片 ${row.positive_images} · ${row.boxes} 框 · 合法负样本 ${row.legal_negative_images} · GT不完整 ${row.gt_incomplete_images}</em></span></label>`).join('')||'<div class="empty">所选素材没有可训练标签</div>');
    panel.querySelector('.train-v3-note')?.insertAdjacentHTML('beforebegin',`<section class="train-v4-labels"><header><b>3. 本次素材可训练标签</b><span>只显示所选素材真实涉及的标签，任务创建后冻结</span></header>${labelHtml}</section>`);
    ...
    if(start)start.disabled=s.train.size<2||ls.loading||!ls.selected.size||(!!algorithm?.versions?.length&&(!base?.version_name||!!base.error));
    refreshTrainingLabelsV4();
  }
  async function refreshTrainingLabelsV4(){
    const s=splitState(),train=[...s.train].sort(),test=s.mode==='independent_test_set'?[...s.test].sort():[],key=JSON.stringify([train,test]),ls=labelState();
    if(!train.length){ls.key=key;ls.loading=false;ls.loaded=true;ls.labels=[];ls.selected.clear();ls.error='';return}
    if(ls.key===key&&(ls.loading||ls.loaded))return;
    ls.key=key;ls.loading=true;ls.loaded=false;ls.error='';ls.labels=[];ls.selected.clear();
    try{const report=await api(`/api/v63/projects/${pid()}/training-labels`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({train_image_ids:train,test_image_ids:test})});if(ls.key!==key)return;ls.labels=report.available_training_labels||[];ls.selected=new Set(ls.labels.filter(row=>Number(row.positive_images)>0).map(row=>String(row.label_id)))}catch(error){if(ls.key===key)ls.error=String(error.message||error)}finally{if(ls.key===key){ls.loading=false;ls.loaded=true;renderSplit()}}
  }
  window.toggleTrainingLabelV4=(labelId,on)=>{const ls=labelState();on?ls.selected.add(String(labelId)):ls.selected.delete(String(labelId));renderSplit()};
  ...
  window.startAlgorithmTraining429=async function(aid){
    ...
    const result=await previousStart?.(aid);state.train428Config=state.train428Config||{};state.train428Config.device=state.trainingDevicesV3.recommended||'auto';state.trainSplitV3=freshState();state.trainLabelsV4={key:'',loading:false,loaded:false,labels:[],selected:new Set(),error:''};state.train429Selected=state.trainSplitV3.train;[40,140,340,650].forEach(delay=>setTimeout(renderSplit,delay));return result
  };
  window.submitTrain429=async function(){
  ...
  window.showTrainLog423=async function(id){
    try{
      ...
      const e=job.device_evidence||{},r=job.resolved_resources||{},m=job.runtime_metrics||{},args=job.actual_train_params||{},diagnosis=m.diagnostic||{},requested=job.requested_config||{},effective=job.effective_config||{},actual=job.actual_config||{};
