...
(()=>{
  ...
  window.cleanDetailCore429=async function(id){
    try{
      const r=await api(`/api/v47/projects/${pid()}/clean-tasks/${id}/result`),t=r.task||{},res=r.result||{},items=res.items||[],rules=res.rules||t.request_payload||{},stats={};
      ...
      modal('清洗任务详情',`<div class="clean429"><section class="clean429-head"><div><span>任务结果 · ${esc((window.PlatformCore?.cleaning?.cleanScopeChoices?.({})||[]).find(x=>x.value===scope)?.label||scope)}</span><h2>${esc(t.name||'自动清洗')}</h2><p>图片质量与正式标注状态分开呈现；清洗不会自动删除图片，也不会修改 Ground Truth。</p></div></section><div class="review427-summary"><div><span>扫描数量</span><b>${total}</b></div><div><span>通过</span><b>${passed}</b></div><div><span>需复核</span><b>${Math.max(0,reviewItems.length-failed.length)}</b></div><div><span>失败</span><b>${failed.length}</b></div></div><section class="clean429-card"><header><b>本次基础清洗</b></header><div class="clean429-rules">${ruleNames429(rules).map(x=>`<span>${esc(x)}</span>`).join('')||'<span>未记录规则</span>'}</div></section><section class="clean429-card"><header><b>图片质量问题分布</b></header><div class="clean429-stats">${Object.entries(stats).map(([k,v])=>`<div><span>${esc(k)}</span><b>${v} 张</b><em>${total?((v/total)*100).toFixed(1):0}%</em></div>`).join('')||'<div class="empty">没有发现需要复核的图片质量问题</div>'}</div></section><section class="clean429-card"><header><b>图片质量 · 逐图复核</b><span>仅展示需复核/失败项；标注状态与 provenance 来自后端正式真相。</span></header><div class="review427-grid">${reviewItems.map(x=>`<article class="review427-card ${x.suggest_delete?'flagged':''}"><img src="${x.url||((state.images||[]).find(i=>i.id===x.image_id)?.url)||''}" loading="lazy"><section><b>${esc(x.filename||'')}</b><div class="issues"><span>${esc(cleanAnnotationStateText429(x.annotation_state))}</span><span>${esc(cleanAnnotationProvenanceText429(x.annotation_provenance))}</span>${(x.issues||[]).map(y=>`<span>${esc(y.name)} · ${esc(y.detail)}</span>`).join('')}${failed.includes(x)?'<span class="err">扫描失败</span>':''}</div>${x.suggest_delete?`<label><input type="checkbox" ${state.v427CleanConfirm.has(String(x.image_id))?'checked':''} onchange="toggleCleanItem427('${x.image_id}',this.checked)"> 建议剔除（仍需人工确认）</label>`:''}</section></article>`).join('')||'<div class="empty">本次图片质量全部通过，无需逐图复核</div>'}</div></section><div class="row end"><button class="btn" onclick="closeModal()">关闭</button>${t.status==='awaiting_confirmation'?`<button class="btn primary" onclick="confirmClean429('${id}')">确认清洗结果</button>`:''}</div></div>`,true)
    }catch(e){toast(e.message||e)}
  };
