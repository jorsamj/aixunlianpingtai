...
(function(){
  ...
  window.submitRknnHardwareVerify=async function(id){const file=document.getElementById('rknnVerifyFile')?.files?.[0];if(!file)return toast('请选择测试图片');const btn=document.getElementById('rknnVerifySubmit'),status=document.getElementById('rknnVerifyState');if(btn)btn.disabled=true;try{const form=new FormData();form.append('file',file);let task=await api(`/api/v39/projects/${pid()}/deploy/jobs/${id}/hardware-tests`,{method:'POST',body:form});const taskId=task.id||task.task_id;if(!taskId)throw new Error('板端验证任务创建失败');let attempt=0;while(attempt++<700){const upper=String(task.status||'').toUpperCase(),active=window.PlatformCore?.taskPoller?.isTaskActive?.(upper)??['QUEUED','WAITING_RESOURCE','PREPARING','RUNNING','PAUSING','PAUSED','RESUMING','CANCEL_REQUESTED','RETRYING'].includes(upper);patchRknnHardwareVerifyProgress(status,task,taskId);if(!active)break;await new Promise(resolve=>setTimeout(resolve,900));task=await api(`/api/v62/projects/${pid()}/tasks/${taskId}`)}if(String(task.status||'').toUpperCase()!=='SUCCEEDED')throw new Error(task.error||`板端验证未通过：${task.status}`);await loadDeployData(true);renderDeployJobsOnly();closeModal();toast('RKNN 已完成瑞芯微实机 Runtime 验证')}catch(e){if(status){status.className='alert err';status.textContent=e.message||String(e)}toast(e.message||e)}finally{if(btn)btn.disabled=false}};
