...
(function(){
  ...
  async function pollDeployJobs(){
    ...
    if((state.deployJobs||[]).some(j=>['queued','waiting_resource','running'].includes(j.status))){clearTimeout(window.__deployPollV39);window.__deployPollV39=setTimeout(pollDeployJobs,1800)}
  }
