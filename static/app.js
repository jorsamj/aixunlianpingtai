...
(()=>{
  ...
  window.startAlgorithmTraining429=async function(aid){
    ...
    const algorithmId=String(aid||'');
    try{
      const latest=await api(`/api/v12/projects/${pid()}/algorithms/${encodeURIComponent(algorithmId)}`);
      const fresh=latest?.algorithm;
      if(!fresh)throw new Error('训练算法不存在或已被删除');
      ...
      const readiness=window.ExternalAlgorithmPlatformRuntime?.trainingReadiness?.(algorithmId);
      if(readiness&&readiness.ready===false){toast(readiness.message||'该算法当前无法创建训练任务');return false}
