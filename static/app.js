...
(()=>{
  ...
  window.confirmIterationAction429=async function(aid,vid){
    ...
    try{
      ...
      const confirmed=result.action||{};
      v.confirmed_iteration_action=confirmed;
      if(action==='continue_training'){
        ...
        setTimeout(()=>{
          ...
          if(root&&!root.querySelector('.iteration-action429')){
            root.insertAdjacentHTML('afterbegin',`<div class="alert ok iteration-action429"><b>已确认继续训练</b><span>基于 ${esc(v.version_name||vid)} 的评测决策进入训练草稿；只有点击“开始训练”后才会创建真实训练任务。</span></div>`);
          }
        },30);
        return;
      }
      if(action==='supplement_data'){
        ...
        toast(labels.length?`已生成补数据草稿，优先关注：${labels.join('、')}`:'已生成补数据草稿，请确认补充素材范围');
        return;
      }
      if(action==='business_validation'){
        state.businessValidationEntry=confirmed.validation_entry||{};
        closeModal();setPage('测试发布');toast('已确认进入业务验证，请继续选择验证方式');
        return;
      }
      toast('已记录人工处理动作');
