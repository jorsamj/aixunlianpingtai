...
window.installUsability417=function(){
  ...
  function refreshProjected417(){const root=document.querySelector('.train429-create'),note=root?.querySelector('.train429-split-summary b'),p=projected417();if(note)note.textContent=`预计训练 ${p.train} 张 / 试验 ${p.experiment} 张`;const input=document.getElementById('tr429ExperimentPercent');if(input&&!input.dataset.bound417){input.dataset.bound417='1';input.addEventListener('input',refreshProjected417)}const aid=state.train428AlgorithmId,a=(state.algorithms||[]).find(x=>String(x.id)===String(aid)),needsBase=!!a?.versions?.length,base=state.iteration414?.[aid],start=root?.querySelector('.train428-footer .btn.primary');if(start)start.disabled=(state.train429Selected?.size||0)<2||(needsBase&&(!base?.version_name||!!base.error))}
  ...
  window.startAlgorithmTraining429=async function(aid){const a=(state.algorithms||[]).find(x=>String(x.id)===String(aid)),latest=a?.versions?.[0],pending=baseStartTraining417?.(aid);state.train429Selected=new Set();if(latest)state.iteration414[aid]={version_id:latest.id,version_name:latest.version_name,model_name:latest.model_name,path:latest.stored_path||latest.path||''};window.refreshTrain429?.();if(latest)showIterationBase414(aid);[30,180].forEach(delay=>setTimeout(()=>{if(latest&&!state.iteration414?.[aid]?.version_name)state.iteration414[aid]={version_id:latest.id,version_name:latest.version_name,model_name:latest.model_name,path:latest.stored_path||latest.path||''};if(latest)showIterationBase414(aid);refreshProjected417()},delay));const result=await pending;refreshProjected417();return result};
  ...
  render=function(){const result=baseRender417?.(),badge=document.getElementById('versionBadge'),footer=document.querySelector('.nav-footer b');if(badge)badge.textContent='v42.21.0';if(footer)footer.textContent='v42.21.0';return result};
  [120,600,1600].forEach(delay=>setTimeout(()=>{const badge=document.getElementById('versionBadge'),footer=document.querySelector('.nav-footer b');if(badge)badge.textContent='v42.21.0';if(footer)footer.textContent='v42.21.0'},delay));
...
(function(){
  ...
  window.renderVideo424=async function(){clearTimeout(state.video424Timer);await loadVideo424();document.getElementById('view').innerHTML=`<section class="taskpage424"><div class="taskpage424-head"><div></div><button class="btn primary" onclick="createVideoTask424()">＋ 创建切帧任务</button></div><section class="panel"><div class="table-wrap"><table class="table task424-table"><thead><tr><th>视频</th><th>切帧方式</th><th>素材归属</th><th>状态</th><th>进度</th><th>当前帧</th><th>创建/完成</th><th>操作</th></tr></thead><tbody id="video424Rows">${state.video424.map(videoTaskRow424).join('')||'<tr data-video-empty><td colspan="8">暂无切帧任务</td></tr>'}</tbody></table></div></section></section>`;if(state.video424.some(t=>videoCore424()?.isActiveVideoTask(t)))state.video424Timer=setTimeout(refreshVideo424Delta,2000)};
...
(()=>{
  ...
  window.submitTrain428=async function(){const a=(state.algorithms||[]).find(x=>x.id===state.train428AlgorithmId),t=selectedTarget428(),ta=targetAlgorithm428(),c=state.train428Config||baseCfg428(),trainIds=selectedIds428('train'),valIds=selectedIds428('val'),priority=readTrainingPriority428('tr428Priority');if(!a||!t||!ta)return toast('训练资源或训练算法不可用');if(!trainIds.length)return toast('请至少选择一张已标注训练集图片');if(!valIds.length)return toast('请至少选择一张已标注试验集图片');if(priority==null)return;const payload={framework:t.framework==='paddle'?'paddle':'ultralytics',target:t.type==='server'?'remote':'local',server_id:t.server_id,algorithm:ta.key||'',algorithm_asset_id:a.id,model:c.model||ta.base_model||'',epochs:c.epochs,imgsz:c.imgsz,batch:c.batch,device:c.device||'cpu',dataset_id:'__all__',include_empty:false,patience:c.patience,workers:c.workers,optimizer:c.optimizer,lr0:c.lr0,lrf:c.lrf,momentum:c.momentum,weight_decay:c.weight_decay,warmup_epochs:c.warmup_epochs,close_mosaic:c.close_mosaic,mosaic:c.mosaic,mixup:c.mixup,hsv_h:c.hsv_h,hsv_s:c.hsv_s,hsv_v:c.hsv_v,degrees:c.degrees,translate:c.translate,scale:c.scale,shear:c.shear,perspective:c.perspective,flipud:c.flipud,fliplr:c.fliplr,cache:c.cache,pretrained:c.pretrained,amp:c.amp,rect:c.rect,cos_lr:c.cos_lr,freeze:c.freeze,multi_scale:c.multi_scale,save_period:c.save_period,seed:c.seed,deterministic:c.deterministic,train_image_ids:trainIds,val_image_ids:valIds,val_max_samples:c.val_max_samples,eval_interval:c.eval_interval,eval_metric:c.eval_metric,continue_threshold:c.continue_threshold,stop_threshold:c.stop_threshold,auto_supplement:false,supplement_count:0,queue_priority:priority,auto_convert_targets:c.auto_convert_targets||[],ai_intervention_enabled:false,ai_intervention_epochs:[],ai_model_config_id:''};try{const r=await api(`/api/v12/projects/${pid()}/train/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});closeModal();await loadRelated();state.alg428Expanded[a.id]=true;renderAlgorithms423();toast(`${r.job?.status==='running'?'训练已开始':'训练已进入队列'} · ${r.job?.id||''}`)}catch(e){toast(e.message||e)}};
...
(()=>{
  ...
  async function extras412(){if(!state.project)return;const id=state.project.id;const q=p=>p.catch(()=>null);const [info,opts,pending,testModels,infer,rec,local,modelConfigs]=await Promise.all([
    q(api(`/api/projects/${id}`)),q(api(`/api/training_options?project_id=${id}`)),q(api(`/api/v12/projects/${id}/publish/pending`)),q(api(`/api/v12/projects/${id}/test_models`)),q(api('/api/v16/inference_envs')),q(api('/api/system/recommendation')),q(api('/api/local_models')),q(api('/api/v35/model-configs'))
  ]);if(info){state.project=info.project||state.project;state.jobs=info.jobs||[];state.models=info.models||[]}state.targets=opts?.targets||[];state.pending=pending?.items||[];state.testModels=testModels?.items||[];state.inferenceEnvs=infer?.items||[];state.rec=rec;state.localModels=local?.items||[];state.modelConfigs=modelConfigs?.items||state.modelConfigs||[];try{renderSummary()}catch(e){};pollAnnotationIndex412();
  }
...
window.__exactMaterialTrainingV3=true;
/* Legacy dataset split UI. Retained for load-order compatibility and superseded below. */
(()=>{
  if(window.__exactMaterialTrainingV3)return;
  ...
  const splitState=()=>state.trainSplitV2||(state.trainSplitV2={mode:'random_test_from_training_pool',train:new Set(),test:new Set(),experiment:20,validation:20});
  function datasetRows(){return(state.datasets||[]).map(d=>({id:String(d.id),name:d.name||d.id,count:Number(d.images||d.image_count||0)}))}
  function checks(role){const s=splitState(),selected=s[role];return datasetRows().map(d=>`<label class="check"><input type="checkbox" ${selected.has(d.id)?'checked':''} onchange="toggleTrainDatasetV2('${role}','${d.id}',this.checked)"> ${esc(d.name)}（${d.count} 张）</label>`).join('')||'<span class="empty">暂无数据集</span>'}
  function renderSplit(){const root=document.querySelector('.train429-create'),panel=root?.querySelectorAll('.train428-panel')?.[1];if(!panel)return;const s=splitState(),random=s.mode==='random_test_from_training_pool';panel.innerHTML=`<header><b>2. 训练 / 验证 / 试验数据</b></header><div class="form two"><div class="field full"><label>试验集来源</label><div class="row"><label class="check"><input type="radio" name="trV2Mode" value="random_test_from_training_pool" ${random?'checked':''} onchange="setTrainSplitModeV2(this.value)"> 从训练数据随机抽取</label><label class="check"><input type="radio" name="trV2Mode" value="independent_test_set" ${!random?'checked':''} onchange="setTrainSplitModeV2(this.value)"> 单独选择试验数据集</label></div></div><div class="field"><label>训练数据集</label><div class="train-v2-datasets">${checks('train')}</div><small>每次创建默认不选择，必须由用户确认。</small></div>${random?`<div class="field"><label>随机试验集比例</label><input id="trV2Experiment" class="input" type="number" min="0.1" max="99.9" step="0.1" value="${s.experiment}" oninput="state.trainSplitV2.experiment=Number(this.value)"><small>先抽试验集，再从剩余数据抽验证集。</small></div>`:`<div class="field"><label>独立试验数据集</label><div class="train-v2-datasets">${checks('test')}</div><small>不能与训练数据集重复。</small></div>`}<div class="field"><label>验证集比例（进阶默认）</label><input id="trV2Validation" class="input" type="number" min="0.1" max="99.9" step="0.1" value="${s.validation}" oninput="state.trainSplitV2.validation=Number(this.value)"><small>验证集只用于训练过程；试验集仅作最终评估。</small></div></div>`}
  window.toggleTrainDatasetV2=function(role,id,on){const s=splitState();on?s[role].add(id):s[role].delete(id);if(on){const other=role==='train'?'test':'train';s[other].delete(id)}renderSplit()};
  window.setTrainSplitModeV2=function(mode){splitState().mode=mode;renderSplit()};
  window.startAlgorithmTraining429=async function(aid){state.trainSplitV2={mode:'random_test_from_training_pool',train:new Set(),test:new Set(),experiment:20,validation:20};const result=await previousStart?.(aid);[20,100,300].forEach(delay=>setTimeout(renderSplit,delay));return result};
  window.submitTrain429=async function(){
    const a=(state.algorithms||[]).find(x=>x.id===state.train428AlgorithmId),target=(state.targets||[]).find(x=>x.id===document.getElementById('tr429Target')?.value),algorithm=(target?.algorithms||[]).find(x=>x.key===document.getElementById('tr429Alg')?.value)||(target?.algorithms||[])[0],c=state.train428Config||{},s=splitState(),priority=readTrainingPriority428('tr429Priority');
...
(()=>{
  ...
  function renderPicker(){
    ...
    grid.innerHTML=current.map(row=>{const id=imageId(row),on=p.selected.has(id);return `<label class="train-v3-card ${on?'on':''}"><input type="checkbox" ${on?'checked':''} onchange="toggleTrainMaterialV3('${id}',this.checked)"><img src="${row.url}" loading="lazy" decoding="async"><b title="${esc(row.filename||'')}">${esc(row.filename||'')}</b><span>${esc((row.labels||[]).map(labelText).join('、')||'无标签')}</span></label>`}).join('')||'<div class="empty">没有符合筛选条件的可训练图片</div>';
  ...
  window.toggleTrainMaterialV3=(id,on)=>{const p=state.trainMaterialPickerV3;if(!p)return;on?p.selected.add(String(id)):p.selected.delete(String(id));renderPicker()};
  ...
  window.startAlgorithmTraining429=async function(aid){const result=await previousStart?.(aid);state.trainSplitV3=freshState();state.train429Selected=state.trainSplitV3.train;[40,140,340,650].forEach(delay=>setTimeout(renderSplit,delay));return result};
  window.submitTrain429=async function(){
