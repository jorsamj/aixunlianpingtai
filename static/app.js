...
window.installUsability417=function(){
  ...
  const baseToggleReference417=window.toggleRef429;
  window.toggleRef429=function(id){baseToggleReference417?.(id);syncReferenceLabels417()};
  ...
  function projected417(){const total=state.train429Selected?.size||0,pct=Math.max(1,Math.min(99,Number(document.getElementById('tr429ExperimentPercent')?.value||state.train429ExperimentPercent||20))),fn=window.PlatformCore?.training?.projectedRandomSplit;if(fn)return fn(total,pct);if(total<2)return{train:total,experiment:0};const experiment=Math.max(1,Math.min(total-1,Math.round(total*pct/100)));return{train:total-experiment,experiment}}
  function refreshProjected417(){const root=document.querySelector('.train429-create'),note=root?.querySelector('.train429-split-summary b'),p=projected417();if(note)note.textContent=`预计训练 ${p.train} 张 / 试验 ${p.experiment} 张`;const input=document.getElementById('tr429ExperimentPercent');if(input&&!input.dataset.bound417){input.dataset.bound417='1';input.addEventListener('input',refreshProjected417)}}
  ...
  window.showIterationBase414=function(aid){baseShowIteration417?.(aid);const base=state.iteration414?.[aid],root=document.querySelector('.train429-create'),select=root?.querySelector('#tr429Alg'),field=select?.closest('.field'),model=root?.querySelector('#tr429Model'),start=[...root?.querySelectorAll('.train428-footer .btn.primary')||[]].at(-1);if(start)start.disabled=!base||!!base.error;if(base?.version_name){if(field){select.style.display='none';let locked=field.querySelector('.iteration417-engine');if(!locked){locked=document.createElement('div');locked.className='input iteration417-engine';field.appendChild(locked)}field.querySelector('label').textContent='训练引擎（迭代任务锁定）';locked.textContent='Ultralytics Detect'}if(model)model.textContent=`${base.version_name} · ${base.model_name||'最新模型成果'}`}refreshProjected417()};
  const baseStartTraining417=window.startAlgorithmTraining429;
  window.startAlgorithmTraining429=async function(aid){const result=await baseStartTraining417?.(aid);setTimeout(()=>{const root=document.querySelector('.train429-create'),start=[...root?.querySelectorAll('.train428-footer .btn.primary')||[]].at(-1);if(start&&!state.iteration414?.[aid])start.disabled=true;refreshProjected417()},30);return result};
  const baseOpenSettings417=window.openTrainSettings429;
  window.openTrainSettings429=function(){const result=baseOpenSettings417?.();setTimeout(()=>{const base=state.iteration414?.[state.train428AlgorithmId],model=document.getElementById('ts428Model');if(base?.version_name&&model){const text=`${base.version_name} · ${base.model_name||'最新模型成果'}`;if(model.tagName==='SELECT'){model.innerHTML=`<option value="${esc(base.path||base.model_name||'latest')}">${esc(text)}</option>`;model.value=base.path||base.model_name||'latest'}else model.value=text;model.readOnly=true;model.disabled=true;const label=model.closest('.field')?.querySelector('label');if(label)label.textContent='迭代起点（锁定最新版本）'}},30);return result};
  window.openTrainSettings428=window.openTrainSettings429;
  const baseRender417=render;
  render=function(){const result=baseRender417?.();const badge=document.getElementById('versionBadge');if(badge)badge.textContent='v42.17.0';return result};
  [120,600,1600].forEach(delay=>setTimeout(()=>{const badge=document.getElementById('versionBadge');if(badge)badge.textContent='v42.17.0'},delay));
