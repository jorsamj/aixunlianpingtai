...
window.openVersionRollback=(aid,targetVersionId)=>{const algorithm=(state.algorithms||[]).find(item=>String(item.id)===String(aid)),versions=algorithm?.versions||[],currentVersionId=String(algorithm?.current_version_id||''),current=versions.find(item=>String(item.id)===currentVersionId),target=versions.find(item=>String(item.id)===String(targetVersionId));if(!algorithm||!current||!target)return toast('版本信息已变化，请刷新算法列表后重试');if(currentVersionId===String(targetVersionId))return toast('该版本已经是当前版本');modal('回退算法版本',`<div class="version-rollback"><div class="version-rollback-algorithm"><span>算法名称</span><b>${esc(algorithm.name||algorithm.code||algorithm.id)}</b></div><div class="version-rollback-flow"><div><span>当前版本</span><b>${esc(current.version_name||current.id)}</b></div><i>→</i><div><span>目标版本</span><b>${esc(target.version_name||target.id)}</b></div></div><label class="version-rollback-delete"><input id="rollbackDeleteCurrent" type="checkbox"><span><b>回退后删除当前版本 ${esc(current.version_name||current.id)}</b><small>默认保留。存在运行中训练、转换或部署测试引用时，服务器会拒绝组合操作。</small></span></label><div class="version-rollback-boundary">当前平台尚无可信在线部署实例注册表；本次会检查可证明的训练、转换和部署测试活动引用。</div><div class="row end"><button class="btn soft" onclick="closeModal()">取消</button><button id="rollbackConfirmButton" class="btn primary" onclick="submitVersionRollback('${esc(aid)}','${esc(targetVersionId)}','${esc(currentVersionId)}')">确认回退</button></div></div>`)};
window.submitVersionRollback=async(aid,targetVersionId,currentVersionId)=>{const button=document.getElementById('rollbackConfirmButton'),deleteCurrent=!!document.getElementById('rollbackDeleteCurrent')?.checked;if(button){button.disabled=true;button.textContent=deleteCurrent?'正在回退并删除…':'正在回退…'}const result=await safe(api(`/api/v12/projects/${pid()}/algorithms/${aid}/versions/${targetVersionId}/rollback`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({delete_current_version:deleteCurrent,expected_current_version_id:currentVersionId})}));if(!result){if(button){button.disabled=false;button.textContent=deleteCurrent?'确认回退并删除':'确认回退'}return}closeModal();const runtime=window.AlgorithmListRuntime;if(!runtime?.refresh){toast('回退已保存，请刷新页面查看当前版本');return}await runtime.refresh({render:true});if(deleteCurrent&&result.cleanup_status&&result.cleanup_status!=='cleanup_completed')toast('版本已回退，部分物理产物清理失败，请查看操作记录');else toast(deleteCurrent?'已回退并删除原当前版本':'已回退，原当前版本已保留')};
  ...
  window.refreshConvertResource428=function(){
    ...
    if(sel){
      ...
      sel.onchange=()=>{
        ...
        if(target==='rockchip'&&chip)chip.value=(r?.supported_chips||[])[0]||'rk3588';
    ...
    const first=ready[0];
    if(chip){
      if(target==='rockchip')chip.value=(first?.supported_chips||[])[0]||'rk3588';
