...
export function installZipImportRuntime({getState=()=>({}),projectId=()=>getState()?.project?.id,notify=m=>window.toast?.(m),fetchImpl=globalThis.fetch,pollMs=1000}={}) {
  ...
  function body(job){const v=zipView(job,jobs),active=activeZipJobs(jobs),queue=v.queue.waits?`队列第 ${v.queue.position} 位 · 前面 ${v.queue.ahead} 个任务`:(active.length>1?`当前 ${active.length} 个活动 ZIP 导入任务`:'后台任务状态以服务器为准');return `<div class="zip411"><section class="zip411-head"><div><b>${esc(job?.file_name||'ZIP 数据导入')}</b><span>${bytes(job?.uploaded_bytes||0)}</span></div><button class="btn" onclick="closeModal()">关闭窗口</button></section><div class="zip411-main"><div class="zip411-progress"><div><span>${esc(v.stage)}</span><b>${Math.round(v.progress)}%</b></div><i><em style="width:${v.progress}%"></em></i><p>${esc(v.message)}</p></div><div class="alert ${v.queue.waits?'warn':'ok'}">${esc(queue)}</div><div class="zip411-times"><div><span>上传时间</span><b>${seconds(job?.upload_seconds)}</b></div><div><span>ZIP扫描</span><b>${seconds(job?.scan_seconds)}</b></div><div><span>后台处理</span><b>${seconds(job?.processing_seconds)}</b></div></div>${result(job)}</div></div>`}
  function uploadBody(){return `<div class="zip411"><div class="zip411-main"><div class="zip411-progress"><div><span>正在上传 ZIP</span><b>${Math.round(uploading?.progress||0)}%</b></div><i><em style="width:${uploading?.progress||0}%"></em></i><p>${esc(uploading?.message||'准备上传')}</p></div><div class="alert warn">服务器返回任务 ID 后，任务进入可刷新恢复的后台阶段。</div></div></div>`}
  ...
  function render(){const d=dock(),active=activeZipJobs(jobs);if(uploading){d.classList.remove('hidden');d.innerHTML=`<span><b>正在上传 ZIP</b><small>${Math.round(uploading.progress)}% · ${esc(uploading.message)}</small></span><strong>展开</strong>`;return}if(!active.length){d.classList.add('hidden');return}current=pickZipJob(jobs);const v=zipView(current,jobs);d.classList.remove('hidden');d.innerHTML=`<span><b>${esc(v.stage)}</b><small>${Math.round(v.progress)}% · ${active.length} 个活动任务</small></span><strong>展开</strong>`}
  function arm(){if(timer)clearTimeout(timer);timer=null;if(!destroyed&&activeZipJobs(jobs).length)timer=setTimeout(()=>reconcile('poll').catch(()=>{}),pollMs)}
  ...
  const runtime={upload,reconcile,open,snapshot:()=>({jobs:[...jobs],current}),destroy(){destroyed=true;if(timer)clearTimeout(timer);document.getElementById('zipImportDurableDock')?.remove()}};
  window.ZipImportRuntime=runtime;window.doUploadZip426=input=>upload(input).catch(()=>{});reconcile('bootstrap').catch(()=>{});return runtime;
}
