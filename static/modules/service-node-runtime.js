...
function meter(label, percent, detail) {
  ...
  return `<div class="node633-meter"><div><span>${escapeHtml(label)}</span><b>${value == null ? '-' : `${Math.round(value)}%`}</b></div><i><em style="width:${width}%"></em></i><small>${escapeHtml(detail || '-')}</small></div>`;
}
...
function gpuCards(node) {
  ...
  if (!rows.length) {
    return `<div class="node633-no-gpu">${gpu.error ? `GPU 探测失败：${escapeHtml(gpu.error)}` : '未检测到 NVIDIA GPU'}</div>`;
  }
  return rows.map(item => {
    ...
    return `<div class="node633-gpu-card">
      <div class="node633-gpu-head"><b>${escapeHtml(item.name || `GPU ${item.index}`)}</b><span>${escapeHtml(item.id || `cuda:${item.index}`)}</span></div>
      ${meter('GPU 利用率', item.utilization_percent, item.temperature_c == null ? '温度 -' : `温度 ${item.temperature_c}℃`)}
      ${meter('显存', vram, `${formatBytes(used)} / ${formatBytes(total)} · 空闲 ${formatBytes(item.memory_free_bytes)}`)}
    </div>`;
  }).join('');
}
...
function workerRows(node) {
  ...
  return workers.map(worker => `<div class="node633-worker">
    <b>${escapeHtml(worker.worker_id || '-')}</b>
    <span>${escapeHtml((worker.roles || []).join(' / ') || '-')}</span>
    <span>${worker.online ? '在线' : '离线'} · PID ${escapeHtml(worker.pid ?? '-')}</span>
  </div>`).join('');
}
...
function taskRows(node) {
  ...
  return tasks.map(task => `<div class="node633-task">
    <div><b>${escapeHtml(task.kind || '-')}</b><span>${escapeHtml(task.task_id || '-')}</span></div>
    <div><span>${escapeHtml(task.stage || task.status || '-')}</span><b>${Math.round(Number(task.progress || 0))}%</b></div>
  </div>`).join('');
}
...
export function renderNodeCard(node) {
  ...
  const memoryPercent = safePercent(memory.usage_percent) ?? (
    Number(memory.total_bytes) > 0 ? Number(memory.used_bytes || 0) / Number(memory.total_bytes) * 100 : null
  );
  const diskPercent = safePercent(disk.usage_percent) ?? (
    Number(disk.total_bytes) > 0 ? Number(disk.used_bytes || 0) / Number(disk.total_bytes) * 100 : null
  );
  return `<article class="node633-card" data-node-card="${escapeHtml(node?.node_id || '')}">
    <header class="node633-card-head">
      <div><div class="node633-title"><b>${escapeHtml(node?.display_name || node?.node_id || '未命名节点')}</b><span class="pill ${status.className}">${escapeHtml(status.label)}</span></div><p>${escapeHtml(node?.node_id || '-')} · ${escapeHtml(node?.hostname || '尚未上报主机名')}</p></div>
      <div class="node633-actions">
        <button class="btn small" data-node-action="edit" data-node-id="${escapeHtml(node?.node_id || '')}">编辑</button>
        <button class="btn small ${node?.enabled ? 'soft' : 'primary'}" data-node-action="toggle" data-node-id="${escapeHtml(node?.node_id || '')}">${node?.enabled ? '停用' : '启用'}</button>
        <button class="btn small" data-node-action="rotate" data-node-id="${escapeHtml(node?.node_id || '')}">轮换 Token</button>
        <button class="btn small danger" data-node-action="delete" data-node-id="${escapeHtml(node?.node_id || '')}">删除</button>
      </div>
    </header>
    <div class="node633-meta-grid">
      <div><span>连接方式</span><b>${node?.connection_mode === 'local' ? '本机 Agent' : '远程 Agent'}</b></div>
      <div><span>最后心跳</span><b>${escapeHtml(timeAgo(node?.heartbeat_age_seconds))}</b></div>
      <div><span>系统</span><b>${escapeHtml([node?.os_name, node?.architecture].filter(Boolean).join(' / ') || '-')}</b></div>
      <div><span>Agent / Build</span><b>${escapeHtml([node?.agent_version, node?.build_id].filter(Boolean).join(' / ') || '-')}</b></div>
    </div>
    ...
    <div class="node633-resource-grid">
      ${meter('CPU', cpu.usage_percent, `${cpu.physical_cores ?? '-'} 物理核 / ${cpu.logical_cores ?? '-'} 逻辑核`)}
      ${meter('内存', memoryPercent, `${formatBytes(memory.used_bytes)} / ${formatBytes(memory.total_bytes)} · 可用 ${formatBytes(memory.available_bytes)}`)}
      ${meter('磁盘', diskPercent, `${escapeHtml(disk.path || '-')} · ${formatBytes(disk.used_bytes)} / ${formatBytes(disk.total_bytes)} · 空闲 ${formatBytes(disk.free_bytes)}`)}
    </div>
    <div class="node633-gpus">${gpuCards(node)}</div>
    <div class="node633-runtime-grid">
      <div><span>PyTorch</span><b>${escapeHtml(runtime.torch_version || '-')}</b></div>
      <div><span>CUDA</span><b>${escapeHtml(runtime.cuda_version || '-')}</b></div>
      <div><span>CUDA 可用</span><b>${runtime.cuda_available === true ? '是' : runtime.cuda_available === false ? '否' : '-'}</b></div>
      <div><span>Agent 进程</span><b>PID ${escapeHtml(process.pid ?? '-')} · RSS ${formatBytes(process.rss_bytes)} · ${escapeHtml(process.threads ?? '-')} 线程</b></div>
      <div><span>文件句柄</span><b>open files ${escapeHtml(process.open_files ?? '-')} · fd/handle ${escapeHtml(process.file_descriptors ?? process.handles ?? '-')}</b></div>
      <div><span>Agent 地址</span><b>${escapeHtml(node?.agent_url || '-')}</b></div>
    </div>
...
function nodeFormHtml(node, capabilities) {
  ...
  return `<div class="form node633-form" data-node-form="1">
    <div class="field"><label>节点 ID</label><input id="node633Id" class="input" ${edit ? 'disabled' : ''} value="${escapeHtml(node?.node_id || '')}" placeholder="例如 gpu-a800-01"></div>
    <div class="field"><label>节点名称</label><input id="node633Name" class="input" value="${escapeHtml(node?.display_name || '')}" placeholder="例如 A800 训练节点"></div>
    <div class="field"><label>连接方式</label><select id="node633Mode" class="select"><option value="agent" ${node?.connection_mode !== 'local' ? 'selected' : ''}>远程 Agent</option><option value="local" ${node?.connection_mode === 'local' ? 'selected' : ''}>本机 Agent</option></select></div>
    <div class="field"><label>Agent 地址</label><input id="node633Url" class="input" value="${escapeHtml(node?.agent_url || '')}" placeholder="可选，例如 http://10.0.0.20:8030"></div>
    <label class="field check"><input id="node633Enabled" type="checkbox" ${node?.enabled !== false ? 'checked' : ''}> 启用该节点</label>
    <div class="field full"><label>允许执行的能力</label><div class="node633-cap-picker">${capabilities.map(capability => `<label><input type="checkbox" value="${escapeHtml(capability)}" ${selected.has(capability) ? 'checked' : ''}><span>${escapeHtml(capabilityLabel(capability))}</span><small>${escapeHtml(capability)}</small></label>`).join('')}</div></div>
    <div class="row end"><button class="btn soft" data-node-form-cancel>取消</button><button class="btn primary" id="node633Save">${edit ? '保存修改' : '创建节点'}</button></div>
  </div>`;
}
...
function tokenModalHtml(node, token) {
  const commands = buildAgentCommands({
    ...
    nodeId: node.node_id,
    token,
    capabilities: node.allowed_capabilities || [],
  });
  return `<div class="node633-token" data-node-token="1">
    <div class="alert warn"><b>Agent Token 只显示这一次</b><div>关闭后平台不会再返回明文 Token；如遗失，请执行“轮换 Token”。</div></div>
    <div class="field"><label>节点</label><div class="node633-secret-row"><code>${escapeHtml(node.node_id)}</code></div></div>
    <div class="field"><label>Agent Token</label><div class="node633-secret-row"><code id="node633TokenValue">${escapeHtml(token)}</code><button class="btn small" data-copy-target="node633TokenValue">复制</button></div></div>
    <div class="field"><label>Linux 启动命令</label><div class="node633-command"><code id="node633LinuxCommand">${escapeHtml(commands.linux)}</code><button class="btn small" data-copy-target="node633LinuxCommand">复制</button></div></div>
    <div class="field"><label>Windows PowerShell 启动命令</label><div class="node633-command"><code id="node633WindowsCommand">${escapeHtml(commands.windows)}</code><button class="btn small" data-copy-target="node633WindowsCommand">复制</button></div></div>
    <div class="row end"><button class="btn primary" data-node-token-close>我已保存 Token</button></div>
  </div>`;
}
...
export function installServiceNodeRuntime({notify = message => window.toast?.(message)} = {}) {
  ...
  function clearPoll() {
    window.PollRegistryRuntime?.clear?.(POLL_KEY);
  }
  ...
  async function load({silent = false} = {}) {
    try {
      ...
      if (Array.isArray(body?.supported_capabilities) && body.supported_capabilities.length) {
        capabilities = body.supported_capabilities;
      }
      return nodes;
    } catch (error) {
  ...
  function pageHtml() {
    return `<section class="node633-shell" data-service-node-page="1">
      <div class="node633-page-head"><div><h2>服务节点</h2><p>统一查看各节点实时资源、Agent/Worker 状态和可执行能力。节点启停只改变中央允许状态，不会绕过现有任务 lease/fencing。</p></div><div class="row"><button class="btn" data-node-page-refresh>刷新</button><button class="btn primary" data-node-page-create>＋ 新增服务节点</button></div></div>
      <div class="node633-list">${nodes.length ? nodes.map(renderNodeCard).join('') : '<div class="empty node633-empty">暂无服务节点。新增节点后，在目标服务器运行 Node Agent 完成注册心跳。</div>'}</div>
    </section>`;
  }
  ...
  function selectedCapabilities(root) {
    return [...root.querySelectorAll('.node633-cap-picker input:checked')].map(input => input.value);
  }
  ...
  function openForm(node = null) {
    ...
    root.querySelector('#node633Save')?.addEventListener('click', async event => {
      ...
      try {
        if (node) {
          await requestJson(`${API_ROOT}/${encodeURIComponent(node.node_id)}`, {
            method: 'PATCH',
          ...
          const created = await requestJson(API_ROOT, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
  ...
  async function toggleNode(nodeId) {
    ...
    if (!node) return;
    try {
      await requestJson(`${API_ROOT}/${encodeURIComponent(nodeId)}`, {
        method: 'PATCH',
        headers: {'Content-Type': 'application/json'},
  ...
  async function rotateToken(nodeId) {
    const node = findNode(nodeId);
    if (!node) return;
    if (!window.confirm?.(`确认轮换 ${node.display_name || nodeId} 的 Agent Token？旧 Token 会立即失效。`)) return;
    try {
  ...
  async function deleteNode(nodeId) {
    const node = findNode(nodeId);
    if (!node) return;
    if (!window.confirm?.(`确认删除服务节点“${node.display_name || nodeId}”？运行中的 Worker/任务存在时服务器会拒绝删除。`)) return;
    try {
